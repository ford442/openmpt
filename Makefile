...
ifeq ($(SHARED_SONAME),1)
	...
	$(INSTALL_MAKE_DIR) $(DESTDIR)$(PREFIX)/lib
	$(INSTALL_LIB) bin/$(FLAVOUR_DIR)libopenmpt3$(SOSUFFIX) $(DESTDIR)$(PREFIX)/lib/libopenmpt3$(SOSUFFIX)
endif
	$(INSTALL_MAKE_DIR) $(DESTDIR)$(PREFIX)/lib
endif
ifeq ($(STATIC_LIB),1)
...
bin/$(FLAVOUR_DIR)libopenmpt$(SOSUFFIX): $(LIBOPENMPT_OBJECTS) $(LIBOPENMPT_LIBS)
	$(INFO) [LD] $@
ifeq ($(NO_SHARED_LINKER_FLAG),1)
	$(SILENT)$(LINK.cc) $(LIBOPENMPT_LDFLAGS) $(SO_LDFLAGS) $^ $(LOADLIBES) $(LDLIBS) -o ttt.js
else
	$(SILENT)$(LINK.cc) -shared $(LIBOPENMPT_LDFLAGS) $(SO_LDFLAGS) $^ $(LOADLIBES) $(LDLIBS) -o ttt.js
endif
ifeq ($(SHARED_SONAME),1)

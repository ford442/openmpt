...
void resize(ORDERINDEX newSize, PATTERNINDEX pat) {
std::vector<PATTERNINDEX>::resize(std::min(MAX_ORDERS, newSize), pat);
}
...
void Replace(PATTERNINDEX oldPat, PATTERNINDEX newPat) {
if(oldPat != newPat)
std::replace(begin(), end(), oldPat, newPat);
}
...
static constexpr PATTERNINDEX
GetInvalidPatIndex() { return uint16_max; }
// Returns the internal representation of an ignore '+++' index
static constexpr PATTERNINDEX
GetIgnoreIndex() { return uint16_max - 1; }
...
// Returns the internal representation of a stop '---' index
static constexpr PATTERNINDEX
GetInvalidPatIndex() { return ModSequence::GetInvalidPatIndex(); }
// Returns the internal representation of an ignore '+++' index
static constexpr PATTERNINDEX
GetIgnoreIndex() { return ModSequence::GetIgnoreIndex(); }
#ifdef MODPLUG_TRACKER

...
MPT_BINARY_STRUCT(SymFileHeader,
8)
struct SymEvent {
enum Command : uint8 {
KeyOn = 0,
...
MPT_BINARY_STRUCT(SymVirtualHeader,
20)
...
const double length =
        sourceSmp.nLength * std::pow(2.0, (event.note - events[0].note) / -12.0) * sourceSmp.nC5Speed * rateFactor;
...
const double increment =
        std::pow(2.0, (event.note - events[0].note) / 12.0 + finetune / 96.0) * sourceSmp.nC5Speed * rateFactor;
...
int32 volume = 4096 * sampleBoost /
               10000;  // avoid clipping the filters if the virtual sample is later also filtered (see e.g. 303 emulator.symmod)
if(!asQueue)
...
const SmpLength procCount = std::min(writeCount - chn.oldOffset, mpt::saturate_round<SmpLength>(
        (chn.nLength - chn.position.ToDouble()) / chn.increment.ToDouble()));
...
CopySample<SC::ConversionChain < SC::ConvertFixedPoint < int16, mixsample_t, 27>, SC::DecodeIdentity < mixsample_t>>>(
        target.sample16() + writeOffset, writeCount, 1, buffer.data(), sizeof(buffer), 2);
writeOffset += writeCount;
}
return true;
}
};
MPT_BINARY_STRUCT(SymVirtualInst,
128)
...
MPT_BINARY_STRUCT(SymTranswaveInst,
128)
...
mptSmp.uFlags.reset(
        CHN_LOOP | CHN_PINGPONGLOOP | CHN_SUSTAINLOOP | CHN_PANNING);  // Avoid these coming in from sample files
...
uint8 effectiveVolume = (volume > 0 && volume < 200) ? static_cast<uint8>(std::min(volume.get(), uint8(100)) * 128u /
                                                                          100) : 128;
...
if(!newSample)
return;
CopySample<SC::ConversionChain < SC::Convert < int16, int8>, SC::DecodeIdentity < int8>>>(newSample, mptSmp.nLength *
                                                                                                     mptSmp.GetNumChannels(), 1, mptSmp.sample8(), mptSmp.GetSampleSizeInBytes(), 1);
...
if(volFadeFlag == 2) {
auto sampleData = mpt::as_span(mptSmp.sample16(), mptSmp.nLength * mptSmp.GetNumChannels());
int32 amp = volFadeFrom << 24, inc = Util::muldivr(volFadeTo - volFadeFrom, 1 << 24,
                                                   static_cast<SmpLength>(sampleData.size()));
for(auto &sample : sampleData) {
...
const int32 cutoffStep = numFilterPoints > 1 ? Util::muldivr(filterPoint[1].cutoff - filterPoint[0].cutoff, 1 << 23,
                                                             static_cast<SmpLength>(sampleData.size())) : 0;
const int32 resoStep = numFilterPoints > 1 ? Util::muldivr(filterPoint[1].resonance - filterPoint[0].resonance, 1 << 23,
                                                           static_cast<SmpLength>(sampleData.size())) : 0;
...
constexpr
int32 scale = 1 << (16 - (sizeof(sampleData[0]) * 8));
for(auto &sample : sampleData) {
...
const auto FindLoopEnd = [](auto sampleData, const uint8 numChannels, SmpLength loopStart, SmpLength loopLen,
                            const int threshold) {
...
if(mptSmp.uFlags[CHN_16BIT])
loopLen = FindLoopEnd(mpt::as_span(mptSmp.sample16(), mptSmp.nLength * mptSmp.GetNumChannels()),
                      mptSmp.GetNumChannels(), loopStart, loopLen, 6 * 256);
else
loopLen = FindLoopEnd(mpt::as_span(mptSmp.sample8(), mptSmp.nLength * mptSmp.GetNumChannels()), mptSmp.GetNumChannels(),
                      loopStart, loopLen, 6);
}
return {loopStart, loopLen};
}
};
MPT_BINARY_STRUCT(SymInstrument,
256)
...
return std::tie(pattern, start, length, transpose, speed) <
       std::tie(other.pattern, other.start, other.length, other.transpose, other.speed);
}
};
MPT_BINARY_STRUCT(SymPosition,
32)
...
sample.nLength = mpt::saturate_cast<SmpLength>(
        file.BytesLeft() / (sampleIO.GetNumChannels() * sampleIO.GetBitDepth() / 8u));
...
constexpr
size_t blockSize = buf.size() / 2;  // Size of block in 16-bit samples
...
mpt::String::WriteAutoBuf(macro) = MPT_AFORMAT("F0F000{} F0F001{} F0F00200")(mpt::afmt::HEX0<2>(cutoff),
                                                                             mpt::afmt::HEX0<2>(reso));
else if(type == 2)  // highpass filter
mpt::String::WriteAutoBuf(macro) = MPT_AFORMAT("F0F000{} F0F001{} F0F00210")(mpt::afmt::HEX0<2>(cutoff),
                                                                             mpt::afmt::HEX0<2>(reso));
...
mpt::String::WriteAutoBuf(macro) = MPT_AFORMAT("F0F080{} F0F081{} F0F082{}")(mpt::afmt::HEX0<2>(type),
                                                                             mpt::afmt::HEX0<2>(length),
                                                                             mpt::afmt::HEX0<2>(feedback));
return true;
...
if(auto numChannels = static_cast<CHANNELINDEX>(file.ReadUint32BE()); !m_nChannels && numChannels > 0 &&
                                                                      numChannels <= MAX_BASECHANNELS) {
...
isSymphoniePro = true;
if(uint32 val = file.ReadUint32BE(); (chunkType == ChunkType::StereoDetune || chunkType == ChunkType::StereoPhase) &&
                                     val)
...
chnState.tonePortaRemain = static_cast<uint16>(distance -
                                               std::min(distance, chnState.tonePortaAmt * (patternSpeed - 1)));
}
break;
...
m.command = CMD_CHANNELVOLUME;
m.param = chnState.calculatedVol = static_cast<uint8>(Util::muldivr_unsigned(chnState.lastVol, chnState.channelVol,
                                                                             100));
}
...
next.param = chnState.calculatedVol = static_cast<uint8>(Util::muldivr_unsigned(chnState.lastVol, chnState.channelVol,
                                                                                100));
}
}
break;
...
sampleVib = chnState.sampleVibDepth *
            (std::sin(chnState.sampleVibPhase * (mpt::numbers::pi * 2.0 / 1024.0) + 1.5 * mpt::numbers::pi) - 1.0) /
            4.0;
...
m.volcmd = VOLCMD_TONEPORTAMENTO;
chnState.tonePortaRemain -= std::min(chnState.tonePortaRemain,
                                     static_cast<uint16>(chnState.tonePortaAmt * (patternSpeed - 1)));
}

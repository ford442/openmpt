...
constexpr
uint32 MagicLE(const char(&id)[5]) {
return static_cast<uint32>((static_cast<uint8>(id[3]) << 24) | (static_cast<uint8>(id[2]) << 16) |
                           (static_cast<uint8>(id[1]) << 8) | static_cast<uint8>(id[0]));
}
constexpr uint16
MagicLE(const char(&id)[3]) {
...
constexpr uint32
MagicBE(const char(&id)[5]) {
return static_cast<uint32>((static_cast<uint8>(id[0]) << 24) | (static_cast<uint8>(id[1]) << 16) |
                           (static_cast<uint8>(id[2]) << 8) | static_cast<uint8>(id[3]));
}
constexpr uint16
MagicBE(const char(&id)[3]) {
...
bool ReadOrderFromArray(ModSequence &order, const T(&orders)[arraySize], size_t howMany = arraySize,
                        uint16 stopIndex = uint16_max, uint16 ignoreIndex = uint16_max) {
...
// 'stopIndex' is treated as '---', 'ignoreIndex' is treated as '+++'. If the format doesn't support such indices, just pass uint16_max.
template<typename T>
bool ReadOrderFromFile(ModSequence &order, FileReader &file, size_t howMany, uint16 stopIndex = uint16_max,
                       uint16 ignoreIndex = uint16_max) {
static_assert(mpt::is_binary_safe<T>::value);

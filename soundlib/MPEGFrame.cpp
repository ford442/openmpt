...
MPEGFrame::MPEGFrame(FileReader &file)
        : frameSize(0), numSamples(0), isValid(false), isLAME(false) {
uint8 header[4];
...
isValid = true;
frameSize = (((mpegCoefficients[mpeg1][layer] * (bitRates[mpeg1][layer][bitRate] * 1000) /
               samplingRates[version][sampleRate]) + padding)) * (layer == 0 ? 4 : 1);

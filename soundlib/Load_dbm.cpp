...
MPT_BINARY_STRUCT(DBMFileHeader,
8)
// IFF-style Chunk
struct DBMChunk {
...
MPT_BINARY_STRUCT(DBMInfoChunk,
10)
...
MPT_BINARY_STRUCT(DBMInstrument,
50)
...
m_nChannels = Clamp<uint16, uint16>(infoData.channels, 1,
                                    MAX_BASECHANNELS);    // note: MAX_BASECHANNELS is currently 127, but DBPro 2 supports up to 128 channels, DBPro 3 apparently up to 254.
m_nInstruments = std::min(static_cast<INSTRUMENTINDEX>(infoData.instruments),
...
m_modFormat.madeWithTracker = MPT_UFORMAT("DigiBooster Pro {}.{}")(mpt::ufmt::hex(fileHeader.trkVerHi),
                                                                   mpt::ufmt::hex(fileHeader.trkVerLo));
...
lostGlobalCommands.insert(lostGlobalCommands.begin(),
                          lostCommand);  // Insert at front so that the last command of same type "wins"
...
DigiBoosterEcho::PluginChunk chunk = DigiBoosterEcho::PluginChunk::Create(settings[1], settings[3], settings[5],
                                                                          settings[7]);

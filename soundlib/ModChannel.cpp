...
void
ModChannel::Reset(ResetFlags resetMask, const CSoundFile &sndFile, CHANNELINDEX sourceChannel, ChannelFlags muteFlag) {
...
void ModChannel::RecalcTuningFreq(Tuning::RATIOTYPE vibratoFactor, Tuning::NOTEINDEXTYPE arpeggioSteps,
...
nPeriod = mpt::saturate_round<uint32>((nC5Speed << FREQ_FRACBITS) * vibratoFactor *
                                      pModInstrument->pTuning->GetRatio(note - NOTE_MIDDLEC + arpeggioSteps,
                                                                        nFineTune + m_PortamentoFineSteps));
}
// IT command S73-S7E
void ModChannel::InstrumentControl(uint8
param,
const CSoundFile &sndFile
)
{
...
break;
case 0x7: VolEnv.flags.
reset(ENV_ENABLED);
break;
case 0x8: VolEnv.flags.
set(ENV_ENABLED);
break;
case 0x9: PanEnv.flags.
reset(ENV_ENABLED);
break;
case 0xA: PanEnv.flags.
set(ENV_ENABLED);
break;
case 0xB: PitchEnv.flags.
reset(ENV_ENABLED);
break;
case 0xC: PitchEnv.flags.
set(ENV_ENABLED);
break;

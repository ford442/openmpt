...
MPT_BINARY_STRUCT(KMChunkHeader,
8)
...
MPT_BINARY_STRUCT(KMSampleHeader,
40)
...
MPT_BINARY_STRUCT(KMSampleReference,
34)
...
MPT_BINARY_STRUCT(KMSongHeader,
32 + 31 * 34 + 14)
...
MPT_BINARY_STRUCT(KMFileHeader,
sizeof(KMChunkHeader) + sizeof(KMSongHeader))
...
MUS_SAMPLE_UNUSED = 255;  // Sentinel value to check if a sample needs to be duplicated
...
static constexpr ROWINDEX
MUS_PATTERN_LENGTH = 64;
...
uint8 mask;
} effTrans[] =
        {
                ...
                {CMD_MODCMDEX,       0x50},
                {CMD_OFFSET,         0x00},
                {CMD_TONEPORTAMENTO, 0x00},
                {CMD_TONEPORTAVOL,   0x00},
                {CMD_VIBRATO,        0x00},
                {CMD_VIBRATOVOL,     0x00},
                {CMD_ARPEGGIO,       0x00},
                {CMD_PORTAMENTOUP,   0x00},
                {CMD_PORTAMENTODOWN, 0x00},
                {CMD_VOLUMESLIDE,    0x00},
                {CMD_MODCMDEX,       0x90},
                {CMD_TONEPORTAMENTO, 0xFF},
                {CMD_MODCMDEX,       0xC0},
                {CMD_SPEED,          0x00},
                {CMD_TREMOLO,        0x00},
        };
...
Patterns[pat].WriteEffect(
        EffectWriter(CMD_PATTERNBREAK, static_cast<ModCommand::PARAM>(restartRow)).Row(row).RetryNextRow());
}
}
Order.SetSequence(0);

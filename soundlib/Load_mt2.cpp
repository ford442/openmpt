...
MPT_BINARY_STRUCT(MT2FileHeader,
126)
...
MPT_BINARY_STRUCT(MT2DrumsData,
274)
...
MPT_BINARY_STRUCT(MT2TrackSettings,
1030)
...
MPT_BINARY_STRUCT(MT2Instrument,
106)
...
MPT_BINARY_STRUCT(MT2IEnvelope,
72)
...
MPT_BINARY_STRUCT(MT2InstrSynth,
32)
...
static void ReadMT2Automation(uint16
version,
FileReader &file
)
{
...
InitializeChannels();
m_modFormat.formatName = MPT_UFORMAT("MadTracker {}.{}")(fileHeader.version >> 8,
                                                         mpt::ufmt::hex0<2>(fileHeader.version & 0xFF));
m_modFormat.type = U_("mt2");
m_modFormat.madeWithTracker = mpt::ToUnicode(mpt::Charset::Windows1252,
                                             mpt::String::ReadBuf(mpt::String::maybeNullTerminated,
                                                                  fileHeader.trackerName));
...
m_nVSTiVolume = 48;
m_nSamplePreAmp = 48 *
                  2;    // Double pre-amp because we will halve the volume of all non-drum instruments, because the volume of drum samples can exceed that of normal samples
...
const ROWINDEX fillRows = std::min((uint32)
repeatCount, (uint32)
numRows - (row + 1));
...
const NewNoteAction NNA[4] = {NewNoteAction::NoteCut, NewNoteAction::Continue, NewNoteAction::NoteOff,
                              NewNoteAction::NoteFade};
const DuplicateCheckType DCT[4] = {DuplicateCheckType::None, DuplicateCheckType::Note, DuplicateCheckType::Sample,
                                   DuplicateCheckType::Instrument};
const DuplicateNoteAction DNA[4] = {DuplicateNoteAction::NoteCut,
                                    DuplicateNoteAction::NoteFade /* actually continue, but IT doesn't have that */,
                                    DuplicateNoteAction::NoteOff, DuplicateNoteAction::NoteFade};
...
if(sampleHeader.depth > 1) {
mptSmp.uFlags.set(CHN_16BIT);
mptSmp.nLength /= 2u;
}
if(sampleHeader.channels > 1) {

...
static uint32 SaveITEditHistory(const CSoundFile &sndFile, std::ostream *file)
{
	...
	// Process existing history items
	if(start < sndFile.GetFileHistory().size())
	{
		size_t count = std::min(num, sndFile.GetFileHistory().size()) - start;
		if(count > 0)
		{
			for(const auto &mptHistoryEntry : mpt::as_span(sndFile.GetFileHistory()).subspan(start, count))
			{
				FileHistory mptHistory = mptHistoryEntry; // Make a copy as in original
				ITHistoryStruct itHistory;
	...
	// Handle appending current session history if applicable (MODPLUG_TRACKER specific)
#ifdef MODPLUG_TRACKER
	if(pModDoc != nullptr)
	{
		size_t existingHistorySize = sndFile.GetFileHistory().size();
		// Only append new entries if 'num' indicates that more entries than currently exist should be written,
		// and 'start' was such that we weren't already past the end of existing history.
		if (num > existingHistorySize && start <= existingHistorySize)
		{
			// Number of new entries to generate corresponds to the iterations
			// where 'n' in the original loop would be >= existingHistorySize and < num.
			// In this refactoring, this means we are adding exactly one "current session" entry if it's captured by 'num'.
			if (existingHistorySize < num) // This check ensures we are indeed adding the "current session"
			{
				FileHistory mptHistory;
				MemsetZero(mptHistory.loadDate);
				SYSTEMTIME systime;
				GetLocalTime(&systime);
				mptHistory.loadDate = mpt::Date::AnyGregorian::FromSYSTEMTIME(systime);
				if(sndFile.GetFileHistory().empty()) // Check against actual current history size for this new entry
				{
					mptHistory.openTime = static_cast<uint32>(pModDoc->GetEditTime().Get கர்seconds() * HISTORY_TIMER_PRECISION);
				} else
				{
					// Add time from previous session (the last one actually in sndFile.GetFileHistory())
					mptHistory.openTime = sndFile.GetFileHistory().back().openTime + static_cast<uint32>(pModDoc->GetEditTime().Get கர்seconds() * HISTORY_TIMER_PRECISION);
				}
				ITHistoryStruct itHistory;
				itHistory.ConvertToIT(mptHistory);
				mpt::IO::Write(f, itHistory);
			}
		} else if (num > start && start >= existingHistorySize)
		{
			// This case handles when 'start' is already beyond existing history,
			// so all 'fnum' (derived from 'num - start') entries are new.
			// This implies we are only writing "current session" entries, possibly multiple if history was short.
			for (size_t i = 0; i < (num - start); ++i)
			{
				...
				// Logic for openTime if multiple "new" entries are generated:
				// The original loop's accumulation was based on 'n' and existingHistorySize.
				// If start >= existingHistorySize, all entries are "new".
				// The first "new" entry (i=0 here) should behave as if history was empty for accumulation start,
				// or chain from the actual last entry if history was not empty *before this save operation started*.
				// This simplified logic might not perfectly replicate the old accumulation if many entries are cut by 'start'
				// AND multiple new entries are generated by 'num'.
				// However, typically 'num' is existingHistorySize + 1 (or just existingHistorySize if pModDoc is null).
				// If 'start' cuts into existingHistorySize, and num is existingHistorySize + 1, then
				// the range-based loop handles the remaining existing, and this block handles the +1.
				if(sndFile.GetFileHistory().empty())
				{
					 // This assumes GetEditTime() is total time for this session if history is empty.
					 // If multiple purely new entries were to be chained, this would need an accumulator starting from 0 for this block.
					mptHistory.openTime = static_cast<uint32>(pModDoc->GetEditTime().Get கர்seconds() * HISTORY_TIMER_PRECISION);
				} else
				{
					mptHistory.openTime = sndFile.GetFileHistory().back().openTime + static_cast<uint32>(pModDoc->GetEditTime().Get கர்seconds() * HISTORY_TIMER_PRECISION);

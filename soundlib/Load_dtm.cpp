...
MPT_BINARY_STRUCT(DTMFileHeader,
22)
// IFF-style Chunk
struct DTMChunk {
...
MPT_BINARY_STRUCT(DTMInstrument,
15)
...
MPT_BINARY_STRUCT(DTMEnvelope::DTMEnvPoint,
2)
MPT_BINARY_STRUCT(DTMEnvelope,
34)
...
if(patternFormat != DTM_PT_PATTERN_FORMAT && patternFormat != DTM_204_PATTERN_FORMAT &&
   patternFormat != DTM_206_PATTERN_FORMAT) {
return false;
}
} else {
return false;
...
ChnSettings[chn].nPan = static_cast<uint16>(128 +
                                            Util::muldivr(std::min(static_cast<int>(panning[chn]), int(180)) - 90, 128,
                                                          90));
}
chunk.Skip(16);
...
mptEnv.resize(std::min(
        {static_cast<std::size_t>(env.numPoints), std::size(env.points), static_cast<std::size_t>(MAX_ENVPOINTS)}));
...
m->vol = std::min(volume,
                  uint8(64));  // Volume can go up to 255, but we do not support over-amplification at the moment.
}
if(instr) {
m->instr = instr;
}

...
void InstrumentEnvelope::Sanitize(uint8
maxValue)
{
if(!
empty()
...
{
it->
tick = std::max(it->tick, (it - 1)->tick);
LimitMax(it
->value, maxValue);
}
}
...
LimitMax(nSustainEnd,
static_cast<
decltype(nSustainEnd)
>(
size()
- 1));
...
void ModInstrument::Transpose(int8
amount)
{
for(
auto &note

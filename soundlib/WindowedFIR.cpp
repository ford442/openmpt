...
double CWindowedFIR::coef(int _PCnr, double _POfs, double _PCut, int _PWidth,
                          int _PType) //float _PPos, float _PFc, int _PLen )
{
...
_LWc = 0.35875 - 0.48829 * std::cos(_LPIdl * _LPosU) + 0.14128 * std::cos(2.0 * _LPIdl * _LPosU) -
       0.01168 * std::cos(3.0 * _LPIdl * _LPosU);
break;
case WFIR_BLACKMAN4T74:
_LWc = 0.40217 - 0.49703 * std::cos(_LPIdl * _LPosU) + 0.09392 * std::cos(2.0 * _LPIdl * _LPosU) -
       0.00183 * std::cos(3.0 * _LPIdl * _LPosU);
break;
case WFIR_KAISER4T: // kaiser-bessel, alpha~7.5
_LWc = 0.40243 - 0.49804 * std::cos(_LPIdl * _LPosU) + 0.09831 * std::cos(2.0 * _LPIdl * _LPosU) -
       0.00122 * std::cos(3.0 * _LPIdl * _LPosU);
break;
default:
_LWc = 1.0;
break;
...
lut[_LIdx + _LCc] = (signed short) ((_LCoef < -WFIR_QUANTSCALE) ? -WFIR_QUANTSCALE : ((_LCoef > WFIR_QUANTSCALE)
                                                                                      ? WFIR_QUANTSCALE : _LCoef));
#else

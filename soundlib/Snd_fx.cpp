...
#else
static constexpr
auto GLOBALVOL_7BIT_FORMATS_EXT = MOD_TYPE_NONE;
#endif // MODPLUG_TRACKER
static constexpr auto GLOBALVOL_7BIT_FORMATS =
        MOD_TYPE_IT | MOD_TYPE_MPT | MOD_TYPE_IMF | MOD_TYPE_J2B | MOD_TYPE_MID | MOD_TYPE_AMS | MOD_TYPE_DBM |
        MOD_TYPE_PTM | MOD_TYPE_MDL | MOD_TYPE_DTM | GLOBALVOL_7BIT_FORMATS_EXT;
...
static uint32 GetLinearSlideDownTable(const CSoundFile *sndFile, uint32 i) {
MPT_ASSERT(i < std::size(LinearSlideDownTable));
return sndFile->m_playBehaviour[kPeriodsAreHertz] ? LinearSlideDownTable[i] : LinearSlideUpTable[i];
}
static uint32 GetLinearSlideUpTable(const CSoundFile *sndFile, uint32 i) {
MPT_ASSERT(i < std::size(LinearSlideDownTable));
return sndFile->m_playBehaviour[kPeriodsAreHertz] ? LinearSlideUpTable[i] : LinearSlideDownTable[i];
}
static uint32 GetFineLinearSlideDownTable(const CSoundFile *sndFile, uint32 i) {
MPT_ASSERT(i < std::size(FineLinearSlideDownTable));
return sndFile->m_playBehaviour[kPeriodsAreHertz] ? FineLinearSlideDownTable[i] : FineLinearSlideUpTable[i];
}
static uint32 GetFineLinearSlideUpTable(const CSoundFile *sndFile, uint32 i) {
MPT_ASSERT(i < std::size(FineLinearSlideDownTable));
return sndFile->m_playBehaviour[kPeriodsAreHertz] ? FineLinearSlideUpTable[i] : FineLinearSlideDownTable[i];
}
...
IGNORE_CHANNEL = uint32_max;
GetLengthMemory(const CSoundFile &sf)
        : sndFile(sf), state(std::make_unique<CSoundFile::PlayState>(sf.m_PlayState)) {
Reset();
}
void Reset() {
#ifndef NO_PLUGINS
...
if(numTicks == IGNORE_CHANNEL || numTicks == 0 || (!chn.IsSamplePlaying() && !chnSettings[channel].incChanged) ||
...
forbiddenCommands.set(CMD_PORTAMENTOUP);
forbiddenCommands.set(CMD_PORTAMENTODOWN);
forbiddenCommands.set(CMD_XFINEPORTAUPDOWN);
forbiddenCommands.set(CMD_NOTESLIDEUP);
forbiddenCommands.set(CMD_NOTESLIDEUPRETRIG);
forbiddenCommands.set(CMD_NOTESLIDEDOWN);
forbiddenCommands.set(CMD_NOTESLIDEDOWNRETRIG);
forbiddenVolCommands.set(VOLCMD_PORTAUP);
forbiddenVolCommands.set(VOLCMD_PORTADOWN);
...
playState.m_nPattern = playState.m_nCurrentOrder < orderList.size() ? orderList[playState.m_nCurrentOrder]
                                                                    : orderList.GetInvalidPatIndex();
if(!Patterns.IsValidPat(playState.m_nPattern) && playState.m_nPattern != orderList.GetInvalidPatIndex() &&
   target.mode == GetLengthTarget::SeekPosition && playState.m_nCurrentOrder == target.pos.order) {
...
if((!Patterns.IsValidPat(playState.m_nPattern)) &&
   visitedRows.Visit(playState.m_nCurrentOrder, 0, playState.Chn, ignoreRow)) {
if(!hasSearchTarget) {
retval.lastOrder = playState.m_nCurrentOrder;
retval.lastRow = 0;
}
if(target.mode == GetLengthTarget::NoTarget ||
   !visitedRows.GetFirstUnvisitedRow(playState.m_nNextOrder, playState.m_nRow, true)) {
...
if(target.mode == GetLengthTarget::SeekPosition && playState.m_nCurrentOrder == target.pos.order &&
   playState.m_nRow == target.pos.row) {
retval.targetReached = true;
break;
}
...
const bool moduleTooComplex =
        target.mode != GetLengthTarget::SeekSeconds && visitedRows.ModuleTooComplex(allowedPatternLoopComplexity);
if(moduleTooComplex) {
...
case CMD_VIBRATOVOL:
if(adjustMode & eAdjust) {
uint32 vibTicks = ((GetType() & (MOD_TYPE_IT | MOD_TYPE_MPT)) && !m_SongFlags[SONG_ITOLDEFFECTS]) ? numTicks
                                                                                                  : nonRowTicks;
...
case CMD_TREMOLO:
if(adjustMode & eAdjust) {
uint32 tremTicks = ((GetType() & (MOD_TYPE_IT | MOD_TYPE_MPT)) && !m_SongFlags[SONG_ITOLDEFFECTS]) ? numTicks
                                                                                                   : nonRowTicks;
...
if(m.vol <= std::size(chn.pModSample->cues) && chn.pModSample != nullptr) {
SmpLength offset;
if(m.vol == 0)
...
switch (m.param & 0xF0) {
case 0xA0:
FineVolumeUp(chn, m.param & 0x0F, false);
break;
case 0xB0:
FineVolumeDown(chn, m.param & 0x0F, false);
break;
}
}
break;
...
memory.RenderChannel(nChn, oldTickDuration);  // Re-sync what we've got so far
SetFinetune(nChn, playState,
            false);  // TODO should render each tick individually for CMD_FINETUNE_SMOOTH for higher sync accuracy
break;
}
chn.isFirstTick = true;
switch (m.volcmd) {
case VOLCMD_FINEVOLUP:
FineVolumeUp(chn, m.vol, m_playBehaviour[kITVolColMemory]);
break;
case VOLCMD_FINEVOLDOWN:
FineVolumeDown(chn, m.vol, m_playBehaviour[kITVolColMemory]);
break;
case VOLCMD_VOLSLIDEUP:
case VOLCMD_VOLSLIDEDOWN: {
...
if(m_nInstruments && !instrumentChanged && sampleChanged && chn.pCurrentSample != nullptr &&
   m_playBehaviour[kITMultiSampleInstrumentNumber] && !chn.rowCommand.IsNote()) {
returnAfterVolumeAdjust = true;
}
...
if((instrumentChanged || sampleChanged) && bPorta && m_playBehaviour[kFT2PortaIgnoreInstr] &&
   (chn.pModInstrument != nullptr || chn.pModSample != nullptr)) {
...
// Update Volume
if(bUpdVol && (!(GetType() & (MOD_TYPE_MOD | MOD_TYPE_S3M)) ||
               ((pSmp != nullptr && pSmp->HasSampleData()) || chn.HasMIDIOutput()))) {
if(pSmp) {
...
resetAlways = !(GetType() & (MOD_TYPE_IT | MOD_TYPE_MPT | MOD_TYPE_DBM)) || instrumentChanged || pIns == nullptr ||
...
if((m_playBehaviour[kITPingPongNoReset] || !(GetType() & (MOD_TYPE_IT | MOD_TYPE_MPT))) && pSmp == chn.pModSample &&
   !instrumentChanged)
chn.dwFlags = (chn.dwFlags & (CHN_CHANNELFLAGS | CHN_PINGPONGFLAG));
else
...
if((pIns->PitchEnv.dwFlags & (ENV_ENABLED | ENV_FILTER)) == (ENV_ENABLED | ENV_FILTER) &&
   !m_playBehaviour[kITFilterBehaviour]) {
...
if(chn.rowCommand.note == NOTE_KEYOFF && m_playBehaviour[kITInstrWithNoteOffOldEffects] &&
   m_SongFlags[SONG_ITOLDEFFECTS] && sampleChanged) {
if(chn.pModSample) {
...
void CSoundFile::NoteChange(ModChannel &chn, int note, bool bPorta, bool bResetEnv, bool bManual,
                            CHANNELINDEX channelHint) const {
...
if((!(GetType() & (MOD_TYPE_IT | MOD_TYPE_MPT))) || (m_nInstruments != 0 && !m_playBehaviour[kITInstrWithNoteOff]))
chn.nVolume = 0;
...
chn.nPortamentoDest = pIns->pTuning->GetStepDistance(chn.nNote, chn.m_PortamentoFineSteps,
                                                     static_cast<Tuning::NOTEINDEXTYPE>(note), 0);
...
// Volume Swing
if(pIns->nVolSwing) {
chn.nVolSwing = static_cast<int16>(((mpt::random<int8>(AccessPRNG()) * pIns->nVolSwing) / 64 + 1) *
                                   (m_playBehaviour[kITSwingBehaviour] ? chn.nInsVol : ((chn.nVolume + 1) / 2)) / 199);
}
// Pan Swing
if(pIns->nPanSwing) {
...
if(bPorta ||
   !(m_playBehaviour[kFT2PortaTargetNoReset] || m_playBehaviour[kITClearPortaTarget] || GetType() == MOD_TYPE_MOD)) {
...
if(!m_playBehaviour[kITVibratoTremoloPanbrello] && (GetType() & (MOD_TYPE_IT | MOD_TYPE_MPT)) &&
...
if(m_playBehaviour[kITFT2DontResetNoteOffOnPorta] && bPorta &&
   (!m_SongFlags[SONG_ITCOMPATGXX] || chn.rowCommand.instr == 0))
...
if(isFirstTick) {
PlugParamValue targetvalue = ModCommand::GetValueEffectCol(chn.rowCommand.command, chn.rowCommand.param) /
                             PlugParamValue(ModCommand::maxColumnValue);
chn.m_plugParamTargetValue = targetvalue;
chn.m_plugParamValueStep = (targetvalue - m_MixPlugins[plugin - 1].pMixPlugin->GetParameter(plugparam)) /
                           PlugParamValue(m_PlayState.TicksOnRow());
}
if(m_PlayState.m_nTickCount + 1 ==
   m_PlayState.TicksOnRow()) {    // On last tick, set parameter exactly to target value.
...
continue;
} else if(nStartTick >= (m_PlayState.m_nMusicSpeed + m_PlayState.m_nFrameDelay) &&
          m_playBehaviour[kITOutOfRangeDelay]) {
...
if(nStartTick != 0 && chn.rowCommand.note == NOTE_KEYOFF && chn.rowCommand.volcmd == VOLCMD_PANNING &&
   m_playBehaviour[kFT2PanWithDelayedNoteOff]) {
...
} else if((GetType() & (MOD_TYPE_IT | MOD_TYPE_MPT | MOD_TYPE_J2B)) && GetNumInstruments() != 0 &&
...
bool keepInstr = (GetType() & (MOD_TYPE_IT | MOD_TYPE_MPT))
                 ...
                 || (m_playBehaviour[kMODSampleSwap] && !chn.IsSamplePlaying() &&
...
bPorta = false;
          ...
          chn.position.GetUInt() > chn.nLength) {
...
if(instrChange) {
InstrumentChange(chn, chn.nNewIns, bPorta, chn.pModSample == nullptr && chn.pModInstrument == nullptr,
                 !(GetType() & (MOD_TYPE_XM | MOD_TYPE_MT2)));
chn.nNewIns = 0;
}
...
if(m_playBehaviour[kST3NoMutedChannels] &&
   ChnSettings[nChn].dwFlags[CHN_MUTE])    // not even effects are processed on muted S3M channels
continue;
...
doVolumeColumn = m_PlayState.m_nTickCount != 0 && (m_PlayState.m_nTickCount != nStartTick ||
                                                   (chn.rowCommand.instr == 0 && volcmd != VOLCMD_TONEPORTAMENTO));
}
...
if((!chn.nPeriod || !chn.nNote)
   ...
   && !m_playBehaviour[kITArpeggio] && (GetType() & (MOD_TYPE_IT | MOD_TYPE_MPT)))
break;
if(!param && (GetType() & (MOD_TYPE_XM | MOD_TYPE_MOD)))
...
NoteSlide(chn, param, cmd == CMD_NOTESLIDEUP || cmd == CMD_NOTESLIDEUPRETRIG,
          cmd == CMD_NOTESLIDEUPRETRIG || cmd == CMD_NOTESLIDEDOWNRETRIG);
break;
...
const bool doFineSlides = !doFinePortamentoAsRegular && !(GetType() &
                                                          (MOD_TYPE_MOD | MOD_TYPE_XM | MOD_TYPE_MT2 | MOD_TYPE_MED |
                                                           MOD_TYPE_AMF0 | MOD_TYPE_DIGI | MOD_TYPE_STP |
                                                           MOD_TYPE_DTM));
...
// Test case: Porta-LinkMem.xm
if(param) chn.nOldFinePortaUpDown = (chn.nOldFinePortaUpDown & 0x0F) | (param << 4);
else
param = (chn.nOldFinePortaUpDown >> 4);
...
// FT2 compatibility: E1x / E2x / X1x / X2x memory is not linked
// Test case: Porta-LinkMem.xm
if(param) chn.nOldFinePortaUpDown = (chn.nOldFinePortaUpDown & 0xF0) | (param & 0x0F);
else
param = (chn.nOldFinePortaUpDown & 0x0F);
} else if(GetType() == MOD_TYPE_MT2) {
if(param) chn.nOldFinePortaUpDown = param; else param = chn.nOldFinePortaUpDown;
}
if(chn.isFirstTick && chn.nPeriod && param) {
DoFreqSlide(chn, chn.nPeriod, param * -4);
if(chn.nPeriod > 0xFFFF && !m_playBehaviour[kPeriodsAreHertz] &&
   (!m_SongFlags[SONG_LINEARSLIDES] || GetType() == MOD_TYPE_XM))
chn.nPeriod = 0xFFFF;
}
}
...
// FT2 compatibility: E1x / E2x / X1x / X2x memory is not linked
// Test case: Porta-LinkMem.xm
if(param) chn.nOldExtraFinePortaUpDown = (chn.nOldExtraFinePortaUpDown & 0x0F) | (param << 4);
else
param = (chn.nOldExtraFinePortaUpDown >> 4);
} else if(GetType() == MOD_TYPE_MT2) {
if(param) chn.nOldFinePortaUpDown = param; else param = chn.nOldFinePortaUpDown;
}
if(chn.isFirstTick && chn.nPeriod && param)
...
// FT2 compatibility: E1x / E2x / X1x / X2x memory is not linked
// Test case: Porta-LinkMem.xm
if(param) chn.nOldExtraFinePortaUpDown = (chn.nOldExtraFinePortaUpDown & 0xF0) | (param & 0x0F);
else
param = (chn.nOldExtraFinePortaUpDown & 0x0F);
} else if(GetType() == MOD_TYPE_MT2) {
if(param) chn.nOldFinePortaUpDown = param; else param = chn.nOldFinePortaUpDown;
}
if(chn.isFirstTick && chn.nPeriod && param) {
...
int16 newTuning = mpt::saturate_cast<int16>(
        static_cast<int32>(CalculateXParam(playState.m_nPattern, playState.m_nRow, channel, nullptr)) - 0x8000);
if(isSmooth) {
...
if(GetType() & (MOD_TYPE_IT | MOD_TYPE_MPT | MOD_TYPE_AMS | MOD_TYPE_DMF | MOD_TYPE_DBM | MOD_TYPE_IMF | MOD_TYPE_PSM |
                MOD_TYPE_J2B | MOD_TYPE_ULT | MOD_TYPE_OKT | MOD_TYPE_MT2 | MOD_TYPE_MDL)) {
...
if((GetType() &
    (MOD_TYPE_MOD | MOD_TYPE_XM | MOD_TYPE_MT2 | MOD_TYPE_MED | MOD_TYPE_DIGI | MOD_TYPE_STP | MOD_TYPE_DTM))) {
...
if(!chn.isFirstTick || m_SongFlags[SONG_FASTVOLSLIDES] ||
   (m_PlayState.m_nMusicSpeed == 1 && GetType() == MOD_TYPE_DBM)) {
...
if(param) chn.nOldFineVolUpDown = (param << 4) | (chn.nOldFineVolUpDown & 0x0F);
else
param = (chn.nOldFineVolUpDown >> 4);
} else if(volCol) {
...
case 0x10:
if((param) || (GetType() & (MOD_TYPE_XM | MOD_TYPE_MT2))) FinePortamentoUp(chn, param);
break;
// E2x: Fine Portamento Down
case 0x20:
if((param) || (GetType() & (MOD_TYPE_XM | MOD_TYPE_MT2))) FinePortamentoDown(chn, param);
break;
// E3x: Set Glissando Control
case 0x30:
chn.dwFlags.set(CHN_GLISSANDO, param != 0);
break;
...
// EAx: Fine Volume Up
case 0xA0:
if((param) || (GetType() & (MOD_TYPE_XM | MOD_TYPE_MT2))) FineVolumeUp(chn, param, false);
break;
// EBx: Fine Volume Down
case 0xB0:
if((param) || (GetType() & (MOD_TYPE_XM | MOD_TYPE_MT2))) FineVolumeDown(chn, param, false);
break;
// ECx: Note Cut
case 0xC0:
...
// S9x: Sound Control
case 0x90:
ExtendedChannelEffect(chn, param);
break;
// SAx: Set 64k Offset
case 0xA0:
if(m_SongFlags[SONG_FIRSTTICK]) {
...
const int vol = Util::muldiv((chn.nVolume + swing) * m_PlayState.m_nGlobalVolume, chn.nGlobalVol * chn.nInsVol,
                             1 << 20);
...
uint32 CSoundFile::SendMIDIData(CHANNELINDEX nChn, bool isSmooth, const unsigned char *macro, uint32 macroLen,
                                PLUGINDEX plugin) {
if(macroLen < 1) {
return 0;
}
...
chn.nResonance = (uint8)
CalculateSmoothParamChange((float) chn.nResonance, (float) param);
}
chn.nRestoreResonanceOnNewNote = 0;
...
if(plugChannel > 0 && plugChannel <=
                      GetNumChannels())    // XXX do we need this? I guess it might be relevant for previewing notes in the pattern... Or when using this mechanism for volume/panning!
{
PLUGINDEX plug = 0;
...
if(m.vol == 0)
offset = Util::muldivr_unsigned(chn.nLength, offset,
                                256u << (8u * std::max(uint32(1), extendedRows)));  // o00 + Oxx = Percentage Offset
...
if(note >= NOTE_MIN && note <= NOTE_MAX && chn.nLength &&
   (!chn.dwFlags[CHN_ADLIB] || GetType() != MOD_TYPE_S3M || m_playBehaviour[kOPLRealRetrig]))
...
offset--;
if(offset >= 0 && offset <= static_cast<int>(std::size(chn.pModSample->cues)) && chn.pModSample != nullptr) {
if(offset == 0)
offset = chn.oldOffset;
else
...
if(amount < 0) {
// Go down
period = mpt::saturate_cast<int32>(
        Util::mul32to64_unsigned(1712 * 8363, period) / (Util::mul32to64_unsigned(period, -amount) + 1712 * 8363));
} else if(amount > 0) {
// Go up
...
const ModInstrument *pIns = chn.pModInstrument;
if((pIns->VolEnv.dwFlags[ENV_LOOP] || (GetType() & (MOD_TYPE_XM | MOD_TYPE_MT2 | MOD_TYPE_MDL))) &&
   pIns->nFadeOut != 0) {
chn.dwFlags.set(CHN_NOTEFADE);
}
if(pIns->VolEnv.nReleaseNode != ENV_RELEASE_NODE_UNSET && chn.VolEnv.nEnvValueAtReleaseJump == NOT_YET_RELEASED) {
chn.VolEnv.nEnvValueAtReleaseJump = mpt::saturate_cast<int16>(
        pIns->VolEnv.GetValueFromPosition(chn.VolEnv.nEnvPosition, 256));
...
TEMPO CSoundFile::ConvertST2Tempo(uint8
tempo)
{
...
static constexpr uint32
st2MixingRate = 23863; // Highest possible setting in ST2
...
TEMPO()
.
SetRaw(Util::muldivrfloor(st2MixingRate, 5 * TEMPO::fractFact, samplesPerTick * 2)
);
}
...
uint32 CSoundFile::GetNoteFromPeriod(uint32
period,
int32 nFineTune, uint32
nC5Speed) const
{
if(!period) return 0;
...
uint32 CSoundFile::GetPeriodFromNote(uint32
note,
int32 nFineTune, uint32
nC5Speed) const
{
...
NOTE_MIN;
if(!
UseFinetuneAndTranspose()
)
{
if(
GetType() &
(MOD_TYPE_MDL | MOD_TYPE_DTM))
{
...
return
Util::muldiv_unsigned(nC5Speed, LinearSlideUpTable[(note % 12u) * 16u]
<< (note / 12u), 65536 << 5);
...
if ((nFineTune) || (note < 24) || (note >= 24 +
std::size(ProTrackerPeriodTable)
))
...
uint32 CSoundFile::GetFreqFromPeriod(uint32
period,
uint32 c5speed, int32
nPeriodFrac) const
{
if (!period) return 0;
if (
...
return (XMLinearTable[period % 768] << (FREQ_FRACBITS + 2)) >>
octave;
} else
{
if(!period)
period = 1;
return ((8363 * 1712L) << FREQ_FRACBITS) /
period;
}
} else if(
UseFinetuneAndTranspose()
...
{
return ((3546895L * 4) << FREQ_FRACBITS) /
period;
} else if(
GetType()
== MOD_TYPE_669)
...
return (period + c5speed - 8363) <<
FREQ_FRACBITS;
} else if(
GetType() &
(MOD_TYPE_MDL | MOD_TYPE_DTM))
{
LimitMax(period, Util::MaxValueOfType(period)
>> 8);
if (!c5speed)
c5speed = 8363;
return
Util::muldiv_unsigned(c5speed, (
1712L << 7) << FREQ_FRACBITS, (period << 8) + nPeriodFrac);
} else
{
LimitMax(period, Util::MaxValueOfType(period)
>> 8);
...
static_assert(FREQ_FRACBITS
<= 8, "Check this shift operator");
return
uint32(((uint64(period)
<< 8) + nPeriodFrac) >> (8 - FREQ_FRACBITS));
} else if(m_SongFlags[SONG_LINEARSLIDES])
{
if(!c5speed)
c5speed = 8363;
return
Util::muldiv_unsigned(c5speed, (
1712L << 8) << FREQ_FRACBITS, (period << 8) + nPeriodFrac);
} else
{
...
PLUGINDEX plug = 0;
if(m_PlayState.Chn[nChn].pModInstrument != nullptr) {
if(respectMutes == RespectMutes && m_PlayState.Chn[nChn].pModSample &&
   m_PlayState.Chn[nChn].pModSample->uFlags[CHN_MUTE]) {
plug = 0;
} else {

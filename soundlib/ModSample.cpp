...
if(((fromType & MOD_TYPE_XM) && (toType & (MOD_TYPE_IT | MOD_TYPE_MPT))) ||
   ((toType & MOD_TYPE_XM) && (fromType & (MOD_TYPE_IT | MOD_TYPE_MPT)))) {
...
nGlobalVol = 64;
uFlags.reset(
        CHN_PANNING | CHN_SUSTAINLOOP | CHN_LOOP | CHN_PINGPONGLOOP | CHN_PINGPONGSUSTAIN | CHN_ADLIB | SMP_MODIFIED |
        SMP_KEEPONDISK);
if(type == MOD_TYPE_XM) {
uFlags.set(CHN_PANNING);
}
...
PrecomputeLoop(T *target, const T *sampleData, SmpLength loopEnd, int numChannels, bool pingpong, bool ITPingPongMode)
        : target(target), sampleData(sampleData), loopEnd(loopEnd), numChannels(numChannels), pingpong(pingpong),
          ITPingPongMode(ITPingPongMode) {
if(loopEnd > 0) {
CopyLoop(true);
CopyLoop(false);
}
...
const int numSamples =
        2 * InterpolationMaxLookahead + (direction ? 1 : 0);  // Loop point is included in forward loop expansion
...
std::pair <int8, int8> ModSample::FrequencyToTranspose(uint32
freq)
{
if(!freq)
return {
};
const auto f2t = mpt::saturate_round<int32>(
        std::log(freq * (1.0 / 8363.0)) * (12.0 * 128.0 * (1.0 / mpt::numbers::ln2)));

...
MPT_BINARY_STRUCT(PSMFileHeader,
12)
// RIFF-style Chunk
struct PSMChunk {
...
MPT_BINARY_STRUCT(PSMSongHeader,
11)
...
MPT_BINARY_STRUCT(PSMSampleHeader,
96)
...
MPT_BINARY_STRUCT(PSMSinariaSampleHeader,
96)
...
char songName[10] = {};
PSMSubSong()
        : channelPanning(MAX_BASECHANNELS, 128), channelVolume(MAX_BASECHANNELS, 64),
          channelSurround(MAX_BASECHANNELS, false) {}
};
// Portamento effect conversion (depending on format version)
static uint8 ConvertPSMPorta(uint8
param,
bool sinariaFormat
)
{
...
subsong.restartPos = static_cast<ORDERINDEX>(restartChunk -
                                             firstOrderChunk);    // Close enough - we assume that order list is continuous (like in any real-world PSM)
...
if(!subChunk.ReadArray(mapTable)
   ...
   || mapTable[2] != 0x00 || mapTable[3] !=
                             0x00  // "at 0" (actually this appears to be the adding part - changing this to 0x01 0x00 offsets all samples by 1)
...
m_modFormat.formatName = sinariaFormat ? U_("Epic MegaGames MASI (New Version / Sinaria)") : U_(
...
note = NOTE_NOTECUT;
else if(note < 129) note = (note & 0x0F) + 12 * (note >> 4) + 13;
} else {
if(note < 85) note += 36;
}
m.note = note;
...
Patterns[startPattern].WriteEffect(
        EffectWriter(CMD_PANNING8, subsong.channelPanning[chn]).Row(0).Channel(chn).RetryNextRow());
}
}
...
Patterns[endPattern].WriteEffect(EffectWriter(CMD_POSITIONJUMP, mpt::saturate_cast<ModCommand::PARAM>(
        subsong.startOrder + subsong.restartPos)).Row(lastRow).RetryPreviousRow());
}
...
MPT_BINARY_STRUCT(PSM16FileHeader,
146)
...
MPT_BINARY_STRUCT(PSM16SampleHeader,
64)
...
MPT_BINARY_STRUCT(PSM16PatternHeader,
4)
...
m_modFormat.type = U_("psm");
m_modFormat.charset = mpt::Charset::CP437;
m_nChannels = Clamp(CHANNELINDEX(fileHeader.numChannelsPlay), CHANNELINDEX(fileHeader.numChannelsReal),
                    MAX_BASECHANNELS);
...
ChnSettings[i].Reset();
ChnSettings[i].nPan =
        ((15 - (file.ReadUint8() & 0x0F)) * 256 + 8) / 15;    // 15 seems to be left and 0 seems to be right...
...
ModCommand &m = *Patterns[pat].GetpModCommand(curRow, std::min(static_cast<CHANNELINDEX>(chnFlag & channelMask),
                                                               static_cast<CHANNELINDEX>(m_nChannels - 1)));
if(chnFlag & noteFlag) {
// note + instr present

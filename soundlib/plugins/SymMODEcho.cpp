...
OPENMPT_NAMESPACE_BEGIN
        IMixPlugin
*
SymMODEcho::Create(VSTPluginLib
&factory,
CSoundFile &sndFile, SNDMIXPLUGIN
*mixStruct)
{
return new (std::nothrow)
SymMODEcho(factory, sndFile, mixStruct
);
}
SymMODEcho::SymMODEcho(VSTPluginLib &factory, CSoundFile &sndFile, SNDMIXPLUGIN *mixStruct)
        : IMixPlugin(factory, sndFile, mixStruct), m_chunk(PluginChunk::Default()) {
...
PlugParamValue SymMODEcho::GetParameter(PlugParamIndex
index)
{
...
void SymMODEcho::SetParameter(PlugParamIndex
index,
...
{
m_chunk.param[index] =
mpt::saturate_round<uint8>(std::clamp(value, 0.0f, 1.0f)
* 127.0f);
RecalculateEchoParams();
}
}

...
IMixPlugin::IMixPlugin(VSTPluginLib &factory, CSoundFile &sndFile, SNDMIXPLUGIN *mixStruct)
        : m_Factory(factory), m_SndFile(sndFile), m_pMixStruct(mixStruct) {
...
void IMixPlugin::SetDryRatio(uint32
param)
{
param = std::min(param, uint32(127));
m_pMixStruct->
fDryRatio = 1.0f - (param / 127.0f);
}
...
void IMixPlugin::ProcessMixOps(float *MPT_RESTRICT pOutL, float *MPT_RESTRICT pOutR, float *MPT_RESTRICT leftPlugOutput,
                               float *MPT_RESTRICT rightPlugOutput, uint32 numFrames) {
...
dryRatio *= m_fGain;
float *MPT_RESTRICT
plugInputL = m_mixBuffer.GetInputBuffer(0);
float *MPT_RESTRICT
plugInputR = m_mixBuffer.GetInputBuffer(1);
...
float IMixPlugin::RenderSilence(uint32
numFrames)
{
...
PlugParamIndex numParams = std::min(GetNumParameters(),
                                    static_cast<int32>((std::numeric_limits<uint32>::max() - sizeof(uint32)) /
                                                       sizeof(IEEE754binary32LE)));
...
IMidiPlugin::IMidiPlugin(VSTPluginLib &factory, CSoundFile &sndFile, SNDMIXPLUGIN *mixStruct)
        : IMixPlugin(factory, sndFile, mixStruct), m_MidiCh{{}} {
...
MidiSend(MIDIEvents::Event(MIDIEvents::evControllerChange, midiCh, nParam,
                           static_cast<uint8>(nController)));    // param and controller are swapped (old broken implementation)
else
...
SendMidiPitchBend(GetMidiChannel(trackerChn),
                  EncodePitchBendParam(Clamp(pitchbend, MIDIEvents::pitchBendMin, MIDIEvents::pitchBendMax)));
}
...
void IMidiPlugin::SendMidiPitchBend(uint8
midiCh,
int32 newPitchBendPos
)
{
MPT_ASSERT(EncodePitchBendParam(MIDIEvents::pitchBendMin) <= newPitchBendPos &&
           newPitchBendPos <= EncodePitchBendParam(MIDIEvents::pitchBendMax));
m_MidiCh[midiCh].
midiPitchBendPos = newPitchBendPos;
...
auto newPitchBendPos = EncodePitchBendParam(
        Clamp(m_SndFile.m_PlayState.Chn[trackChannel].GetMIDIPitchBend(), MIDIEvents::pitchBendMin,
              MIDIEvents::pitchBendMax));
...
bool IMidiPlugin::IsNotePlaying(uint8
note,
CHANNELINDEX trackerChn
)
{
if(!
ModCommand::IsNote(note)
|| trackerChn >=
std::size(m_MidiCh[GetMidiChannel(trackerChn)]
.noteOnMap[note]))
return false;
...
NOTE_MIN;
return (m_MidiCh[
GetMidiChannel(trackerChn)
].noteOnMap[note][trackerChn] != 0);
}
void IMidiPlugin::ReceiveMidi(uint32
midiCode)
{
ResetSilence();
...
PLUGINDEX receiver;
if(m_pMixStruct !=
nullptr &&(receiver = m_pMixStruct->GetOutputPlugin()
) != PLUGINDEX_INVALID)
{
...
void SNDMIXPLUGIN::SetGain(uint8
gain)
{
Info.
gain = gain;
if(pMixPlugin != nullptr) pMixPlugin->
RecalculateGain();
}

...
OPENMPT_NAMESPACE_BEGIN
        IMixPlugin
*
LFOPlugin::Create(VSTPluginLib
&factory,
CSoundFile &sndFile, SNDMIXPLUGIN
*mixStruct)
{
return new (std::nothrow)
LFOPlugin(factory, sndFile, mixStruct
);
}
LFOPlugin::LFOPlugin(VSTPluginLib &factory, CSoundFile &sndFile, SNDMIXPLUGIN *mixStruct)
        : IMixPlugin(factory, sndFile, mixStruct), m_PRNG(mpt::make_prng<mpt::fast_prng>(mpt::global_prng())) {
...
if(plugin != nullptr) {
if(m_outputToCC) {
plugin->MidiSend(MIDIEvents::CC(static_cast<MIDIEvents::MidiCC>(m_outputParam & 0x7F),
                                static_cast<uint8>((m_outputParam >> 8) & 0x0F),
                                mpt::saturate_round<uint8>(value * 127.0f)));
} else {
...
PlugParamValue LFOPlugin::GetParameter(PlugParamIndex
index)
{
switch(index)
{
case kAmplitude: return
m_amplitude;
case kOffset: return
m_offset;
case kFrequency: return
m_frequency;
case kTempoSync: return m_tempoSync ? 1.0f : 0.0f;
case kWaveform: return
WaveformToParam(m_waveForm);
...
void LFOPlugin::SetParameter(PlugParamIndex
index,
...
break;
case kPolarity:
m_polarity = (value >= 0.5f);
break;
case kBypassed:
m_bypassed = (value >= 0.5f);
break;
case kLoopMode:
m_oneshot = (value >= 0.5f);
break;
case kCurrentPhase:
if(value == 0)
{
...
bool LFOPlugin::MidiSend(uint32
midiCode)
{
if(
IMixPlugin *plugin = GetOutputPlugin()
...
bool LFOPlugin::IsNotePlaying(uint8
note,

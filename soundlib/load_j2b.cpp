...
static constexpr
VibratoType j2bAutoVibratoTrans[] =
        {
...
MPT_BINARY_STRUCT(J2BFileHeader,
24)
...
MPT_BINARY_STRUCT(AMFFRiffChunk,
8)
...
MPT_BINARY_STRUCT(AMFFMainChunk,
73)
...
void ConvertEnvelope(uint8 flags, uint8 numPoints, uint8 sustainPoint, uint8 loopStart, uint8 loopEnd,
                     const EnvPoint (&points)[10], InstrumentEnvelope &mptEnv) const {
...
mptEnv.dwFlags.set(ENV_LOOP, (flags & AMFFEnvelope::envLoop) && mptEnv.nLoopStart <= mptEnv.nLoopEnd &&
                             mptEnv.nLoopStart <= mptEnv.size());
}
...
ConvertEnvelope(envFlags & 0x0F, envNumPoints & 0x0F, envSustainPoints & 0x0F, envLoopStarts & 0x0F, envLoopEnds & 0x0F,
                volEnv, mptIns.VolEnv);
ConvertEnvelope(envFlags >> 4, envNumPoints >> 4, envSustainPoints >> 4, envLoopStarts >> 4, envLoopEnds >> 4, panEnv,
                mptIns.PanEnv);
}
};
MPT_BINARY_STRUCT(AMFFEnvelope::EnvPoint,
3)
MPT_BINARY_STRUCT(AMFFEnvelope,
65)
...
MPT_BINARY_STRUCT(AMFFInstrumentHeader,
225)
...
MPT_BINARY_STRUCT(AMFFSampleHeader,
64)
...
MPT_BINARY_STRUCT(AMEnvelope::EnvPoint,
4)
MPT_BINARY_STRUCT(AMEnvelope,
48)
...
MPT_BINARY_STRUCT(AMInstrumentHeader,
326)
...
MPT_BINARY_STRUCT(AMSampleHeader,
60)
...
static constexpr EffectCommand
amEffTrans[] =
        {
...
ModCommand &m = *sndFile.Patterns[pat].GetpModCommand(row, std::min(static_cast<CHANNELINDEX>(flags & channelMask),
                                                                    static_cast<CHANNELINDEX>(channels - 1)));
if(flags & dataFlag) {
...
MPT_BINARY_STRUCT(AMFFRiffChunkFormat,
4)

...
uint8
CSoundFile::FrequencyToCutOff(double frequency) const {
...
//                                           <4.8737671609324025>
double cutoff = (std::log(frequency) - 4.8737671609324025) *
                (m_SongFlags[SONG_EXFILTERRANGE] ? (20.0 / mpt::numbers::ln2) : (24.0 / mpt::numbers::ln2));
Limit(cutoff, 0.0, 127.0);
return mpt::saturate_round<uint8>(cutoff);
}
uint32 CSoundFile::CutOffToFrequency(uint32
nCutOff,
int envModifier
) const
{
...
{
Fc =
110.0f * std::pow(2.0f, 0.25f + computedCutoff / (m_SongFlags[SONG_EXFILTERRANGE] ? 20.0f * 512.0f : 24.0f * 512.0f));
} else
{
...
Limit(freq,
120, 20000);
if(freq * 2 > (int)m_MixerSettings.gdwMixingFreq)
freq = m_MixerSettings.gdwMixingFreq / 2;
return static_cast
<uint32>(freq);
}

...
MPT_BINARY_STRUCT(DMFFileHeader,
66)
struct DMFChunk {
...
MPT_BINARY_STRUCT(DMFPatternHeader,
8)
// Sample header
...
MPT_BINARY_STRUCT(DMFSampleHeader,
16)
...
static uint8 DMFporta2MPT(uint8
val,
const uint8 internalTicks,
const bool hasFine
)
{
if(val == 0)
...
if(!isFine)
val = std::max(uint8(1), static_cast<uint8>((val + internalTicks - 2) / (internalTicks -
                                                                         1)));    // no slides on first tick! "+ internalTicks - 2" for rounding precision
...
static uint8 DMFtremor2MPT(uint8
val,
const uint8 internalTicks
)
{
...
static uint8 DMFdelay2MPT(uint8
val,
const uint8 internalTicks
)
{
...
static uint8 DMFvibrato2MPT(uint8
val,
const uint8 internalTicks
)
{
...
return;
const bool isTonePortaEffect = (effect == CMD_PORTAMENTOUP || effect == CMD_PORTAMENTODOWN ||
...
if(isTonePortaEffect &&
   (m->command == CMD_PORTAMENTOUP || m->command == CMD_PORTAMENTODOWN || m->command == CMD_TONEPORTAMENTO)) {
if(m->param < 0xE0) {
...
return;
          ...
          (m->command == CMD_VOLUMESLIDE || m->command == CMD_TONEPORTAVOL || m->command == CMD_VIBRATOVOL)) {
isSame = true;
}
if(isTonePortaEffect
...
return;
          ...
          && (m->volcmd == VOLCMD_FINEVOLUP || m->volcmd == VOLCMD_FINEVOLDOWN || m->volcmd == VOLCMD_VOLSLIDEUP ||
...
static PATTERNINDEX
ConvertDMFPattern(FileReader &file, const uint8 fileVersion, DMFPatternSettings &settings, CSoundFile &sndFile) {
// Pattern flags
enum PatternFlags {
// Global Track
...
const CHANNELINDEX numChannels = std::min(static_cast<CHANNELINDEX>(sndFile.GetNumChannels() - 1),
                                          static_cast<CHANNELINDEX>(patHead.numTracks));
...
const int tickspeed = (settings.realBPMmode) ? std::max(1, settings.tempoBPM * settings.beat * 2) : (
        (settings.tempoTicks + 1) * 30);
...
if(sndFile.Patterns[pat].WriteEffect(
        EffectWriter(CMD_S3MCMDEX, (0xA0 | (effect1 - 6))).Row(row - 1).Channel(chn).RetryPreviousRow())) {
...
if(sndFile.Patterns[pat].WriteEffect(
        EffectWriter(CMD_S3MCMDEX, (0x30 | (effect2 - 8))).Row(row - 1).Channel(chn).RetryPreviousRow())) {
...
if(sndFile.Patterns[pat].WriteEffect(
        EffectWriter(CMD_S3MCMDEX, (0x40 | (effect3 - 4))).Row(row - 1).Channel(chn).RetryPreviousRow())) {
...
if(tempoChange) {
tempoChange = false;
sndFile.Patterns[pat].WriteEffect(
        EffectWriter(CMD_TEMPO, static_cast<ModCommand::PARAM>(tempo)).Row(row).Channel(0).RetryNextRow());
sndFile.Patterns[pat].WriteEffect(
        EffectWriter(CMD_SPEED, static_cast<ModCommand::PARAM>(speed)).Row(row).RetryNextRow());
}
...
sndFile.Patterns[pat].WriteEffect(
        EffectWriter(CMD_S3MCMDEX, 0x60u | Clamp(param, uint8(1), uint8(15))).Row(row).AllowMultiple());
}
writeDelay = 0;
}  // End for all rows
...
Patterns[pat].WriteEffect(EffectWriter(CMD_POSITIONJUMP, static_cast<ModCommand::PARAM>(seqLoopStart)).Row(
        Patterns[pat].GetNumRows() - 1).RetryPreviousRow());
}
}
...
SampleIO(
        ...
        (sampleHeader.flags & DMFSampleHeader::smpCompMask) == DMFSampleHeader::smpComp1 ? SampleIO::DMF
                                                                                         : SampleIO::signedPCM)
...
m_SongFlags = SONG_LINEARSLIDES |
              SONG_ITCOMPATGXX;  // this will be converted to IT format by MPT. SONG_ITOLDEFFECTS is not set because of tremor and vibrato.
m_nDefaultSpeed = 6;

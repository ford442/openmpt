...
static_assert (std::size(effectTypes)
== MAX_EFFECTS);
...
static_assert(std::size(volumeEffectTypes)
== MAX_VOLCMDS);
...
break;  // this does different things in IT and ST3
case 0xD0:
if(param == 0xD0) { command = CMD_NONE; }
break;  // ditto
}
}
...
break;  // map to unused X5x
case 0x60:
command = CMD_XFINEPORTAUPDOWN;
break;  // map to unused X6x
case 0x80:
command = CMD_PANNING8;
param = (param & 0x0F) * 0x11;
break; // FT2 does actually not support E8x
case 0x90:
command = CMD_XFINEPORTAUPDOWN;
break;  // map to unused X9x
case 0xA0:
command = CMD_XFINEPORTAUPDOWN;
break;  // map to unused XAx
case 0xB0:
param = (param & 0x0F) | 0x60;
break;
case 0x70:
command = CMD_NONE;
break;  // No NNA / envelope control in MOD/XM format
}
}
...
if(volcmd == VOLCMD_NONE &&
   (((param & 0xF0) && ((param & 0x0F) == 0x0F)) || ((param & 0x0F) && ((param & 0xF0) == 0xF0)))) {
...
case 0x70:
command = CMD_NONE;
break;    // No NNA / envelope control in S3M format
case 0x90:
if(param == 0x91) {
command = CMD_PANNING8;
param = 0xA4;
...
if(newTypeIsIT_MPT) {
if(!oldTypeIsIT_MPT && command == CMD_NONE)
switch (volcmd) {
case VOLCMD_PANSLIDELEFT:
command = CMD_PANNINGSLIDE;
param = vol << 4;
...
if(volcmd == VOLCMD_OFFSET && !newSpecs.HasVolCommand(VOLCMD_OFFSET) &&
   (command == CMD_NONE || command == CMD_OFFSET || !newSpecs.HasCommand(command))) {
...
bool ModCommand::ConvertVolEffect(uint8 & effect, uint8 & param, bool
force)
{
switch(effect)
{
...
std::pair<EffectCommand, ModCommand::PARAM>
ModCommand::TwoRegularCommandsToMPT(uint8 & effect1, uint8 & param1, uint8 & effect2, uint8 & param2) {
for(uint8 n = 0; n < 4; n++) {

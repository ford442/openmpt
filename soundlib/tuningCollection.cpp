...
static void
ReadTuning(std::istream &iStrm, CTuningCollection &Tc, const std::size_t dummy, mpt::Charset defaultCharset) {
...
Tuning::SerializationResult
CTuningCollection::Deserialize(std::istream &iStrm, mpt::ustring &name, mpt::Charset defaultCharset) {
...
if(oldLoadingResult ==
   Tuning::SerializationResult::NoMagic) {    // An old version was not recognised - trying new version.
iStrm.clear();
...
ssb.ReadIterItem(iter, name, [charset](std::istream &iStrm, mpt::ustring &ustr, const std::size_t dummy) {
...
ssb.ReadIterItem(iter, *this, [charset](std::istream &iStrm, CTuningCollection &Tc, const std::size_t dummy) {
return ReadTuning(iStrm, Tc, dummy, charset);
});
}
if(ssb.GetStatus() & srlztn::SNT_FAILURE)
return Tuning::SerializationResult::Failure;
...
Tuning::SerializationResult
CTuningCollection::DeserializeOLD(std::istream &inStrm, mpt::ustring &uname, mpt::Charset defaultCharset) {

...
MPT_BINARY_STRUCT(PLMFileHeader,
96)
...
MPT_BINARY_STRUCT(PLMSampleHeader,
71)
...
MPT_BINARY_STRUCT(PLMPatternHeader,
32)
...
MPT_BINARY_STRUCT(PLMOrderItem,
4)
...
return fileHeader.headerSize - sizeof(PLMFileHeader) +
       4 * (fileHeader.numOrders + fileHeader.numPatterns + fileHeader.numSamples);
}
...
const CHANNELINDEX numChannels = std::min(patHeader.numChannels.get(),
                                          static_cast<uint8>(fileHeader.numChannels - ord.y));

...
return std::max({nameOffset + nameCount * 5u, exportOffset + exportCount * 8u, importOffset + importCount * 4u}) -
       sizeof(FileHeader);
}
...
// Read import table.
std::vector <int32>
ReadImportTable(FileReader &file, const FileHeader &fileHeader, const std::vector <std::string> &names) {
...
std::pair <FileReader, int32>
ReadExportTableEntry(FileReader &file, const FileHeader &fileHeader, const std::vector <int32> &classes,
                     const std::vector <std::string> &names, const char *filterType) {

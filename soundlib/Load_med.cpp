...
MPT_BINARY_STRUCT(MMD0FileHeader,
52)
...
MPT_BINARY_STRUCT(MMD0PatternHeader,
2)
...
MPT_BINARY_STRUCT(MMD1PatternHeader,
8)
...
MPT_BINARY_STRUCT(MMDPlaySeqCommand,
4)
...
MPT_BINARY_STRUCT(MMDBlockInfo,
36)
...
MPT_BINARY_STRUCT(MMDInstrHeader,
6)
...
MPT_BINARY_STRUCT(MMDPackedSampleHeader,
14)
...
MPT_BINARY_STRUCT(MMDInstrInfo,
40)
...
MPT_BINARY_STRUCT(MMDDump,
10)
static TEMPO MMDTempoToBPM(uint32
tempo,
bool is8Ch,
bool bpmMode, uint8
rowsPerBeat)
...
SEQUENCEINDEX numSongs = std::min(MAX_SEQUENCES, mpt::saturate_cast<SEQUENCEINDEX>(
        fileHeader.expDataOffset ? fileHeader.extraSongs + 1 : 1));
...
ins.VolEnv.assign({
                          ...
                          EnvelopeNode{static_cast<EnvelopeNode::tick_t>(instrExt.hold +
                                                                         (instrExt.decay ? 64u / instrExt.decay : 0u)),
                                       ENVELOPE_MIN},
                  });
if(instrExt.hold == 1)
...
case MMDTag::MMDTAG_TRK_NAME:
nameOffset = tag.data;
break;
case MMDTag::MMDTAG_TRK_NAMELEN:
nameLength = tag.data;
break;
}
}
...
Patterns[pat].WriteEffect(EffectWriter(CMD_POSITIONJUMP, mpt::saturate_cast<ModCommand::PARAM>(to)).Row(
...
switch (version) {
case 0:
madeWithTracker = m_nChannels > 4 ? MPT_ULITERAL("OctaMED v2.10 (MMD0)") : MPT_ULITERAL("MED v2 (MMD0)");
break;
case 1:
madeWithTracker = MPT_ULITERAL("OctaMED v4 (MMD1)");
break;
case 2:
madeWithTracker = MPT_ULITERAL("OctaMED v5 (MMD2)");
break;
case 3:
madeWithTracker = MPT_ULITERAL("OctaMED Soundstudio (MMD3)");
break;
}

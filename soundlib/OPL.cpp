...
void OPL::Mix(int32 * target, size_t
count,
uint32 volumeFactorQ16
)
{
if(!m_isActive)
return;
...
uint16 OPL::ChannelToRegister(uint8
oplCh)
{
if(oplCh < 9)
return
...
uint16 OPL::OperatorToRegister(uint8
oplCh)
{
static constexpr uint8
OPLChannelToOperator[] = {
...
NoteOff(c);
Volume(c, 0,
       false);  // Note that a volume of 0 is not complete silence; the release portion of the sound will still be heard at -48dB
if(unassign)

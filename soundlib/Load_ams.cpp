...
MPT_BINARY_STRUCT(AMSFileHeader,
11)
// AMS Sample Header
...
MPT_BINARY_STRUCT(AMSSampleHeader,
17)
...
return fileHeader.extraSize + 3u + fileHeader.numSamps * (1u + sizeof(AMSSampleHeader)) + fileHeader.numOrds * 2u +
       fileHeader.numPats * 4u;
}
...
MPT_BINARY_STRUCT(AMS2FileHeader,
7)
...
MPT_BINARY_STRUCT(AMS2Envelope,
5)
...
MPT_BINARY_STRUCT(AMS2Instrument,
5)
// AMS2 Sample Header
...
MPT_BINARY_STRUCT(AMS2SampleHeader,
20)
...
MPT_BINARY_STRUCT(AMS2Description,
11)
...
m_modFormat.type = U_("ams");
m_modFormat.madeWithTracker = MPT_UFORMAT("Velvet Studio {}.{}")(fileHeader.versionHigh.get(),
                                                                 mpt::ufmt::dec0<2>(fileHeader.versionLow.get()));
m_modFormat.charset = mpt::Charset::CP437;
uint16 headerFlags;
...
m_SongFlags = SONG_ITCOMPATGXX | SONG_ITOLDEFFECTS |
              ((headerFlags & AMS2FileHeader::linearSlides) ? SONG_LINEARSLIDES : SongFlags(0));
...
if(descriptionHeader.packedLen > sizeof(descriptionHeader) &&
   file.CanRead(descriptionHeader.packedLen - sizeof(descriptionHeader))) {

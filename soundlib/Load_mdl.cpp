...
MPT_BINARY_STRUCT(MDLFileHeader,
5)
// RIFF-style Chunk
struct MDLChunk {
...
MPT_BINARY_STRUCT(MDLInfoBlock,
91)
...
MPT_BINARY_STRUCT(MDLSampleHeader,
14)
struct MDLEnvelope {
uint8 envNum;
struct {
...
MPT_BINARY_STRUCT(MDLPatternHeader,
18)
...
static constexpr VibratoType
MDLVibratoType[] = {
VIB_SINE, VIB_RAMP_DOWN, VIB_SQUARE, VIB_SINE };
...
m_modFormat.madeWithTracker = U_("Digitrakker ") + (
        (fileHeader.version == 0x11) ? U_("3") // really could be 2.99b - close enough
                                     : (fileHeader.version == 0x10) ? U_("2.3")
                                                                    : (fileHeader.version == 0x00) ? U_(
                        "2.0 - 2.2b") // there was no 1.x release
                                                                                                   : U_(""));
...
mptSmp.nVibSweep = mpt::saturate_cast<decltype(mptSmp.nVibSweep)>(
        Util::muldivr_unsigned(mptSmp.nVibDepth, 256, mptSmp.nVibSweep));
else

...
static_assert(sizeof(chnState[0].nPatternLoopCount) <= sizeof(uint8),
              "Loop count cannot be used as byte input for hash generator");
...
RowVisitor::RowVisitor(const CSoundFile &sndFile, SEQUENCEINDEX sequence)
        : m_sndFile(sndFile), m_sequence(sequence) {
Initialize(true);
}
...
const auto end = (begin != m_visitedLoopStates.end()) ? m_visitedLoopStates.lower_bound({visitedPatterns[pat], numRows})
                                                      : m_visitedLoopStates.end();
...
if((m->command == CMD_S3MCMDEX && (m->param & 0xF0) == 0xB0) ||
   (m->command == CMD_MODCMDEX && (m->param & 0xF0) == 0x60)) {
...
ROWINDEX RowVisitor::VisitedRowsVectorSize(PATTERNINDEX pattern) const
noexcept
{
if(m_sndFile.Patterns.
IsValidPat(pattern)
)
return m_sndFile.Patterns[pattern].
GetNumRows();
else

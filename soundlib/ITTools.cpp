...
// Nodes and Loops
num = (uint8)
std::min(mptEnv.size(), uint32(25));
lpb = (uint8)
mptEnv.nLoopStart;
lpe = (uint8)
mptEnv.nLoopEnd;
slb = (uint8)
mptEnv.nSustainStart;
sle = (uint8)
mptEnv.nSustainEnd;
...
if(std::max(mptIns.VolEnv.nLoopStart, mptIns.VolEnv.nLoopEnd) >= mptIns.VolEnv.size())
mptIns.VolEnv.dwFlags.reset(ENV_LOOP);
if(std::max(mptIns.VolEnv.nSustainStart, mptIns.VolEnv.nSustainEnd) >= mptIns.VolEnv.size())
mptIns.VolEnv.dwFlags.reset(ENV_SUSTAIN);
}
...
dct = static_cast<uint8>((mptIns.nDCT < DuplicateCheckType::Plugin || !compatExport) ? mptIns.nDCT
...
for(int i = 0; i < 120; i++) {
keyboard[i * 2] = (mptIns.NoteMap[i] >= NOTE_MIN && mptIns.NoteMap[i] <= NOTE_MAX) ? (mptIns.NoteMap[i] - NOTE_MIN)
                                                                                   : static_cast<uint8>(i);
...
SampleIO ITSample::GetSampleFormat(uint16
cwtv) const
{
SampleIO sampleIO(
...
fatdate = static_cast<uint16>(mptHistory.loadDate.tm_mday | ((mptHistory.loadDate.tm_mon + 1) << 5) |
                              ((mptHistory.loadDate.tm_year - 80) << 9));
fattime = static_cast<uint16>((mptHistory.loadDate.tm_sec / 2) | (mptHistory.loadDate.tm_min << 5) |
                              (mptHistory.loadDate.tm_hour << 11));
} else {
fatdate = 0;
fattime = 0;
}
...
uint32 DecodeITEditTimer(uint16
cwtv,
uint32 editTime
)
{

...
static constexpr RATIOTYPE
s_DefaultFallbackRatio = 1.0f;
static constexpr NOTEINDEXTYPE
s_NoteMinDefault = -64;
static constexpr UNOTEINDEXTYPE
s_RatioTableSizeDefault = 128;
static constexpr USTEPINDEXTYPE
s_RatioTableFineSizeMaxDefault = 1000;
public:
...
//note that are valid.
MPT_FORCEINLINE NoteRange
GetNoteRange() const {
return NoteRange{m_NoteMin,
                 static_cast<NOTEINDEXTYPE>(m_NoteMin + static_cast<NOTEINDEXTYPE>(m_RatioTable.size()) - 1)};
}
...
STEPINDEXTYPE
GetStepDistance(const NOTEINDEXTYPE &noteFrom, const STEPINDEXTYPE &stepDistFrom, const NOTEINDEXTYPE &noteTo,
                const STEPINDEXTYPE &stepDistTo) const {
...
static std::unique_ptr <CTuning>
CreateGroupGeometric(const mpt::ustring &name, UNOTEINDEXTYPE groupsize, RATIOTYPE groupratio,
                     USTEPINDEXTYPE finestepcount) {
std::unique_ptr <CTuning> pT = std::unique_ptr<CTuning>(new CTuning());
pT->SetName(name);
...
static std::unique_ptr <CTuning>
CreateGroupGeometric(const mpt::ustring &name, const std::vector <RATIOTYPE> &ratios, RATIOTYPE groupratio,
                     USTEPINDEXTYPE finestepcount) {
std::unique_ptr <CTuning> pT = std::unique_ptr<CTuning>(new CTuning());
pT->SetName(name);
NoteRange range = NoteRange{s_NoteMinDefault,
                            static_cast<NOTEINDEXTYPE>(s_NoteMinDefault + s_RatioTableSizeDefault - 1)};
...
static std::unique_ptr <CTuning>
CreateGeometric(const mpt::ustring &name, UNOTEINDEXTYPE groupsize, RATIOTYPE groupratio,
                USTEPINDEXTYPE finestepcount) {
std::unique_ptr <CTuning> pT = std::unique_ptr<CTuning>(new CTuning());
pT->SetName(name);
...
bool CreateGroupGeometric(const std::vector <RATIOTYPE> &v, const RATIOTYPE &r, const NoteRange &range,
...
MPT_FORCEINLINE NOTEINDEXTYPE
GetRefNote(NOTEINDEXTYPE
note) const
{

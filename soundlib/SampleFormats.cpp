...
SAMPLEINDEX nSample = GetNextFreeSample(
        nInstr); // may also return samples which are only referenced by the current instrument
...
if(Instruments[nIns] != nullptr && nIns != nInstr) {
...
bool CSoundFile::ReadInstrumentFromSong(INSTRUMENTINDEX targetInstr, const CSoundFile &srcSong,
                                        INSTRUMENTINDEX sourceInstr) {
...
static bool IMAADPCMUnpack16(int16 * target, SmpLength
sampleLen,
FileReader file, uint16
blockAlign,
uint32 numChannels
)
{
...
static constexpr WAVFormatChunk::SampleFormats
SupportedFormats[] = {WAVFormatChunk::fmtPCM, WAVFormatChunk::fmtFloat, WAVFormatChunk::fmtIMA_ADPCM,
                      WAVFormatChunk::fmtMP3, WAVFormatChunk::fmtALaw, WAVFormatChunk::fmtULaw};
if(!wavFile.IsValid()
...
// MP3 in WAV
bool loadedMP3 =
        ReadMP3Sample(nSample, sampleChunk, false, true) || ReadMediaFoundationSample(nSample, sampleChunk, true);
if(!loadedMP3) {
return false;
}
} else if(!wavFile.IsExtensibleFormat() && wavFile.MayBeCoolEdit16_8() &&
          wavFile.GetSampleFormat() == WAVFormatChunk::fmtPCM && wavFile.GetBitsPerSample() == 32 &&
          wavFile.GetBlockAlign() == wavFile.GetNumChannels() * 4) {
...
} else if(!wavFile.IsExtensibleFormat() && wavFile.GetSampleFormat() == WAVFormatChunk::fmtPCM &&
          wavFile.GetBitsPerSample() == 24 && wavFile.GetBlockAlign() == wavFile.GetNumChannels() * 4) {
// Syntrillium Cool Edit hack to store IEEE 32bit floating point
...
sampleIO.ReadSample(sample, sampleChunk);
} else if(wavFile.GetSampleFormat() == WAVFormatChunk::fmtALaw ||
          wavFile.GetSampleFormat() == WAVFormatChunk::fmtULaw) {
// a-law / u-law
...
default:
case 0:
bitDepth = SampleIO::_8bit;
break;
case 1:
bitDepth = SampleIO::_16bit;
break;
case 2:
bitDepth = SampleIO::_24bit;
break;
case 3:
bitDepth = SampleIO::_32bit;
break;
case 7:
bitDepth = SampleIO::_64bit;
break;
}
sampleIO |= bitDepth;
...
WAVWriter file(ff);
file.WriteFormat(sample.GetSampleRate(GetType()), sample.GetElementarySampleSize() * 8, sample.GetNumChannels(),
...
MPT_BINARY_STRUCT(Wave64FileHeader,
40)
...
MPT_BINARY_STRUCT(Wave64ChunkHeader,
24)
struct Wave64Chunk {
...
MPT_BINARY_STRUCT(Wave64Chunk,
24)
static void
Wave64TagFromLISTINFO(mpt::ustring &dst, uint16 codePage, const ChunkReader::ChunkList<RIFFChunk> &infoChunk,
                      RIFFChunk::ChunkIdentifiers id) {
...
file.Rewind();
constexpr
mpt::UUID guidRIFF = "66666972-912E-11CF-A5D6-28DB04C10000"_uuid;
constexpr
mpt::UUID guidWAVE = "65766177-ACF3-11D3-8CD1-00C04F8EDB8A"_uuid;
constexpr
mpt::UUID guidLIST = "7473696C-912F-11CF-A5D6-28DB04C10000"_uuid;
constexpr
mpt::UUID guidFMT = "20746D66-ACF3-11D3-8CD1-00C04F8EDB8A"_uuid;
//constexpr mpt::UUID guidFACT       = "74636166-ACF3-11D3-8CD1-00C04F8EDB8A"_uuid;
constexpr
mpt::UUID guidDATA = "61746164-ACF3-11D3-8CD1-00C04F8EDB8A"_uuid;
...
constexpr
mpt::UUID guidCSET = "54455343-ACF3-11D3-8CD1-00C04F8EDB8A"_uuid;
...
MPT_BINARY_STRUCT(GF1PatchFileHeader,
129)
...
MPT_BINARY_STRUCT(GF1Instrument,
63)
...
MPT_BINARY_STRUCT(GF1SampleHeader,
96)
...
MPT_BINARY_STRUCT(GF1Layer,
47)
static double PatchFreqToNote(uint32
nFreq)
{
return
std::log(nFreq
/ 2044.0) * (12.0 * 1.44269504088896340736);    // 1.0/std::log(2.0)
}
static int32 PatchFreqToNoteInt(uint32
nFreq)
{
return
mpt::saturate_round<int32>(PatchFreqToNote(nFreq)
);
}
...
int32 nMaxNote = (sampleHeader.high_freq > 100) ? PatchFreqToNoteInt(sampleHeader.high_freq)
                                                : static_cast<uint8>(NOTE_MAX);
int32 nBaseNote = (sampleHeader.root_freq > 100) ? PatchFreqToNoteInt(sampleHeader.root_freq) : -1;
if(!sampleHeader.scale_factor && layerHeader.samples == 1) {
nMinNote = 0;
nMaxNote = NOTE_MAX;
}
// Fill Note Map
...
if(!file.ReadStruct(sampleHeader)
   ...
   || !file.Seek((sampleHeader.dataPointer[1] << 4) | (sampleHeader.dataPointer[2] << 12) |
                 (sampleHeader.dataPointer[0] << 20))) {
return false;
}
DestroySampleThreadsafe(nSample);
ModSample &sample = Samples[nSample];
...
MPT_BINARY_STRUCT(CAFFileHeader,
8)
...
MPT_BINARY_STRUCT(CAFChunkHeader,
12)
struct CAFChunk {
...
MPT_BINARY_STRUCT(CAFAudioFormat,
32)
static void
CAFSetTagFromInfoKey(mpt::ustring &dst, const std::map <std::string, std::string> &infoMap, const std::string &key) {
...
SampleIO::Endianness endianness = (audioFormat.mFormatFlags & CAFkCAFLinearPCMFormatFlagIsLittleEndian)
                                  ? SampleIO::littleEndian : SampleIO::bigEndian;
SampleIO::Encoding encoding = (audioFormat.mFormatFlags & CAFkCAFLinearPCMFormatFlagIsFloat) ? SampleIO::floatPCM
                                                                                             : SampleIO::signedPCM;
SampleIO::Bitdepth bitdepth = static_cast<SampleIO::Bitdepth>(
        (audioFormat.mBytesPerPacket / audioFormat.mChannelsPerFrame) * 8);
...
SmpLength length = mpt::saturate_cast<SmpLength>(
        (audioData.GetLength() / audioFormat.mBytesPerPacket) * audioFormat.mFramesPerPacket);
ModSample &mptSample = Samples[nSample];
DestroySampleThreadsafe(nSample);
mptSample.Initialize();
mptSample.nLength = length;
...
MPT_BINARY_STRUCT(AIFFCommonChunk,
18)
...
MPT_BINARY_STRUCT(AIFFSoundChunk,
8)
...
MPT_BINARY_STRUCT(AIFFInstrumentLoop,
6)
...
MPT_BINARY_STRUCT(AIFFInstrumentChunk,
20)
...
for(auto c : field) {
if(!mpt::is_in_range(c, 'a', 'z') && !mpt::is_in_range(c, 'A', 'Z') && !mpt::is_in_range(c, '0', '9') && c != '-' &&
   c != '_') {
return false;
}
}
...
return false;
SampleIO sampleIO(SampleIO::_8bit, channels == 1 ? SampleIO::mono : SampleIO::stereoInterleaved,
                  bigEndian ? SampleIO::bigEndian : SampleIO::littleEndian, SampleIO::signedPCM);
switch (encoding) {
case 1:
sampleIO |= SampleIO::_16bit;         // u-law
sampleIO |= SampleIO::uLaw;
break;
case 2:
break;                                // 8-bit linear PCM
case 3:
sampleIO |= SampleIO::_16bit;
break;  // 16-bit linear PCM
case 4:
sampleIO |= SampleIO::_24bit;
break;  // 24-bit linear PCM
case 5:
sampleIO |= SampleIO::_32bit;
break;  // 32-bit linear PCM
case 6:
sampleIO |= SampleIO::_32bit;         // 32-bit IEEE floating point
sampleIO |= SampleIO::floatPCM;
break;
case 7:
sampleIO |= SampleIO::_64bit;         // 64-bit IEEE floating point
sampleIO |= SampleIO::floatPCM;
break;
case 27:
sampleIO |= SampleIO::_16bit;        // a-law
sampleIO |= SampleIO::aLaw;
break;
default:
return false;
...
const SAMPLEINDEX nsamples = std::max(static_cast<SAMPLEINDEX>(instrumentHeader.nos),
                                      *std::max_element(std::begin(dummy.Keyboard), std::end(dummy.Keyboard)));
if(!nsamples)
return false;
...
// Old SchismTracker versions set nos=0
const SAMPLEINDEX nsamples = std::max(static_cast<SAMPLEINDEX>(instrumentHeader.nos),
                                      *std::max_element(std::begin(pIns->Keyboard), std::end(pIns->Keyboard)));
...
#ifndef MODPLUG_NO_FILESAVE
bool
CSoundFile::SaveITIInstrument(INSTRUMENTINDEX nInstr, std::ostream &f, const mpt::PathString &filename, bool compress,
                              bool allowExternal) const {
ITInstrument iti;
ModInstrument *pIns = Instruments[nInstr];
...
MPT_BINARY_STRUCT(IFFSampleHeader,
20)
...
if(!file.ReadStruct(fileHeader)
   ...
   || (memcmp(fileHeader.magic, "8SVX", 4) && memcmp(fileHeader.magic, "16SV", 4) &&
       memcmp(fileHeader.magic, "MAUD", 4))) {
return false;
}
ChunkReader chunkFile(file);

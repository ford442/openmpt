...
MPT_BINARY_STRUCT(DSymFileHeader,
17)
...
output.reserve(
        std::min(size, std::min(mpt::saturate_cast<uint32>(file.BytesLeft()), Util::MaxValueOfType(size) / 50u) * 50u));
static constexpr uint16
lzwBits = 13, MaxNodes = 1 << lzwBits;
static constexpr uint16
ResetDict = 256, EndOfStream = 257;
struct LZWEntry {
uint16 prev;
std::byte value;
};
...
panning[8] = {0x00, 0x00, 0x2B, 0x56, 0x80, 0xAA, 0xD4, 0xFF};
...
m_modFormat.type = U_(
        "dsym");  // RISC OS doesn't use file extensions but this is a common abbreviation used for this tracker

...
// [out] returns true on success.
bool
ReadFixedLineLength(const std::byte *data, const size_t length, const size_t lineLength, const size_t lineEndingLength);
...
void SetRaw(std::string message)
noexcept { assign(std::move(message)); }
};

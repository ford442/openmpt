...
void ReadNoteMap(std::istream &iStrm, std::map <NOTEINDEXTYPE, mpt::ustring> &m, const std::size_t dummy,
                 mpt::Charset charset);
...
enum : uint16 {
s_nDefaultWriteCount = (uint16_max >> 2)
};
};
}
...
static_assert(CTuning::s_RatioTableFineSizeMaxDefault<
static_cast
<USTEPINDEXTYPE>(FINESTEPCOUNT_MAX)
);
CTuning::CTuning()
        : m_TuningType(Type::GENERAL), m_FineStepCount(0) {
m_RatioTable.clear();
...
bool CTuning::CreateGroupGeometric(const std::vector <RATIOTYPE> &v, const RATIOTYPE &r, const NoteRange &range,
                                   const NOTEINDEXTYPE &ratiostartpos) {
if(range.first > range.last || v.size() == 0) {
return false;
}
if(ratiostartpos < range.first || range.last < ratiostartpos ||
   static_cast<UNOTEINDEXTYPE>(range.last - ratiostartpos) < static_cast<UNOTEINDEXTYPE>(v.size() - 1)) {
return false;
}
...
const NOTEINDEXTYPE note = static_cast<NOTEINDEXTYPE>(baseNote +
                                                      mpt::wrapping_divide(baseFineSteps, (fineStepCount + 1)));
...
if(GetType() == Type::GROUPGEOMETRIC) { // update other groups
for(NOTEINDEXTYPE n = m_NoteMin; n < m_NoteMin + static_cast<NOTEINDEXTYPE>(m_RatioTable.size()); ++n) {
if(n == s) {
// nothing
} else if(std::abs(n - s) % m_GroupSize == 0) {
m_RatioTable[n - m_NoteMin] =
        std::pow(m_GroupRatio, static_cast<RATIOTYPE>(n - s) / static_cast<RATIOTYPE>(m_GroupSize)) *
        m_RatioTable[s - m_NoteMin];
}
}
UpdateFineStepTable();
}
...
const RATIOTYPE rFineStep = std::pow(GetRatio(refnote + 1) / GetRatio(refnote),
                                     static_cast<RATIOTYPE>(1) / (m_FineStepCount + 1));
...
ssb.ReadItem(m_TuningName, "0", [charset](std::istream &iStrm, mpt::ustring &ustr, const std::size_t dummy) {
return ReadStr(iStrm, ustr, dummy, charset);
});
...
ssb.ReadItem(m_NoteNameMap, "3", [charset](std::istream &iStrm, std::map <NOTEINDEXTYPE, mpt::ustring> &m,
                                           const std::size_t dummy) { return ReadNoteMap(iStrm, m, dummy, charset); });
...
if(GetType() == Type::GEOMETRIC) {
if(!CreateGeometric(GetGroupSize(), GetGroupRatio(),
                    NoteRange{m_NoteMin, static_cast<NOTEINDEXTYPE>(m_NoteMin + ratiotableSize - 1)})) {
return SerializationResult::Failure;
}
} else {
if(!CreateGroupGeometric(m_RatioTable, GetGroupRatio(),
                         NoteRange{m_NoteMin, static_cast<NOTEINDEXTYPE>(m_NoteMin + ratiotableSize - 1)}, m_NoteMin)) {
return SerializationResult::Failure;
}
}
} else {
...
static bool VectorFromBinaryStream(std::istream &inStrm, std::vector <Tdst> &v,
...
if(tt == Type::GEOMETRIC ||
   tt == Type::GROUPGEOMETRIC) {    //For Groupgeometric this data is the number of ratios in ratiotable.
...
ssb.FinishWrite();
return ((ssb.GetStatus() & srlztn::SNT_FAILURE) != 0) ? Tuning::SerializationResult::Failure
...
if(str.find_first_of('\0') != std::string::npos) { // trim \0 at the end

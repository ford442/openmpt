...
MPT_BINARY_STRUCT(MMCMPFileHeader,
24)
...
MPT_BINARY_STRUCT(MMCMPSubBlock,
8)
...
unpackedData.reserve(
        std::min(unpackedSize, std::min(mpt::saturate_cast<uint32>(file.GetLength()), uint32_max / 20u) * 20u));
// 8-bit deltas
uint8 ptable[256] = {0};

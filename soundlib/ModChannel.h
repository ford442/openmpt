...
resetSetPosFull =
resetSetPosBasic | resetSetPosAdvanced | resetChannelSettings,  // Reset all runtime channel attributes
...
noexcept { return !increment.IsZero(); }
uint32 GetVSTVolume() const
noexcept { return (pModInstrument) ? pModInstrument->nGlobalVol * 4 : nVolume; }
ModCommand::NOTE GetPluginNote(bool realNoteMapping) const;
bool HasMIDIOutput() const
noexcept { return pModInstrument != nullptr && pModInstrument->HasValidMIDIChannel(); }
bool HasCustomTuning() const
noexcept { return pModInstrument != nullptr && pModInstrument->pTuning != nullptr; }
bool InSustainLoop() const
noexcept { return (dwFlags & (CHN_LOOP | CHN_KEYOFF)) == CHN_LOOP && pModSample->uFlags[CHN_SUSTAINLOOP]; }
...
int32 GetMIDIPitchBend() const
noexcept { return (static_cast<int32>(microTuning) + 0x8000) / 4; }
};

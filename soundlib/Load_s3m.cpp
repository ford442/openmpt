...
case '@':
m.command = (m.param ? CMD_DUMMY : CMD_NONE);
break;
case 'A':
m.command = CMD_SPEED;
break;
...
if(!fromIT) m.param = (m.param >> 4) * 10 + (m.param & 0x0F);
break;
case 'D':
...
break;
case '\\':
m.command = fromIT ? CMD_SMOOTHMIDI : CMD_MIDI;
break;
case ']':
m.command = fromIT ? CMD_DELAYCUT : CMD_NONE;
break;
case '[':
m.command = fromIT ? CMD_XPARAM : CMD_NONE;
break;
case '^':
m.command = fromIT ? CMD_FINETUNE : CMD_NONE;
break;
case '_':
m.command = fromIT ? CMD_FINETUNE_SMOOTH : CMD_NONE;
break;
default:
m.command = CMD_NONE;
}
...
#ifndef MODPLUG_NO_FILESAVE
void CSoundFile::S3MSaveConvert(uint8 & command, uint8 & param, bool
toIT,
...
if(toIT && !(
GetType() &
(MOD_TYPE_IT | MOD_TYPE_MPT | MOD_TYPE_XM | MOD_TYPE_MOD)))
{
if (param == 0xA4) { command = 'S';
param = 0x91;
}
...
const int32 schismDateVersion =
        SchismTrackerEpoch + ((fileHeader.cwtv == 0x4FFF) ? fileHeader.reserved2 : (fileHeader.cwtv - 0x4050));
switch (fileHeader.cwtv & S3MFileHeader::trackerMask) {
case S3MFileHeader::trkScreamTracker:
if(fileHeader.cwtv == S3MFileHeader::trkST3_20 && fileHeader.special == 0 && (fileHeader.ordNum & 0x0F) == 0 &&
   fileHeader.ultraClicks == 0 && (fileHeader.flags & ~0x50) == 0) {
...
} else if(fileHeader.cwtv == S3MFileHeader::trkST3_20 && fileHeader.special == 0 && fileHeader.ultraClicks == 0 &&
          fileHeader.flags == 0 && fileHeader.usePanningTable == 0) {
...
if(formatTrackerStr) {
madeWithTracker = MPT_UFORMAT("{} {}.{}")(madeWithTracker, (fileHeader.cwtv & 0xF00) >> 8,
...
if((fileHeader.cwtv & S3MFileHeader::trackerMask) != S3MFileHeader::trkImpulseTracker ||
   fileHeader.cwtv >= S3MFileHeader::trkIT2_14) {
keepMidiMacros = true;
}
}
m_MidiCfg.Reset();
...
const uint32 sampleOffset =
        (sampleHeader.dataPointer[1] << 4) | (sampleHeader.dataPointer[2] << 12) | (sampleHeader.dataPointer[0] << 20);
if((loadFlags & loadSampleData) && sampleHeader.length != 0 && file.Seek(sampleOffset)) {
sampleHeader.GetSampleFormat((fileHeader.formatVersion == S3MFileHeader::oldVersion)).ReadSample(Samples[smp + 1],
                                                                                                 file);
anySamples = true;
}
...
const PATTERNINDEX readPatterns = std::min(static_cast<PATTERNINDEX>(fileHeader.patNum),
                                           static_cast<PATTERNINDEX>(uint8_max));
...
if(pixPlayPanning && zxxCountLeft + zxxCountRight >= m_nChannels &&
   (-zxxCountLeft + zxxCountRight) < static_cast<int>(m_nChannels)) {
...
enum class S3MChannelType : uint8 {
kUnused = 0, kPCM = 1, kAdlib = 2
};
...
AddToLog(LogWarning, MPT_UFORMAT(
        "Pattern channel {} constains both samples and OPL instruments, which is not supported by Scream Tracker 3.")(
        chn + 1));
}
// First, the "left" channels (0...7) are evaluated, then the "right" channels (8...15).
// we should just write in the LLL...RRR manner.
uint8 ch = sampleCh %
           16u; // If there are neither PCM nor AdLib instruments on this channel, just fall back a regular sample-based channel for maximum compatibility.
...
if(sampleCh > 16) {
AddToLog(LogWarning,
         MPT_UFORMAT("This module has more than 16 ({}) sample channels, which is not supported by Scream Tracker 3.")(
                 sampleCh));
}
if(adlibCh > 9) {
AddToLog(LogWarning,
         MPT_UFORMAT("This module has more than 9 ({}) OPL channels, which is not supported by Scream Tracker 3.")(
                 adlibCh));
}

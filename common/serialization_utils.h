...
template <>
inline void WriteItem<std::string>(std::ostream &oStrm, const std::string &str)
{
	WriteItemString(oStrm, str);
}
...
public:
	...
	ID(const char *str, std::size_t len)
		: m_ID(str, len)
	{
	}
	template <typename T>
...
public:
	...
	template <class T>
	bool ReadItem(T &obj, const ID &id)
	{
		return ReadItem(obj, id, srlztn::ReadItem<T>);
	}
	...
	template <class T>
	bool ReadIterItem(const ReadIterator &iter, T &obj)
	{
		return ReadIterItem(iter, obj, srlztn::ReadItem<T>);
	}
	template <class T, class FuncObj>
...
public:
	...
	template <class T>
	void WriteItem(const T &obj, const ID &id)
	{
		WriteItem(obj, id, &srlztn::WriteItem<T>);
	}
...
#ifdef SSB_LOGGING
	void LogWriteEntry(const ID &id, const std::size_t nEntryNum, const std::size_t nBytecount, const std::streamoff rposStart);
#endif

	// Writes mapping item to mapstream.
	void WriteMapItem(const ID &id, const std::streamoff &rposDataStart, const std::size_t &nDatasize, const std::string &pszDesc);

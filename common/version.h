...
noexcept;
public:
MPT_CONSTEXPRINLINE Version()
noexcept
        : m_Version(0)
        {}
explicit MPT_CONSTEXPRINLINE Version(uint32 version)
noexcept
        : m_Version(version)
        {}
explicit MPT_CONSTEXPRINLINE Version(uint8 v1, uint8 v2, uint8 v3, uint8 v4)
noexcept
        : m_Version((static_cast<uint32>(v1) << 24) | (static_cast<uint32>(v2) << 16) | (static_cast<uint32>(v3) << 8) |
                    (static_cast<uint32>(v4) << 0))
        {}
public:
...
public:
explicit MPT_CONSTEXPRINLINE operator bool() const
noexcept
{
return m_Version != 0;
}
...
Version WithoutTestNumber() const
noexcept;
Version WithoutPatchOrTestNumbers() const
noexcept;
public:
...
private:
static MPT_CONSTEXPRINLINE uint8
NibbleFromChar(char x) {
return
...
public:
static MPT_CONSTEXPRINLINE Version
Parse(const char *str, std::size_t len) {
// 0123456789
// 1.23.45.67
uint8 v[4] = {0, 0, 0, 0};
std::size_t field = 0;
...
MPT_CONSTEXPRINLINE bool operator==(const Version &a, const Version &b)
noexcept
{
return a.
GetRawVersion()
...
MPT_CONSTEXPRINLINE Version
operator "" _LiteralVersionImpl(const char *str, std::size_t len) {
...
public:
mpt::ustring
GetUrlWithRevision() const; // i.e. "https://source.openmpt.org/svn/openmpt/trunk/OpenMPT@1234" or empty string
...
constexpr bool HasRevision() const
noexcept
{
return revision != 0;
}
constexpr bool IsEqualTo(VersionWithRevision other) const
noexcept
{
return version == other.version && revision == other.revision;
}
constexpr bool IsEquivalentTo(VersionWithRevision other) const
noexcept
{
...
constexpr bool IsNewerThan(VersionWithRevision other) const
noexcept
{
...
constexpr bool IsOlderThan(VersionWithRevision other) const
noexcept
{
if(version < other.version) {
return true;

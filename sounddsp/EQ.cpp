...
#ifndef NO_EQ
static constexpr
float EQ_BANDWIDTH = 2.0f;
static constexpr std::array<uint32, 33>
gEqLinearToDB =
        {
...
static constexpr std::array<EQBANDSETTINGS, MAX_EQ_BANDS>
gEQDefaults =
        {{
                 // Default: Flat EQ
                 {0, 0, 0, 0, 0, 1, 120},
...
static void EQFilter(Tbuf &buf, const std::array <EQBANDSETTINGS, MAX_EQ_BANDS> &bands,
                     std::array <std::array<EQBANDSTATE, MAX_EQ_BANDS>, MAX_EQ_CHANNELS> &states) {
...
float x = sample;
float y =
        band.a1 * bandState.x1 + band.a2 * bandState.x2 + band.a0 * x + band.b1 * bandState.y1 + band.b2 * bandState.y2;
...
void CEQ::ProcessTemplate(TMixSample *frontBuffer, TMixSample *rearBuffer, std::size_t countFrames,
...
void
CEQ::Process(MixSampleInt *frontBuffer, MixSampleInt *rearBuffer, std::size_t countFrames, std::size_t numChannels) {
ProcessTemplate<MixSampleInt>(frontBuffer, rearBuffer, countFrames, numChannels);
}
void CEQ::Process(MixSampleFloat *frontBuffer, MixSampleFloat *rearBuffer, std::size_t countFrames,
                  std::size_t numChannels) {
...
m_Bands[i].Gain = static_cast<float>(gEqLinearToDB[std::clamp(pGains[i], static_cast<uint32>(0),
                                                              static_cast<uint32>(std::size(gEqLinearToDB) - 1))]) /
                  64.0f;

...
return mpt::saturate_round<int32>(
        (1.0 - (std::sqrt((g + g) * (1.0 - cosw) - g * g * (1.0 - cosw * cosw)) + g * cosw)) * scale_over_1mg);
}
...
static constexpr std::pair<SNDMIX_REVERB_PROPERTIES, const char *>
ReverbPresets[NUM_REVERBTYPES] =
{
...
MPT_FORCEINLINE int32
ftol(float f) { return static_cast<int32>(f); }
...
if(!gnReverbSend) { // and we did not clear the buffer yet, do it now because we will get new data
...
if((!gnReverbSend) && (!gnReverbSamples)) { // no data is sent to reverb and reverb decayed completely
return;
}
if(!gnReverbSend) { // no input data in MixReverbBuffer, so the buffer got not cleared in GetReverbSendBuffer(), do it now for decay
StereoFill(MixReverbBuffer, nSamples, gnRvbROfsVol, gnRvbLOfsVol);
}
uint32 nIn, nOut;
...
// Process Dry/Wet Mix
int32 lMaxRvbGain = (g_RefDelay.lMasterGain > g_LateReverb.lMasterGain) ? g_RefDelay.lMasterGain
                                                                        : g_LateReverb.lMasterGain;
...
void CReverb::ReverbDryMix(int32 * MPT_RESTRICT
pDry,
int32 *MPT_RESTRICT
pWet,
int lDryVol, uint32
...
pDry[i*2] += (pWet[i*2]>>4) *
lDryVol;
pDry[i*2+1] += (pWet[i*2+1]>>4) *
lDryVol;
}
}
uint32 CReverb::ReverbProcessPreFiltering2x(int32 * MPT_RESTRICT
pWet,
uint32 nSamples
)
{
...
uint32 CReverb::ReverbProcessPreFiltering1x(int32 * MPT_RESTRICT
pWet,
uint32 nSamples
)
{
...
void CReverb::ReverbDCRemoval(int32 * MPT_RESTRICT
pBuffer,
uint32 nSamples
)
{
...
// Save some typing
static MPT_FORCEINLINE int32
Clamp16(int32
x) {
return
Clamp(x, std::numeric_limits<int16>::min(), std::numeric_limits<int16>::max()
); }
...
void
CReverb::ProcessReflections(SWRvbRefDelay *MPT_RESTRICT pPreDelay, LR16 *MPT_RESTRICT pRefOut, int32 *MPT_RESTRICT pOut,
                            uint32 nSamples) {
#ifdef ENABLE_SSE2
if(CPU::HasFeatureSet(CPU::feature::sse2))
{
...
void
CReverb::ProcessLateReverb(SWLateReverb *MPT_RESTRICT pReverb, LR16 *MPT_RESTRICT pRefOut, int32 *MPT_RESTRICT pMixOut,
                           uint32 nSamples) {
...
pRefOut++;
int32 delay2LL = pReverb->Delay2[DELAY_OFFSET(RVBDLY2L_LEN)].c.l, delay2LR = pReverb->Delay2[DELAY_OFFSET(
        RVBDLY2L_LEN)].c.r;
int32 delay2RL = pReverb->Delay2[DELAY_OFFSET(RVBDLY2R_LEN)].c.l, delay2RR = pReverb->Delay2[DELAY_OFFSET(
        RVBDLY2R_LEN)].c.r;
...
// Apply decay gain
int32
histDecayL = Clamp16((int32)
pReverb->nDecayDC[0].c.l * pReverb->LPHistory[0].c.l / (1 << 15));
int32
histDecayR = Clamp16((int32)
pReverb->nDecayDC[1].c.r * pReverb->LPHistory[1].c.r / (1 << 15));
...
// Input to second diffuser
int32 delay1LL = pReverb->Delay1[DELAY_OFFSET(RVBDLY1L_LEN)].c.l, delay1LR = pReverb->Delay1[DELAY_OFFSET(
        RVBDLY1L_LEN)].c.r;
int32 delay1RL = pReverb->Delay1[DELAY_OFFSET(RVBDLY1R_LEN)].c.l, delay1RR = pReverb->Delay1[DELAY_OFFSET(
        RVBDLY1R_LEN)].c.r;
int32 delay1GainsL = Clamp16(
        (delay1LL * pReverb->Dif2InGains[0].c.l + delay1LR * pReverb->Dif2InGains[0].c.r) / (1 << 15));
int32 delay1GainsR = Clamp16(
        (delay1RL * pReverb->Dif2InGains[1].c.l + delay1RR * pReverb->Dif2InGains[1].c.r) / (1 << 15));
...
delayPos = (delayPos + 1) & RVBDLY_MASK;
// Accumulate with reverb output
pMixOut[0] += Clamp16(histDelay1LL + delay2outL) * pReverb->RvbOutGains[0].c.l +
              Clamp16(histDelay1LR + delay2outR) * pReverb->RvbOutGains[0].c.r;
pMixOut[1] += Clamp16(histDelay1RL + Clamp16(diff2outCoeffsL)) * pReverb->RvbOutGains[1].c.l +
              Clamp16(histDelay1RR + Clamp16(diff2outCoeffsR)) * pReverb->RvbOutGains[1].c.r;
pMixOut += 2;
}
pReverb->nDelayPos = delayPos;
#undef DELAY_OFFSET

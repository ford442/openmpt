...
PPDecompressor::PPState::PPState(uint32_t
mode) :
_cachedMode(mode)
        {
                // nothing needed
...
bool PPDecompressor::detectHeader(uint32_t
hdr) noexcept {
return (hdr==FourCC("PP11") || hdr==FourCC("PP20"));
}
bool PPDecompressor::detectHeaderXPK(uint32_t
hdr) noexcept
{
...
std::unique_ptr <XPKDecompressor> PPDecompressor::create(uint32_t
hdr,
...
{
return
std::make_unique<PPDecompressor>(hdr, recursionLevel, packedData, state, verify
);
}
...
if(!_rawSize || _startShift >= 0x20 ||
   _rawSize > getMaxRawSize())
throw InvalidFormatError();
}
PPDecompressor::PPDecompressor(uint32_t
hdr,
uint32_t recursionLevel,
const Buffer &packedData, std::unique_ptr<XPKDecompressor::State>
&state,

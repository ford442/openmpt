...
bool FASTDecompressor::detectHeaderXPK(uint32_t
hdr) noexcept {
return hdr==FourCC("FAST");
}
std::unique_ptr <XPKDecompressor> FASTDecompressor::create(uint32_t
hdr,
...
{
return
std::make_unique<FASTDecompressor>(hdr, recursionLevel, packedData, state, verify
);
}
FASTDecompressor::FASTDecompressor(uint32_t
hdr,
uint32_t recursionLevel,
const Buffer &packedData, std::unique_ptr<XPKDecompressor::State>
&state,

...
bool SDHCDecompressor::detectHeaderXPK(uint32_t
hdr) noexcept {
return hdr==FourCC("SDHC");
}
std::unique_ptr <XPKDecompressor> SDHCDecompressor::create(uint32_t
hdr,
...
{
return
std::make_unique<SDHCDecompressor>(hdr, recursionLevel, packedData, state, verify
);
}
SDHCDecompressor::SDHCDecompressor(uint32_t
hdr,
uint32_t recursionLevel,
const Buffer &packedData, std::unique_ptr<XPKDecompressor::State>
&state,

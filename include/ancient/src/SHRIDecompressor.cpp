...
bool SHRIDecompressor::detectHeaderXPK(uint32_t
hdr) noexcept
{
return hdr==FourCC("SHRI");
}
std::unique_ptr <XPKDecompressor> SHRIDecompressor::create(uint32_t
hdr,
...
{
return
std::make_unique<SHRIDecompressor>(hdr, recursionLevel, packedData, state, verify
);
}
SHRIDecompressor::SHRIDecompressor(uint32_t
hdr,
uint32_t recursionLevel,
const Buffer &packedData, std::unique_ptr<XPKDecompressor::State>
&state,
...
const std::string &SHRIDecompressor::getSubName() const
noexcept
{

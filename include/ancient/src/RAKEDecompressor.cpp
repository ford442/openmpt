...
bool RAKEDecompressor::detectHeaderXPK(uint32_t
hdr) noexcept {
return (hdr==FourCC("FRHT") || hdr==FourCC("RAKE"));
}
std::unique_ptr <XPKDecompressor> RAKEDecompressor::create(uint32_t
hdr,
...
{
return
std::make_unique<RAKEDecompressor>(hdr, recursionLevel, packedData, state, verify
);
}
RAKEDecompressor::RAKEDecompressor(uint32_t
hdr,
uint32_t recursionLevel,
const Buffer &packedData, std::unique_ptr<XPKDecompressor::State>
&state,

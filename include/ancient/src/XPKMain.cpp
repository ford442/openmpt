...
bool XPKMain::detectHeader(uint32_t
hdr) noexcept {
...
static std::vector<std::pair<bool(*)(uint32_t), std::unique_ptr<XPKDecompressor>(*)(uint32_t, uint32_t, const Buffer &,
        std::unique_ptr<XPKDecompressor::State> & , bool
)>>
XPKDecompressors = {
...
const std::string &XPKMain::getName() const
noexcept
{
...
size_t XPKMain::getPackedSize() const
noexcept
{
return _packedSize+8;
}
size_t XPKMain::getRawSize() const
noexcept
{
return
_rawSize;
...
std::unique_ptr<XPKDecompressor> XPKMain::createDecompressor(uint32_t
type,
uint32_t recursionLevel,
const Buffer &buffer, std::unique_ptr<XPKDecompressor::State>
&state,
bool verify
)
{
...
second(type, recursionLevel, buffer, state, verify
);
}
throw
InvalidFormatError();

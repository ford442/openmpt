...
bool LZW4Decompressor::detectHeaderXPK(uint32_t
hdr) noexcept {
return hdr==FourCC("LZW4");
}
std::unique_ptr <XPKDecompressor> LZW4Decompressor::create(uint32_t
hdr,
...
{
return
std::make_unique<LZW4Decompressor>(hdr, recursionLevel, packedData, state, verify
);
}
LZW4Decompressor::LZW4Decompressor(uint32_t
hdr,
uint32_t recursionLevel,
const Buffer &packedData, std::unique_ptr<XPKDecompressor::State>
&state,

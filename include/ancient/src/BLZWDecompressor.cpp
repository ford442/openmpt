...
bool BLZWDecompressor::detectHeaderXPK(uint32_t
hdr) {
return hdr==FourCC("BLZW");
}
std::unique_ptr <XPKDecompressor> BLZWDecompressor::create(uint32_t
hdr,
...
{
return
std::make_unique<BLZWDecompressor>(hdr, recursionLevel, packedData, state, verify
);
}
BLZWDecompressor::BLZWDecompressor(uint32_t
hdr,
uint32_t recursionLevel,
const Buffer &packedData, std::unique_ptr<XPKDecompressor::State>
&state,

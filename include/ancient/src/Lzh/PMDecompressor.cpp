...
size_t PMDecompressor::getRawSize() const
noexcept {
// N/A
return 0;
}
...
const std::string &PMDecompressor::getName() const
noexcept
{
...
outputStream.writeByte(processOutput(outputStream.getOffset(),
                                     decodeMTF(readBits(symbolBits[code]) + symbolAdditions[code], dataMTFMap)));
} else {
code -= 8;
uint32_t count;
if(code < 15) {

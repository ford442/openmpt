...
bool LZBSDecompressor::detectHeaderXPK(uint32_t
hdr) noexcept {
return hdr==FourCC("LZBS");
}
std::unique_ptr <XPKDecompressor> LZBSDecompressor::create(uint32_t
hdr,
...
{
return
std::make_unique<LZBSDecompressor>(hdr, recursionLevel, packedData, state, verify
);
}
LZBSDecompressor::LZBSDecompressor(uint32_t
hdr,
uint32_t recursionLevel,
const Buffer &packedData, std::unique_ptr<XPKDecompressor::State>
&state,

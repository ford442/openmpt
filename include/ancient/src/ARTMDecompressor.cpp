...
bool ARTMDecompressor::detectHeaderXPK(uint32_t
hdr) noexcept {
return hdr==FourCC("ARTM");
}
std::unique_ptr <XPKDecompressor> ARTMDecompressor::create(uint32_t
hdr,
...
{
return
std::make_unique<ARTMDecompressor>(hdr, recursionLevel, packedData, state, verify
);
}
ARTMDecompressor::ARTMDecompressor(uint32_t
hdr,
uint32_t recursionLevel,
const Buffer &packedData, std::unique_ptr<XPKDecompressor::State>
&state,
...
const std::string &ARTMDecompressor::getSubName() const
noexcept
{

...
bool SQSHDecompressor::detectHeaderXPK(uint32_t
hdr) noexcept {
return hdr==FourCC("SQSH");
}
std::unique_ptr <XPKDecompressor> SQSHDecompressor::create(uint32_t
hdr,
...
{
return
std::make_unique<SQSHDecompressor>(hdr, recursionLevel, packedData, state, verify
);
}
SQSHDecompressor::SQSHDecompressor(uint32_t
hdr,
uint32_t recursionLevel,
const Buffer &packedData, std::unique_ptr<XPKDecompressor::State>
&state,

...
bool ACCADecompressor::detectHeaderXPK(uint32_t
hdr) noexcept {
return hdr==FourCC("ACCA");
}
std::unique_ptr <XPKDecompressor> ACCADecompressor::create(uint32_t
hdr,
...
{
return
std::make_unique<ACCADecompressor>(hdr, recursionLevel, packedData, state, verify
);
}
ACCADecompressor::ACCADecompressor(uint32_t
hdr,
uint32_t recursionLevel,
const Buffer &packedData, std::unique_ptr<XPKDecompressor::State>
&state,

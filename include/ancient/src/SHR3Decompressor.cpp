...
bool SHR3Decompressor::detectHeaderXPK(uint32_t
hdr) noexcept
{
return hdr==FourCC("SHR3");
}
std::unique_ptr <XPKDecompressor> SHR3Decompressor::create(uint32_t
hdr,
...
{
return
std::make_unique<SHR3Decompressor>(hdr, recursionLevel, packedData, state, verify
);
}
SHR3Decompressor::SHR3Decompressor(uint32_t
hdr,
uint32_t recursionLevel,
const Buffer &packedData, std::unique_ptr<XPKDecompressor::State>
&state,

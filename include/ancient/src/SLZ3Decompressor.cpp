...
bool SLZ3Decompressor::detectHeaderXPK(uint32_t
hdr) noexcept {
return hdr==FourCC("SLZ3");
}
std::unique_ptr <XPKDecompressor> SLZ3Decompressor::create(uint32_t
hdr,
...
{
return
std::make_unique<SLZ3Decompressor>(hdr, recursionLevel, packedData, state, verify
);
}
SLZ3Decompressor::SLZ3Decompressor(uint32_t
hdr,
uint32_t recursionLevel,
const Buffer &packedData, std::unique_ptr<XPKDecompressor::State>
&state,

...
bool CRMDecompressor::detectHeader(uint32_t
hdr) noexcept {
switch (hdr) {
case FourCC("CrM!"):
case FourCC("CrM2"):
...
bool CRMDecompressor::detectHeaderXPK(uint32_t
hdr) noexcept
{
...
std::unique_ptr <XPKDecompressor> CRMDecompressor::create(uint32_t
hdr,
...
{
return
std::make_unique<CRMDecompressor>(hdr, recursionLevel, packedData, state, verify
);
}
...
CRMDecompressor::CRMDecompressor(uint32_t
hdr,
uint32_t recursionLevel,
const Buffer &packedData, std::unique_ptr<XPKDecompressor::State>
&state,

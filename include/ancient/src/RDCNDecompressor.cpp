...
bool RDCNDecompressor::detectHeaderXPK(uint32_t
hdr) noexcept {
return hdr==FourCC("RDCN");
}
std::unique_ptr <XPKDecompressor> RDCNDecompressor::create(uint32_t
hdr,
...
{
return
std::make_unique<RDCNDecompressor>(hdr, recursionLevel, packedData, state, verify
);
}
RDCNDecompressor::RDCNDecompressor(uint32_t
hdr,
uint32_t recursionLevel,
const Buffer &packedData, std::unique_ptr<XPKDecompressor::State>
&state,

...
bool HFMNDecompressor::detectHeaderXPK(uint32_t
hdr) noexcept {
return hdr==FourCC("HFMN");
}
std::unique_ptr <XPKDecompressor> HFMNDecompressor::create(uint32_t
hdr,
...
{
return
std::make_unique<HFMNDecompressor>(hdr, recursionLevel, packedData, state, verify
);
}
HFMNDecompressor::HFMNDecompressor(uint32_t
hdr,
uint32_t recursionLevel,
const Buffer &packedData, std::unique_ptr<XPKDecompressor::State>
&state,
...
_headerSize = tmp &
              0x1ffU;                    // the top 7 bits are flags. No definition what they are and they are ignored in decoder...

...
bool SMPLDecompressor::detectHeaderXPK(uint32_t
hdr) noexcept {
return hdr==FourCC("SMPL");
}
std::unique_ptr <XPKDecompressor> SMPLDecompressor::create(uint32_t
hdr,
...
{
return
std::make_unique<SMPLDecompressor>(hdr, recursionLevel, packedData, state, verify
);
}
SMPLDecompressor::SMPLDecompressor(uint32_t
hdr,
uint32_t recursionLevel,
const Buffer &packedData, std::unique_ptr<XPKDecompressor::State>
&state,

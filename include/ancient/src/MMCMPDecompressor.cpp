...
bool MMCMPDecompressor::detectHeader(uint32_t
hdr) noexcept {
...
const std::string &MMCMPDecompressor::getName() const
noexcept
{
...
size_t MMCMPDecompressor::getPackedSize() const
noexcept
{
return
_packedSize;
}
size_t MMCMPDecompressor::getRawSize() const
noexcept
{
return
_rawSize;
...
ForwardInputStream inputStream(_packedData, OverflowCheck::sum(blockAddr, subBlocks * 8U, 20U, packTableSize),
                               OverflowCheck::sum(blockAddr, subBlocks * 8U, 20U, packedBlockSize));

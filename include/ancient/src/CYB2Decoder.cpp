...
bool CYB2Decoder::detectHeaderXPK(uint32_t
hdr) noexcept {
return hdr==FourCC("CYB2");
}
std::unique_ptr <XPKDecompressor> CYB2Decoder::create(uint32_t
hdr,
...
{
return
std::make_unique<CYB2Decoder>(hdr, recursionLevel, packedData, state, verify
);
}
CYB2Decoder::CYB2Decoder(uint32_t
hdr,
uint32_t recursionLevel,
const Buffer &packedData, std::unique_ptr<XPKDecompressor::State>
&state,

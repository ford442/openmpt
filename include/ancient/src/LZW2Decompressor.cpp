...
bool LZW2Decompressor::detectHeaderXPK(uint32_t
hdr) noexcept {
return hdr==FourCC("LZW2") || hdr==FourCC("LZW3");
}
std::unique_ptr <XPKDecompressor> LZW2Decompressor::create(uint32_t
hdr,
...
{
return
std::make_unique<LZW2Decompressor>(hdr, recursionLevel, packedData, state, verify
);
}
LZW2Decompressor::LZW2Decompressor(uint32_t
hdr,
uint32_t recursionLevel,
const Buffer &packedData, std::unique_ptr<XPKDecompressor::State>
&state,

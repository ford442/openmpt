...
bool TDCSDecompressor::detectHeaderXPK(uint32_t
hdr) noexcept {
return hdr==FourCC("TDCS");
}
std::unique_ptr <XPKDecompressor> TDCSDecompressor::create(uint32_t
hdr,
...
{
return
std::make_unique<TDCSDecompressor>(hdr, recursionLevel, packedData, state, verify
);
}
TDCSDecompressor::TDCSDecompressor(uint32_t
hdr,
uint32_t recursionLevel,
const Buffer &packedData, std::unique_ptr<XPKDecompressor::State>
&state,

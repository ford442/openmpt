...
bool DLTADecode::detectHeaderXPK(uint32_t
hdr) noexcept {
return hdr==FourCC("DLTA");
}
std::unique_ptr <XPKDecompressor> DLTADecode::create(uint32_t
hdr,
...
{
return
std::make_unique<DLTADecode>(hdr, recursionLevel, packedData, state, verify
);
}
DLTADecode::DLTADecode(uint32_t
hdr,
uint32_t recursionLevel,
const Buffer &packedData, std::unique_ptr<XPKDecompressor::State>
&state,

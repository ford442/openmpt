...
bool BZIP2Decompressor::detectHeader(uint32_t
hdr) noexcept {
return ((hdr&0xffff'ff00U)==FourCC("BZh\0") && (hdr&0xffU)>='1' && (hdr&0xffU)<='9');
}
bool BZIP2Decompressor::detectHeaderXPK(uint32_t
hdr) noexcept
{
...
std::unique_ptr <XPKDecompressor> BZIP2Decompressor::create(uint32_t
hdr,
...
{
return
std::make_unique<BZIP2Decompressor>(hdr, recursionLevel, packedData, state, verify
);
}
...
BZIP2Decompressor::BZIP2Decompressor(uint32_t
hdr,
uint32_t recursionLevel,
const Buffer &packedData, std::unique_ptr<XPKDecompressor::State>
&state,
...
const std::string &BZIP2Decompressor::getName() const
noexcept
{
...
const std::string &BZIP2Decompressor::getSubName() const
noexcept
{
...
size_t BZIP2Decompressor::getPackedSize() const
noexcept
{
...
size_t BZIP2Decompressor::getRawSize() const
noexcept
{

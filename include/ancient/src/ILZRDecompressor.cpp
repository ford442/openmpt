...
bool ILZRDecompressor::detectHeaderXPK(uint32_t
hdr) noexcept {
return hdr==FourCC("ILZR");
}
std::unique_ptr <XPKDecompressor> ILZRDecompressor::create(uint32_t
hdr,
...
{
return
std::make_unique<ILZRDecompressor>(hdr, recursionLevel, packedData, state, verify
);
}
ILZRDecompressor::ILZRDecompressor(uint32_t
hdr,
uint32_t recursionLevel,
const Buffer &packedData, std::unique_ptr<XPKDecompressor::State>
&state,

...
static bool readIMPHeader(uint32_t
hdr,
uint32_t &addition
) noexcept {
switch (hdr) {
...
bool IMPDecompressor::detectHeader(uint32_t
hdr) noexcept
{
uint32_t dummy;
return
readIMPHeader(hdr, dummy
);
}
bool IMPDecompressor::detectHeaderXPK(uint32_t
hdr) noexcept
{
...
std::unique_ptr <XPKDecompressor> IMPDecompressor::create(uint32_t
hdr,
...
{
return
std::make_unique<IMPDecompressor>(hdr, recursionLevel, packedData, state, verify
);
}
...
if((_endOffset & 1) || _endOffset < 0xc || _endOffset + 0x32 > packedData.size() ||
   !_rawSize || !_endOffset ||
   _rawSize > getMaxRawSize() || _endOffset > getMaxPackedSize())
...
IMPDecompressor::IMPDecompressor(uint32_t
hdr,
uint32_t recursionLevel,
const Buffer &packedData, std::unique_ptr<XPKDecompressor::State>
&state,

...
bool CBR0Decompressor::detectHeaderXPK(uint32_t
hdr) noexcept {
// CBR1 is practical joke: it is the same as CBR0 but with ID changed
return hdr==FourCC("CBR0") || hdr==FourCC("CBR1");
}
std::unique_ptr <XPKDecompressor> CBR0Decompressor::create(uint32_t
hdr,
...
{
return
std::make_unique<CBR0Decompressor>(hdr, recursionLevel, packedData, state, verify
);
}
CBR0Decompressor::CBR0Decompressor(uint32_t
hdr,
uint32_t recursionLevel,
const Buffer &packedData, std::unique_ptr<XPKDecompressor::State>
&state,

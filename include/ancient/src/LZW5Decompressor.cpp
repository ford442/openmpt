...
bool LZW5Decompressor::detectHeaderXPK(uint32_t
hdr) noexcept {
return hdr==FourCC("LZW5");
}
std::unique_ptr <XPKDecompressor> LZW5Decompressor::create(uint32_t
hdr,
...
{
return
std::make_unique<LZW5Decompressor>(hdr, recursionLevel, packedData, state, verify
);
}
LZW5Decompressor::LZW5Decompressor(uint32_t
hdr,
uint32_t recursionLevel,
const Buffer &packedData, std::unique_ptr<XPKDecompressor::State>
&state,

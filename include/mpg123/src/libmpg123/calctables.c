...
static void print_char_array(const char *indent, const char *name, size_t count, char tab[]) {
size_t block = 72 / 4;
size_t i = 0;
if(name)
...
static void print_value(int fixed, double fixed_scale, const char *name, double val) {
if(name)
...
static void print_array(int statick, int fixed, double fixed_scale, const char *indent, const char *name, size_t count,
                        double tab[]) {
size_t block = 72 / 17;
size_t i = 0;
if(name)
printf("%sconst%s real %s[%zu] = \n", statick ? "static " : "", fixed ? "" : " ALIGNED(16)", name, count);
printf("%s{\n", indent);
while (i < count) {
size_t line = block > count - i ? count - i : block;
printf("%s", indent);
if(fixed)
for(size_t j = 0; j < line; ++j, ++i)
printf("%s%c%11ld", i ? "," : "", j ? ' ' : '\t', (long) (DOUBLE_TO_REAL(fixed_scale * tab[i])));
else
for(size_t j = 0; j < line; ++j, ++i)
...
static void print_array2d(int fixed, double fixed_scale, const char *name, size_t x, size_t y, double tab[][y]) {
printf("static const%s real %s[%zu][%zu] = \n{\n", fixed ? "" : " ALIGNED(16)", name, x, y);
...
static void print_short_array(const char *indent, const char *name, size_t count, short tab[]) {
size_t block = 72 / 8;
size_t i = 0;
if(name)
...
static void print_fixed_array(const char *indent, const char *name, size_t count, real tab[]) {
size_t block = 72 / 13;
size_t i = 0;
if(name)
...
static void print_ushort_array(const char *indent, const char *name, size_t count, unsigned short tab[]) {
size_t block = 72 / 8;
size_t i = 0;
if(name)
...
// C99 allows passing VLA with the fast dimensions first.
static void print_short_array2d(const char *name, size_t x, size_t y, short tab[][y]) {
...
if(!strcmp("l3", argv[1])) {
print_array(1, fixed, SCALE_POW43 / REAL_FACTOR, "", "ispow", sizeof(ispow) / sizeof(*ispow), ispow);
...
print_array(1, fixed, SCALE_15 / REAL_FACTOR, "", "tan1_1", ASIZE(tan1_1), tan1_1);
print_array(1, fixed, SCALE_15 / REAL_FACTOR, "", "tan2_1", ASIZE(tan2_1), tan2_1);
print_array(1, fixed, SCALE_15 / REAL_FACTOR, "", "tan1_2", ASIZE(tan1_2), tan1_2);
print_array(1, fixed, SCALE_15 / REAL_FACTOR, "", "tan2_2", ASIZE(tan2_2), tan2_2);
print_array2d(fixed, SCALE_15 / REAL_FACTOR, "pow1_1", 2, 32, pow1_1);
print_array2d(fixed, SCALE_15 / REAL_FACTOR, "pow2_1", 2, 32, pow2_1);
print_array2d(fixed, SCALE_15 / REAL_FACTOR, "pow1_2", 2, 32, pow1_2);
print_array2d(fixed, SCALE_15 / REAL_FACTOR, "pow2_2", 2, 32, pow2_2);
}
if(fixed)
...
for(int i = 0; i < 9; ++i)
printf("%s\t{ mapbuf0[%d]+%d, mapbuf1[%d]+%d, mapbuf2[%d]+%d }\n", (i ? "," : ""), i, (int) (mapend[i][0] - map[i][0]),
       i, (int) (mapend[i][1] - map[i][1]), i, (int) (mapend[i][2] - map[i][2]));
printf("};\n");

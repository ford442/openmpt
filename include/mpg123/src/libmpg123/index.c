...
else { /* Double the step, half the fill. Should work as well for fill%2 = 1 */
size_t c;
...
if(newsize > 0 && newsize < fi->size) { /* When we reduce buffer size a bit, shrink stuff. */
...
debug3("wanting to add to fill %lu, step %lu, size %lu", (unsigned long) fi->fill, (unsigned long) fi->step,
       (unsigned long) fi->size);
if(fi->fill == fi->size) { /* Index is full, we need to shrink... or grow. */
...
debug3("added pos %li to index with fill %lu and step %lu", (long) pos, (unsigned long) fi->fill,
       (unsigned long) fi->step);
}
}

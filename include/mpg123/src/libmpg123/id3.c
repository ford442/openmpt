...
enum frame_types {
unknown = -2, text = -1, comment, extra, rva2, uslt, picture
};
...
static mpg123_text *
add_id3_text(mpg123_text **list, size_t *size, char id[4], char lang[3], mpg123_string *description) {
mdebug("add_id3_text id=%s lang=%s, desc=%s", id ? (char[5]) {id[0], id[1], id[2], id[3], 0} : "(nil)",
...
if(description) { // Overwrite entry with same description and same ID and language.
...
*/
static void
store_id3_text(mpg123_string *sb, unsigned char *source, size_t source_size, const int noquiet, const int notranslate) {
...
static void
process_comment(mpg123_handle *fr, enum frame_types tt, unsigned char *realdata, size_t realsize, int rva_level,
                char *id) {
/* Text encoding          $xx */
/* Language               $xx xx xx */
...
store_id3_text(&description, descr - 1, text - descr + 1, NOQUIET, fr->p.flags & MPG123_PLAIN_ID3TEXT);
if(tt == comment)
...
{
fprintf(stderr, "Note: ID3 comm/uslt desc of length %"
SIZE_P
".\n", (size_p) xcom->description.fill);
fprintf(stderr, "Note: ID3 comm/uslt text of length %"
SIZE_P
...
} else if(!strncasecmp(localex.description.p, "replaygain_album_", 17)) {
...
free(fr->id3v2_raw);
fr->id3v2_raw = NULL;
} else { /* Closing with a zero for paranoia. */
...
if(NOQUIET)
error4("Bad tag length (not synchsafe): 0x%02x%02x%02x%02x; You got a bad ID3 tag here.", buf[2], buf[3], buf[4],
       buf[5]);
return 0;
}
...
warning2("ID3v2: Won't parse the ID3v2 tag with major version"
         " %u and flags 0x%xu - some extra code may be needed", major, flags);
skiptag = 1;
}
...
error4("Bad (non-synchsafe/too large) tag offset from extended header:"
       "0x%02x%02x%02x%02x", tagdata[0], tagdata[1], tagdata[2], tagdata[3]);
} else if(tagpos < 6) {
ret = 0;
if(NOQUIET)
...
debug5("ID3v2: real tag data apparently ended after %lu bytes with 0x%02x%02x%02x%02x", tagpos, tagdata[tagpos],
       tagdata[tagpos + 1], tagdata[tagpos + 2], tagdata[tagpos + 3]);
...
else if(!bytes_to_long(tagdata + pos, framesize)) {
...
if(fflags & GROUP_FFLAG) { // Just skip group byte.
if(realsize) {
if(VERBOSE3)
...
break;
default:
if(NOQUIET) error1("ID3v2: unknown frame type %i", tt);
}
if(unsyncbuffer)
free(unsyncbuffer);
}

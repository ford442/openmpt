...
mpg123_position(mpg123_handle *mh, long frame_offset, long buffered_bytes, long *current_frame, long *frames_left,
                double *current_seconds, double *seconds_left) {
off_t curframe, frameleft;
long small_curframe, small_frameleft;
int err;
err = MPG123_LARGENAME(mpg123_position)(mh, frame_offset, buffered_bytes, &curframe, &frameleft, current_seconds,
                                        seconds_left);
if(err != MPG123_OK) return err;
...
switch (ioh->iotype) {
case IO_FD:
return ioh->r_read(ioh->fd, buf, count);
case IO_HANDLE:
return ioh->r_h_read(ioh->handle, buf, count);
}
...
switch (ioh->iotype) {
case IO_FD:
return ioh->r_lseek(ioh->fd, smalloff, whence);
case IO_HANDLE:
return ioh->r_h_lseek(ioh->handle, smalloff, whence);
}
return -1;
} else {
errno = EOVERFLOW;
...
int attribute_align_arg
mpg123_replace_reader(mpg123_handle *mh, ssize_t (*r_read)(int, void *, size_t), long (*r_lseek)(int, long, int)) {
...
int attribute_align_arg mpg123_replace_reader_handle(mpg123_handle *mh, ssize_t (*r_read)(void *, void *, size_t),
                                                     long (*r_lseek)(void *, long, int), void (*cleanup)(void *)) {
struct wrap_data *ioh;
if(mh == NULL) return MPG123_ERR;
mpg123_close(mh);
ioh = wrap_get(mh);

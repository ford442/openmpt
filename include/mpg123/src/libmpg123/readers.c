...
if(ret < 0) {
if(NOQUIET) error("reading icy size");
return READER_ERROR;
}
if(ret == 0) break;
...
if(ret < 1) {
if(NOQUIET) error("reading icy-meta");
return READER_ERROR;
}
left -= ret;
}
...
if(ret < 0) {
if(NOQUIET) error1("reading the rest of %li", (long) (count - cnt));
return READER_ERROR;
}
if(ret == 0) break;
cnt += ret;
...
ssize_t
num = len < (off_t)
sizeof(buf) ? (ssize_t) len : (ssize_t)
sizeof(buf);
...
#ifndef NO_FEEDER
else if(fr->rdat.flags & READER_BUFFERED) { /* Perhaps we _can_ go a bit back. */
...
bc->last = newbuf;
debug3("bc_append: new last buffer %p with %"SSIZE_P" B (really %"SSIZE_P")", (void *) bc->last,
       (ssize_p) bc->last->size, (ssize_p) bc->last->realsize);
return 0;
}
...
debug("forget with nothing there!");
...
bc->pos -= b->size;
bc->size -= b->size;
debug5("bc_forget: forgot %p with %lu, pos=%li, size=%li, fileoff=%li", (void *) b->data, (long) b->size,
       (long) bc->pos, (long) bc->size, (long) bc->fileoff);
bc_free(bc, b);
b = n;
}
bc->first = b;
...
if(pos >= bc->fileoff && pos - bc->fileoff < bc->size) { /* We have the position! */
...
} else { /* I expect to get the specific position on next feed. Forget what I have now. */
bc_reset(bc);
bc->fileoff = pos;
...
if(bc->size - bc->pos < count) { /* Add more stuff to buffer. If hitting end of file, adjust count. */

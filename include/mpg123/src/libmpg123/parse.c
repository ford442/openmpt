...
enum parse_codes {
PARSE_MORE = MPG123_NEED_MORE, PARSE_ERR = MPG123_ERR, PARSE_END = 10 /* No more audio data to find. */
...
if(VERBOSE3)fprintf(stderr, "Note: Ignoring Xing frames because of MPG123_IGNORE_STREAMLENGTH\n");
} else {
...
long_tmp = bit_read_long(fr->bsbuf, &lame_offset);
if(fr->p.flags & MPG123_IGNORE_STREAMLENGTH) {
if(VERBOSE3)fprintf(stderr, "Note: Ignoring Xing bytes because of MPG123_IGNORE_STREAMLENGTH\n");
} else {
...
if((off_t) long_tmp != fr->rdat.filelen - fr->audio_start &&
   NOQUIET) { /* 1/filelen instead of 1/(filelen-start), my decision */
...
break;
default:
fr->vbr = MPG123_VBR; /* 00==unknown is taken as VBR */
}
...
mdebug("stopping parsing at %"OFF_P
               " frames as indicated fixed track length", (off_p) fr->num + 1);
return 0;
}
read_again:
...
if(ret == PARSE_BAD) { /* Header was not good. */
...
if(NOQUIET) fprintf(stderr, "\nWarning: Encountered more data after announced end of track (frame %"
OFF_P
"/%"
OFF_P
"). Frankenstein!\n", (off_p) fr->num,
...
if(NOQUIET)
error2("Frame smaller than mandatory side info (%i < %i)!", fr->framesize, fr->ssize);
return PARSE_BAD;
}
break;
#endif
...
#endif
if(left < 0 || lefts < 0) { /* That is the case for non-seekable streams. */
left = 0;
lefts = 0.0;
}
...
val = ((unsigned long) apebuf[11] << 24)
      ...
      | apebuf[8];
debug2("skipping %lu bytes of APE data at %"OFF_P, val, (off_p) fr->rd->tell(fr));
...
debug1("hopefully found one at %"OFF_P, (off_p) fr->rd->tell(fr));
...
fprintf(stderr, "Note: Illegal Audio-MPEG-Header 0x%08lx at offset %"
OFF_P
".\n",
...
if(NOQUIET && (newhead & 0xffffff00) == ('b' << 24) + ('m' << 16) + ('p' << 8))
...
if(NOQUIET)
error1("Giving up resync after %li bytes - your stream is not nice... (maybe increasing resync limit could help).",
       try);
fr->err = MPG123_RESYNC_FAIL;
return PARSE_ERR;
} else {

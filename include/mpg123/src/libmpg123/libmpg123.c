...
break;
case MPG123_LEFT:
mh->equalizer[0][band] = DOUBLE_TO_REAL(val);
break;
case MPG123_RIGHT:
mh->equalizer[1][band] = DOUBLE_TO_REAL(val);
break;
default:
...
break;
case MPG123_LEFT:
ret = REAL_TO_DOUBLE(mh->equalizer[0][band]);
break;
case MPG123_RIGHT:
ret = REAL_TO_DOUBLE(mh->equalizer[1][band]);
break;
...
debug3("updating decoder structure with native rate %li and af.rate %li (new format: %i)", native_rate, mh->af.rate,
       mh->new_format);
...
do_rva(mh);
debug3("done updating decoder structure with native rate %li and af.rate %li and down_sample %i", frame_freq(mh),
       mh->af.rate, mh->down_sample);
mh->decoder_change = 0;
return 0;
}
...
debug4("read of frame %li returned %i (to_decode=%i) at sample %li", (long) mh->num, b, mh->to_decode,
       (long) mpg123_tell(mh));
if(b == MPG123_NEED_MORE) return MPG123_NEED_MORE; /* need another call with data */
else if(b <= 0) {
/* More sophisticated error control? */
if(b == 0 || (mh->rdat.filelen >= 0 && mh->rdat.filepos == mh->rdat.filelen)) { /* We simply reached the end. */
...
if(fr->buffer.fill < needed_bytes) {
if(VERBOSE2)
fprintf(stderr, "Note: broken frame %li, filling up with %"
SIZE_P
" zeroes, from %"
SIZE_P
...
mpg123_framebyframe_decode(mpg123_handle *mh, off_t *num, unsigned char **audio, size_t *bytes) {
...
int attribute_align_arg
mpg123_decode(mpg123_handle *mh, const unsigned char *inmemory, size_t inmemsize, void *outmem, size_t outmemsize,
              size_t *done) {
int ret = MPG123_OK;
size_t mdone = 0;
...
while (ret == MPG123_OK) {
debug4("decode loop, fill %i (%li vs. %li); to_decode: %i", (int) mh->buffer.fill, (long) mh->num,
       (long) mh->firstframe, mh->to_decode);
...
mh->to_decode = mh->to_ignore = FALSE;
mh->buffer.p = mh->buffer.data;
debug2("decoded frame %li, got %li samples in buffer", (long) mh->num,
       (long) (mh->buffer.fill / (samples_to_bytes(mh, 1))));
FRAME_BUFFERCHECK(mh);
}
...
int attribute_align_arg mpg123_info(mpg123_handle *mh, struct mpg123_frameinfo *mi) MPG123_INFO_FUNC
int attribute_align_arg mpg123_info2(mpg123_handle *mh, struct mpg123_frameinfo2 *mi) MPG123_INFO_FUNC
...
debug5("tell: %li/%i first %li buffer %lu; frame_outs=%li", (long) mh->num, mh->to_decode, (long) mh->firstframe,
       (unsigned long) mh->buffer.fill, (long) frame_outs(mh, mh->num));
{ /* Funny block to keep C89 happy. */
off_t pos = 0;
if((mh->num < mh->firstframe) ||
   (mh->num == mh->firstframe && mh->to_decode)) { /* We are at the beginning, expect output from firstframe on. */
...
if(pos < 0 &&
   whence != SEEK_SET) { /* Unless we got the obvious error of NULL handle, this is a special seek failure. */
...
if(mh->rdat.filelen > 0) { /* A bad estimate. Ignoring tags 'n stuff. */
...
debug2("Scanning yielded %"OFF_P" track samples, %"OFF_P" frames.", (off_p) mh->track_samples,
       (off_p) mh->track_frames);
#ifdef GAPLESS
...
int attribute_align_arg
mpg123_id3_raw(mpg123_handle *mh, unsigned char **v1, size_t *v1_size, unsigned char **v2, size_t *v2_size) {
if(!mh)
return MPG123_ERR;
if(v1 != NULL)
...
switch (id3_enc_byte) {
case mpg123_id3_latin1:
return mpg123_text_latin1;
case mpg123_id3_utf16bom:
return mpg123_text_utf16bom; /* ID3v2.3 has UCS-2 with BOM here. */
case mpg123_id3_utf16be:
return mpg123_text_utf16be;
case mpg123_id3_utf8:
return mpg123_text_utf8;
default:
return mpg123_text_unknown;
}
}
...
#ifndef NO_STRING
int attribute_align_arg
mpg123_store_utf8(mpg123_string *sb, enum mpg123_text_encoding enc, const unsigned char *source, size_t source_size) {
switch (enc) {
#ifndef NO_ID3V2
...
int attribute_align_arg
mpg123_store_utf8_2(mpg123_string *sb, int enc, const unsigned char *source, size_t source_size) {
...
static const char *mpg123_error[] =
        {
                ...
                "Feature not in this build.", "Some bad value has been provided.",
                "Low-level seeking has failed (call to lseek(), usually).", "Custom I/O obviously not prepared.",
                "Overflow in LFS (large file support) conversion.", "Overflow in integer conversion.",

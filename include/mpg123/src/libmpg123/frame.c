...
if(NOQUIET)
merror("have external buffer of size %"SIZE_P", need %"SIZE_P, (size_p) fr->buffer.size, (size_p) size);
return MPG123_ERR;
}
}
...
mpg123_framedata(mpg123_handle *mh, unsigned long *header, unsigned char **bodydata, size_t *bodybytes) {
if(mh == NULL) return MPG123_BAD_HANDLE;
...
debug5("fuzzy: want %li of %li, get %li at %li B of %li B",
       (long) want_frame, (long) fr->track_frames, (long) *get_frame, (long) ret,
       (long) (fr->rdat.filelen - fr->audio_start));
return ret;
}
/*
...
default:
if(NOQUIET)
merror("Bad down_sample (%i) ... should not be possible!!", fr->down_sample);
}
return outs;
}
...
debug3("changing scale value from %f to %f (peak estimated to %f)",
       fr->lastscale != -1 ? fr->lastscale : fr->p.outscale, newscale, (double) (newscale * peak));

...
} else if(Vol->IsArchive(true) && (Vol->SFXSize > 0 || CmpExt(fd.Name, L"rar"))) {
...
else if((uint) ItemPos < RecItems.Size()) // Check if found more REV than needed.
{
...
FoundRecVolumes++;
uint RevCRC;
CalcFileSum(&CurFile, &RevCRC, NULL, 1, INT64NDF,
            CALCFSUM_CURPOS | (Cmd->DisablePercentage ? 0 : CALCFSUM_SHOWPROGRESS));

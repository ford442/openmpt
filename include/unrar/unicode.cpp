...
Size += 2;
else if((uint) *Src < 0x10000) //(uint) to avoid Clang/win "always true" warning for 16-bit wchar_t.
{
...
Size += 3;
} else if((uint) *Src < 0x200000) //(uint) to avoid Clang/win "always true" warning for 16-bit wchar_t.
Size += 4;

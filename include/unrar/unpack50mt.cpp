...
DataSize -= BufPos;
break;
} else if(CurData->BlockHeader.LastBlockInFile) {
Done = true;
break;
}
}
...
} else if(Item->Type == UNPDT_FULLREP) {
if(LastLength != 0)
CopyString(LastLength, OldDist[0]);
} else if(Item->Type == UNPDT_FILTER) {
UnpackFilter Filter;

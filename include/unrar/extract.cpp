...
if (Type==FSREDIR_HARDLINK || Type==FSREDIR_FILECOPY)
{
  ...
  */    // OPENMPT ADDITION
} else if(!Arc.FileHead.SplitBefore)
...
bool ValidCRC = !Arc.FileHead.SplitAfter &&
                DataIO.UnpHash.Cmp(&Arc.FileHead.FileHash, Arc.FileHead.UseHashKey ? Arc.FileHead.HashKey : NULL);
...
else if(Arc.FileHead.Method != 0 && Arc.FileHead.UnpSize > 0 && ValidCRC)
...
bool
CmdExtract::ExtractFileCopy(File &New, wchar *ArcName, wchar *NameNew, wchar *NameExisting, size_t NameExistingSize) {
...
DestName[1] = ':';
else if(DestName[0] == '_' && DestName[1] == '_') {
...
#if !defined(SILENT)
else if(!GlobalPassword && !Arc.FileHead.Solid) {
...
if(!FileCreate(Cmd, &CurFile, DestFileName, ASIZE(DestFileName), &UserReject, Arc.FileHead.UnpSize, &Arc.FileHead.mtime,
               true)) {
Success = false;
if(!UserReject) {

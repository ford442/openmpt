...
else if(hd.SplitBefore)
wcsncpyz(RatioStr, L"<--", ASIZE(RatioStr));
else if(hd.SplitAfter)
wcsncpyz(RatioStr, L"-->", ASIZE(RatioStr));
else
...
Type = St(MListUSymlink);
break;
case FSREDIR_WINSYMLINK:
Type = St(MListWSymlink);
break;
case FSREDIR_JUNCTION:
Type = St(MListJunction);
break;
case FSREDIR_HARDLINK:
Type = St(MListHardlink);
break;
...
else if(hd.FileHash.Type == HASH_BLAKE2) {

...
noexcept
        : m_val(val ? 1 : 0) {}
constexpr explicit Bool(ULong val)
noexcept
        : m_val(val ? 1 : 0) {}
...
ULong m_lo;
public:
constexpr HiLoLongLong()
noexcept
        : m_hi(0)
, m_lo(
0) {
}
constexpr HiLoLongLong(LongLong val)
noexcept
        : m_hi(static_cast<ULong>((static_cast<ULongLong>(val) & 0xffffffff00000000ull) >> 32))
, m_lo(
static_cast<ULong>((static_cast
<ULongLong>(val)
& 0x00000000ffffffffull) >> 0)) {
}
constexpr operator LongLong() const
noexcept {
...
public:
constexpr CharBuf(std::nullptr_t)
noexcept
        : CharBuf() {
}
inline CharBuf(const char *str)
noexcept
        : CharBuf() {
        if(str) {
...
inline CharBuf(const std::string_view &str)
noexcept
        : CharBuf() {
        std::copy(str.data(), str.data() + std::min(str.length(), size - 1), buf);
        std::fill(buf + std::min(str.length(), size - 1), buf + size, Char('\0'));
}
inline CharBuf(const std::string &str)
noexcept
        : CharBuf() {
        std::copy(str.data(), str.data() + std::min(str.length(), size - 1), buf);
        std::fill(buf + std::min(str.length(), size - 1), buf + size, Char('\0'));
}
inline CharBuf &operator=(std::nullptr_t)
noexcept {
std::fill(buf, buf + size, Char('\0'));
return *this;
}
inline CharBuf &operator=(const char *str)
noexcept {
if(str) {
std::copy(str, str + std::min(std::strlen(str), size - 1), buf);
std::fill(buf + std::min(std::strlen(str), size - 1), buf + size, Char('\0'));
...
inline CharBuf &operator=(const std::string &str)
noexcept {
std::fill(buf, buf + size, Char('\0'));
std::copy(str.data(), str.data() + std::min(str.length(), size - 1), buf);
std::fill(buf + std::min(str.length(), size - 1), buf + size, Char('\0'));
...
inline CharBuf &operator=(const std::string_view &str)
noexcept {
std::fill(buf, buf + size, Char('\0'));
std::copy(str.data(), str.data() + std::min(str.length(), size - 1), buf);
std::fill(buf + std::min(str.length(), size - 1), buf + size, Char('\0'));
...
inline constexpr std::size_t
SizeOfLongLong = sizeof(LongLong);
inline constexpr std::size_t
SizeOfDouble = sizeof(Double);
inline constexpr std::size_t
SizeOfHiLoLongLong = sizeof(HiLoLongLong);
static_assert(SizeOfChar
== 1);
static_assert(SizeOfByte
== 1);
static_assert(SizeOfBool
== SizeOfLong);
static_assert(SizeOfHiLoLongLong
== SizeOfLongLong);
static_assert(SizeOfLongLong
== 8);

...
public:
Error(ErrorCode ec)
        : std::runtime_error(std::string("ASIO Error ") + std::string(Message(ec))), m_Code(ec) {
return;
}
...
void (*bufferSwitch)(void *context, Long doubleBufferIndex, Bool directProcess)
noexcept = nullptr;
void (*sampleRateDidChange)(void *context, SampleRate sRate)
noexcept = nullptr;
Long (*asioMessage)(void *context, MessageSelector selector, const Long value, const void *message, const Double *opt)
noexcept = nullptr;
const Time *(*bufferSwitchTimeInfo)(void *context, const Time *params, Long doubleBufferIndex, Bool directProcess)
noexcept = nullptr;
};
...
public:
static constexpr Callbacks
init()
noexcept {
Callbacks result = {&CallbackBufferSwitch, &CallbackSampleRateDidChange, &CallbackAsioMessage,
                    &CallbackBufferSwitchTimeInfo};
return result;
}
public:
static void ASIO_CALL CallbackBufferSwitch ASIO_ATTR_CALL(Long doubleBufferIndex, Bool directProcess)
noexcept {
return (*A)[I].callbacks.bufferSwitch((*A)[I].context, doubleBufferIndex, directProcess);
}
static void ASIO_CALL CallbackSampleRateDidChange ASIO_ATTR_CALL(SampleRate sRate)
noexcept {
return (*A)[I].callbacks.sampleRateDidChange((*A)[I].context, sRate);
}
static Long ASIO_CALL
CallbackAsioMessage ASIO_ATTR_CALL(MessageSelector selector, Long value, const void *message, const Double *opt)
noexcept {
return (*A)[I].callbacks.asioMessage((*A)[I].context, selector, value, message, opt);
}
static const Time *
ASIO_CALL CallbackBufferSwitchTimeInfo ASIO_ATTR_CALL(const Time *params, Long doubleBufferIndex, Bool directProcess)
noexcept {
...
static constexpr auto construct_callbacks_array(std::index_sequence<Is...>)
noexcept -> std::array<Callbacks, sizeof...(Is)> {
return {CallbacksWrapper<Tarray, a, Is>::init()...};
}
private:
static inline std::mutex s_AllocationMutex;
static inline std::array<bool, MaxInstances> s_Allocation = detail::init_array < bool, MaxInstances
>(false);
static inline std::array <CallbacksWrapperState, MaxInstances> s_AsioCallbackWrapperStates =
        detail::init_array < CallbacksWrapperState, MaxInstances
>(CallbacksWrapperState{{ nullptr, nullptr, nullptr, nullptr }, nullptr });
static constexpr inline std::array<Callbacks, MaxInstances>
s_AsioCallbacks = construct_callbacks_array<
        std::array < CallbacksWrapperState, MaxInstances>, &s_AsioCallbackWrapperStates
>(
std::make_index_sequence<MaxInstances>()
);
private:
...
static void Free(std::size_t index)
noexcept {
std::lock_guard<std::mutex> guard(s_AllocationMutex);
...
static void Unmultiplex(std::pair<std::size_t, Callbacks> state)
noexcept {
Free(state.first);
}
};
...
public:
virtual operator const Callbacks *() const
noexcept = 0;
virtual operator const Callbacks &() const
noexcept = 0;
virtual operator Callbacks *()
noexcept = 0;
virtual operator Callbacks &()
noexcept = 0;
};
...
public:
operator const Callbacks *() const
noexcept final{
...
struct BufferIndex {
std::uint8_t Index: 1;
constexpr BufferIndex()
noexcept
        : Index(0) {
}
constexpr BufferIndex(Long doubleBufferIndex)
noexcept
        : Index(static_cast<std::uint8_t>(static_cast<ULong>(doubleBufferIndex) & 1u)) {
}
constexpr BufferIndex(std::size_t bufferIndex)
noexcept
...
public:
virtual Long
CallbackMessage(Driver &driver, MessageSelector selector, Long value, const void *message, const Double *opt)
noexcept = 0;
virtual void CallbackSampleRateDidChange(Driver &driver, SampleRate sRate)
noexcept = 0;
virtual void CallbackBufferSwitch(Driver &driver, Long doubleBufferIndex, Bool directProcess)
noexcept = 0;
virtual const Time *
CallbackBufferSwitchTimeInfo(Driver &driver, const Time *params, Long doubleBufferIndex, Bool directProcess)
noexcept = 0;
};
...
public:
bool MessageSelectorSupported(MessageSelector selector) const
noexcept {
bool result = false;
switch (selector) {
...
Long MessageEngineVersion() const
noexcept {
return 2;
}
virtual void MessageResetRequest()
noexcept = 0;
virtual bool MessageBufferSizeChange(Long newSize)
noexcept {
static_cast<void>(newSize);
return false;
}
virtual bool MessageResyncRequest()
noexcept {
return false;
}
virtual void MessageLatenciesChanged()
noexcept {
return;
}
bool MessageSupportsTimeInfo() const
noexcept {
return true;
}
bool MessageSupportsTimeCode() const
noexcept {
return true;
}
virtual Long MessageMMCCommand(Long value, const void *message, const Double *opt)
noexcept {
...
bool MessageSupportsInputMonitor() const
noexcept {
return true;
}
bool MessageSupportsInputGain() const
noexcept {
return true;
}
bool MessageSupportsInputMeter() const
noexcept {
return true;
}
bool MessageSupportsOutputGain() const
noexcept {
return true;
}
bool MessageSupportsOutputMeter() const
noexcept {
return true;
}
virtual void MessageOverload()
noexcept {
return;
}
virtual Long MessageUnknown(MessageSelector selector, Long value, const void *message, const Double *opt)
noexcept {
...
virtual void RealtimeSampleRateDidChange(SampleRate sRate)
noexcept {
static_cast<void>(sRate);
}
virtual void RealtimeRequestDeferredProcessing(bool deferred)
noexcept {
static_cast<void>(deferred);
}
virtual void RealtimeTimeInfo(Time time)
noexcept {
static_cast<void>(time);
}
virtual void RealtimeBufferSwitch(ASIO::BufferIndex bufferIndex)
noexcept = 0;
public:
Long CallbackMessage(Driver &driver, MessageSelector selector, Long value, const void *message, const Double *opt)
noexcept final{
...
void CallbackSampleRateDidChange(Driver &driver, SampleRate sRate)
noexcept final{
        static_cast<void>(driver);
        RealtimeSampleRateDidChange(sRate);
}
void CallbackBufferSwitch(Driver &driver, Long doubleBufferIndex, Bool directProcess)
noexcept final{
        CallbackBufferSwitchTimeInfo(driver, nullptr, doubleBufferIndex, directProcess);
}
const Time *CallbackBufferSwitchTimeInfo(Driver &driver, const Time *params, Long doubleBufferIndex, Bool directProcess)
noexcept final{
        Time time;
        if(params) {
        time = *params;
...
private:
const IDriver &realDriver() const
noexcept {
return *m_Driver;
}
IDriver &realDriver()
...
private:
static Driver *ThisFromVoid(void *context)
noexcept {
...
static constexpr CallbacksWithContext
MyCallbacks()
noexcept {
CallbacksWithContext result = {&CallbackBufferSwitch, &CallbackSampleRateDidChange, &CallbackAsioMessage,
                               &CallbackBufferSwitchTimeInfo};
return result;
}
private:
static void CallbackBufferSwitch(void *context, Long doubleBufferIndex, Bool directProcess)
noexcept {
assert(context);
assert(ThisFromVoid(context)->m_CallbackHandler);
return ThisFromVoid(context)->m_CallbackHandler->CallbackBufferSwitch(*ThisFromVoid(context), doubleBufferIndex,
                                                                      directProcess);
}
static void CallbackSampleRateDidChange(void *context, SampleRate sRate)
noexcept {
assert(context);
assert(ThisFromVoid(context)->m_CallbackHandler);
...
static Long
CallbackAsioMessage(void *context, MessageSelector selector, Long value, const void *message, const Double *opt)
noexcept {
assert(context);
assert(ThisFromVoid(context)->m_CallbackHandler);
...
static const Time *
CallbackBufferSwitchTimeInfo(void *context, const Time *params, Long doubleBufferIndex, Bool directProcess)
noexcept {
assert(context);
assert(ThisFromVoid(context)->m_CallbackHandler);
return ThisFromVoid(context)->m_CallbackHandler->CallbackBufferSwitchTimeInfo(*ThisFromVoid(context), params,
...
try {
CheckOKNoOutOfMemory(
        m_Driver->createBuffers(bufferInfos.data(), static_cast<Long>(bufferInfos.size()), bufferSize, *m_Callbacks));
} catch (...) {
m_Callbacks = nullptr;

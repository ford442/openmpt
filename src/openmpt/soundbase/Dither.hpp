...
MultiChannelDither(Trd &rd, std::size_t channels)
        : DitherChannels(channels), prng(Tdither::prng_init(rd)) {
return;
}
void Reset() {
...
template<uint32 targetbits>
MPT_FORCEINLINE MixSampleFloat
process(std::size_t
channel,
MixSampleFloat sample
)
{
return DitherChannels[channel].template process<targetbits>(sample, prng);
}
...
std::visit([](auto &dither) { return dither.Reset(); },
           m_Dithers);
return;
}
...
std::size_t GetChannels() const {
return std::visit([](auto &dither) { return dither.GetChannels(); },
                  m_Dithers);
}
};

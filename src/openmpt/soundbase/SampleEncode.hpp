...
exp_table[17] = { 0, 7 << 4, 6 << 4, 5 << 4, 4 << 4, 3 << 4, 2 << 4, 1 << 4, 0 << 4, 0, 0, 0, 0, 0, 0, 0, 0 };
static constexpr uint8
mant_table[17] = { 0, 10, 9, 8, 7, 6, 5, 4, 3, 3, 3, 3, 3, 3, 3, 3, 3 };
MPT_FORCEINLINE output_t
operator()(input_t val) {
...
x = static_cast<uint16>(std::clamp(static_cast<uint32>(abs + (33 << 2)), static_cast<uint32>(0),
                                   static_cast<uint32>(0x7fff)));
...
exp_table[17] = { 0, 7 << 4, 6 << 4, 5 << 4, 4 << 4, 3 << 4, 2 << 4, 1 << 4, 0 << 4, 0, 0, 0, 0, 0, 0, 0, 0 };
static constexpr uint8
mant_table[17] = { 0, 10, 9, 8, 7, 6, 5, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4 };
MPT_FORCEINLINE output_t
operator()(input_t val) {

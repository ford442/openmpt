...
Base::Base(ILogger &logger, SoundDevice::Info info, SoundDevice::SysInfo sysInfo)
        : m_Logger(logger), m_Callback(nullptr), m_MessageReceiver(nullptr), m_Info(info), m_SysInfo(sysInfo),
          m_StreamPositionOutputFrames(0), m_RequestFlags(0) {
...
timeInfo.SyncPointSystemTimestamp = CallbackLockedGetReferenceClockNowNanoseconds() +
                                    mpt::saturate_round<int64>(GetEffectiveBufferAttributes().Latency * 1000000000.0);
timeInfo.Speed = 1.0;
}
...
frames = mpt::saturate_round<int64>(
        ...
        (static_cast<double>(static_cast<int64>(now - timeInfo.SyncPointSystemTimestamp)) * timeInfo.Speed *
         m_Settings.Samplerate * (1.0 / (1000.0 * 1000.0))));
} else {

...
inline constexpr mpt::uchar
TypeWAVEOUT[] = MPT_ULITERAL("WaveOut");
inline constexpr mpt::uchar
TypeDSOUND[] = MPT_ULITERAL("DirectSound");
inline constexpr mpt::uchar
TypeASIO[] = MPT_ULITERAL("ASIO");
inline constexpr mpt::uchar
TypePORTAUDIO_WASAPI[] = MPT_ULITERAL("WASAPI");
inline constexpr mpt::uchar
TypePORTAUDIO_WDMKS[] = MPT_ULITERAL("WDM-KS");
inline constexpr mpt::uchar
TypePORTAUDIO_WMME[] = MPT_ULITERAL("MME");
inline constexpr mpt::uchar
TypePORTAUDIO_DS[] = MPT_ULITERAL("DS");
...
Info()
        : default_(Default::None), useNameAsIdentifier(false) {
}
bool IsValid() const {
...
SysInfo(mpt::osinfo::osclass systemClass, mpt::osinfo::windows::Version windowsVersion)
        : SystemClass(systemClass), WindowsVersion(windowsVersion) {
return;
}
SysInfo(mpt::osinfo::osclass systemClass, mpt::osinfo::windows::Version windowsVersion, bool isWine,
        mpt::osinfo::osclass wineHostClass, mpt::osinfo::windows::wine::version wineVersion)
        : SystemClass(systemClass), WindowsVersion(windowsVersion), IsWine(isWine), WineHostClass(wineHostClass),
...
AppInfo()
        : UIHandle(0), BoostedThreadPriorityXP(2)  // THREAD_PRIORITY_HIGHEST
        , BoostedThreadMMCSSClassVista(MPT_USTRING("Pro Audio")), BoostedThreadRealtimePosix(false),
          BoostedThreadNicenessPosix(-5), BoostedThreadRtprioPosix(10)
#if defined(MODPLUG_TRACKER)
...
Settings()
        : Latency(0.1), UpdateInterval(0.005), Samplerate(48000), Channels(2), InputChannels(0),
          sampleFormat(SampleFormat::Float32), ExclusiveMode(false), BoostThreadPriority(true), KeepDeviceRunning(true),
          UseHardwareTiming(false), DitherType(1), InputSourceID(0) {
return;
}
bool operator==(const SoundDevice::Settings &cmp) const {
return true
       && mpt::saturate_round<int64>(Latency * 1000000000.0) ==
          mpt::saturate_round<int64>(cmp.Latency * 1000000000.0)                // compare in nanoseconds
       && mpt::saturate_round<int64>(UpdateInterval * 1000000000.0) ==
          mpt::saturate_round<int64>(cmp.UpdateInterval * 1000000000.0)  // compare in nanoseconds
...
Caps()
        : Available(false), CanUpdateInterval(true), CanSampleFormat(true), CanExclusiveMode(false),
          CanBoostThreadPriority(true), CanKeepDeviceRunning(false), CanUseHardwareTiming(false),
          CanChannelMapping(false), CanInput(false), HasNamedInputSources(false), CanDriverPanel(false),
          HasInternalDither(false), ExclusiveModeDescription(MPT_USTRING("Use device exclusively")),
...
std::vector <SampleFormat> supportedExclusiveModeSampleFormats =
        DefaultSampleFormats < std::vector < SampleFormat >> ();
...
int NumBuffers;
BufferAttributes()
        : Latency(0.0), UpdateInterval(0.0), NumBuffers(0) {
return;
}
};
...
mpt::ustring text;
Statistics()
        : InstantaneousLatency(0.0), LastUpdateInterval(0.0) {
return;
}
};

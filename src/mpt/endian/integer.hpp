...
static constexpr mpt::endian
endian = mpt::endian::big;
};
struct LittleEndian_tag {
static constexpr mpt::endian
endian = mpt::endian::little;
};
constexpr inline uint16
constexpr_bswap16(uint16
x) noexcept {
return uint16(0)
| ((x >> 8) & 0x00FFu)
| ((x << 8) & 0xFF00u);
...
MPT_CONSTEXPRINLINE std::array<std::byte, size>
EndianEncode(T
val) noexcept {
static_assert(Tendian::endian
== mpt::endian::little || Tendian::endian == mpt::endian::big);
...
if
constexpr (endian_type::endian
== mpt::endian::little) {
for (
std::size_t i = 0;
i < sizeof(base_type); ++i) {
data[i] = static_cast<std::byte>(static_cast
<uint8>((uval
>> (i * 8)) & 0xffu));
}
} else {
...
std::size_t i = 0;
i < sizeof(base_type); ++i) {
data[(sizeof(base_type) - 1) - i] = static_cast<std::byte>(static_cast
<uint8>((uval
>> (i * 8)) & 0xffu));
}
}
...
EndianDecode(std::array<std::byte, size>
data) noexcept {
static_assert(Tendian::endian
== mpt::endian::little || Tendian::endian == mpt::endian::big);
static_assert(std::numeric_limits<T>::is_integer);
...
std::size_t i = 0;
i < sizeof(base_type); ++i) {
uval |= static_cast<unsigned_base_type>(static_cast
<uint8>(data[i])
) << (i * 8);
}
} else {
...
i < sizeof(base_type); ++i) {
uval |= static_cast<unsigned_base_type>(static_cast
<uint8>(data[(sizeof(base_type) - 1) - i])
) << (i * 8);
}
}
...
MPT_CONSTEXPR20_FUN uint64
SwapBytesImpl(uint64
value) noexcept {
...
MPT_CONSTEXPR20_FUN uint32
SwapBytesImpl(uint32
value) noexcept {
MPT_MAYBE_CONSTANT_IF (MPT_IS_CONSTANT_EVALUATED20()) {
...
MPT_CONSTEXPR20_FUN uint16
SwapBytesImpl(uint16
value) noexcept {
MPT_MAYBE_CONSTANT_IF (MPT_IS_CONSTANT_EVALUATED20()) {
...
MPT_CONSTEXPR20_FUN uint8
SwapBytesImpl(uint8
value) noexcept {
return
value;
}
MPT_CONSTEXPR20_FUN int8
SwapBytesImpl(int8
value) noexcept {
return
value;
}
MPT_CONSTEXPR20_FUN char SwapBytesImpl(char value)
noexcept {
return
value;
}
...
public:
MPT_CONSTEXPR20_FUN void set(base_type val)
noexcept {
static_assert(std::numeric_limits<T>::is_integer);
MPT_MAYBE_CONSTANT_IF(MPT_IS_CONSTANT_EVALUATED20())
{
...
MPT_CONSTEXPR20_FUN packed
&
operator=(const base_type &val)
noexcept {
set(val);
return *this;
}
MPT_CONSTEXPR20_FUN operator base_type() const
noexcept {
return get();
}
public:
...
template<typename T>
MPT_CONSTEXPR20_FUN auto as_le(T v)
noexcept -> typename mpt::make_le<typename std::remove_const<T>::type>::type {
...
template<typename T>
MPT_CONSTEXPR20_FUN auto as_be(T v)
noexcept -> typename mpt::make_be<typename std::remove_const<T>::type>::type {
...
MPT_CONSTEXPR20_FUN Tpacked
as_endian(typename Tpacked::base_type v)
noexcept {
Tpacked res{};
res = v;
return

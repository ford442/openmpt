...
template<typename Tchar>
constexpr bool is_any_line_ending(Tchar c)
noexcept {
...
return (str.rfind(typename mpt::make_string_type<Tmatch>::type{match}) ==
        (str.length() - typename mpt::make_string_type<Tmatch>::type{match}.length()));
}
...
str.replace(pos, typename mpt::make_string_type<Treplace>::type{old_str}.length(),
            typename mpt::make_string_type<Treplace>::type{new_str});
...
template<typename Tchar>
inline constexpr Tchar
to_lower_ascii(Tchar
c) noexcept {
if (char_constants<Tchar>::A <=
c &&c
<= char_constants<Tchar>::Z) {
c += char_constants<Tchar>::a -
char_constants<Tchar>::A;
}
return
c;
}
...
template<typename Tstring>
inline std::vector <Tstring>
split(const Tstring &str, const Tstring &sep = Tstring(1, char_constants<typename Tstring::value_type>::comma)) {
...
template<typename Tstring>
inline Tstring join(const std::vector <Tstring> &vals,
                    const Tstring &sep = Tstring(1, char_constants<typename Tstring::value_type>::comma)) {
Tstring str;

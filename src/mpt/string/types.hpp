...
template<typename T>
MPT_CONSTEXPRINLINE unsigned char char_value(T x)
noexcept = delete;
template<>
MPT_CONSTEXPRINLINE unsigned char char_value<char>(char x)
noexcept {
return static_cast
<unsigned char>(x);
}
template<>
MPT_CONSTEXPRINLINE unsigned char char_value<unsigned char>(unsigned char x)
noexcept {
return static_cast
<unsigned char>(x);
}
...
static constexpr char encode(char32_t c)
noexcept {
...
static constexpr wchar_t encode(char32_t c)
noexcept {
...
constexpr Tdstchar
unsafe_char_convert(Tsrcchar
src) noexcept {
return
mpt::unsafe_char_converter<Tdstchar>::encode (mpt::unsafe_char_converter<Tsrcchar>::decode(src));
}
#if !defined(MPT_COMPILER_QUIRK_NO_WCHAR)

...
inline mpt::widestring
decode_8bit(const Tsrcstring &str, const char32_t (&table)[256], mpt::widechar replacement = MPT_WIDECHAR('\uFFFD')) {
mpt::widestring res;
...
template<typename Tdststring>
inline Tdststring encode_utf8(const mpt::widestring &str,
                              typename Tdststring::value_type replacement = static_cast<typename Tdststring::value_type>(mpt::char_value(
                                      '?'))) {
...
inline std::wstring
decode_locale_impl(const std::string &str, const std::locale &locale, wchar_t replacement = L'\uFFFD', int retry = 0,
                   bool *progress = nullptr) {
if(str.empty()) {
return std::wstring();
}
...
inline std::string
encode_locale_impl(const std::wstring &str, const std::locale &locale, char replacement = '?', int retry = 0,
                   bool *progress = nullptr) {
if(str.empty()) {
return std::string();
}
...
return string_converter<Tdststring>::encode(
        string_converter<decltype(mpt::as_string(src))>::decode(mpt::as_string(src)));
}
}

...
template<typename T>
inline auto
format(const T &x) -> typename std::enable_if<mpt::test::is_to_stream_writable<std::ostringstream, T>::value, std::string>::type {
std::ostringstream s;
s << x;
return s.str();
}
...
constexpr statistics_counters
&
operator+=(const statistics_counters &other)
noexcept {
total += other.total;
run += other.run;
...
virtual void
case_run(const mpt::source_location &loc, const char *text_a, const char *text_cmp, const char *text_b) = 0;
...
group_statistics
run(mpt::test::reporter_interface &reporter, const mpt::source_location &loc = mpt::source_location::current()) {
...
public:
[[nodiscard]] static inline group *&group_list()
noexcept {
static group *group_list = nullptr;
return group_list;
}
};
inline global_statistics
run_all(mpt::test::reporter_interface &reporter, const mpt::source_location &loc = mpt::source_location::current()) {
...
inline test(mpt::test::context &context_, void (*breakpoint_)(void) = nullptr,
            const mpt::source_location &source_location_ = mpt::source_location::current())
        : context(context_), source_location(source_location_), breakpoint(breakpoint_) {
report_test_begin();
}
inline test(mpt::test::context &context_, const char *name_, void (*breakpoint_)(void) = nullptr,
            const mpt::source_location &source_location_ = mpt::source_location::current())
        : context(context_), name(name_), source_location(source_location_), breakpoint(breakpoint_) {
report_test_begin();
}
inline ~test() {
report_test_end();
...
inline test &expect(Ta &&a, Tcmp cmp, Tb &&b, const char *text_a = nullptr, const char *text_cmp = nullptr,
                    const char *text_b = nullptr) {
...
immediate_breakpoint();
result.info = mpt::test::result_failure{
        mpt::test::format(va) + " " + mpt::test::format(cmp) + " " + mpt::test::format(vb)};
} else {
result.info = mpt::test::result_success{};
}
} catch (...) {

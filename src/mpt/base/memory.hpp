...
template<typename T>
constexpr bool declare_binary_safe(const T &)
noexcept {
return false;
}
constexpr bool declare_binary_safe(const char &)
noexcept {
return true;
}
constexpr bool declare_binary_safe(const uint8 &)
noexcept {
return true;
}
constexpr bool declare_binary_safe(const int8 &)
noexcept {
return true;
}
constexpr bool declare_binary_safe(const std::byte &)
noexcept {
return true;
}
...
template<typename T>
constexpr bool check_binary_size(std::size_t size)
noexcept {
return true
&& (sizeof(T) == size)
&& (
...
static_assert(std::is_integral<Tsrc>::value || mpt::is_byte<Tsrc>::value);
static_assert(std::is_integral<Tdst>::value || mpt::is_byte<Tdst>::value);
return mpt::as_span(mpt::byte_cast_impl<Tdst *, Tsrc *>()(src.data()),
                    mpt::byte_cast_impl<Tdst *, Tsrc *>()(src.data() + src.size()));
}
};
template<typename Tdst, typename Tsrc>
...
noexcept {
return
void_cast_impl<Tdst, Tsrc>()
(src);
}
template<typename T>
MPT_CONSTEXPRINLINE std::byte
as_byte(T
src) noexcept {
static_assert(std::is_integral<T>::value);
return static_cast<std::byte>(static_cast
<uint8>(src)
);
}
template<typename T>

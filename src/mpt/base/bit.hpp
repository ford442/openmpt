...
MPT_FORCEINLINE typename std::enable_if<(sizeof(Tdst) == sizeof(Tsrc)) && std::is_trivially_copyable<Tsrc>::value &&
                                        std::is_trivially_copyable<Tdst>::value, Tdst>::type
bit_cast(const Tsrc &src)
noexcept {
Tdst dst{};
std::memcpy(&dst, &src,
sizeof(Tdst));
...
static_assert(mpt::endian::big
!= mpt::endian::little, "platform with all scalar types having size 1 is not supported");
MPT_FORCEINLINE mpt::endian
endian_probe()
noexcept {
using endian_probe_type = uint32;
static_assert(sizeof(endian_probe_type) == 4);
constexpr endian_probe_type
endian_probe_big = 0x12345678u;
constexpr endian_probe_type
endian_probe_little = 0x78563412u;
const std::array<std::byte, sizeof(endian_probe_type)> probe{{std::byte{0x12}, std::byte{0x34}, std::byte{
        0x56}, std::byte{0x78}}};
...
if
constexpr ((mpt::endian::native
== mpt::endian::little) || (mpt::endian::native == mpt::endian::big)) {
return
mpt::endian::native;
} else {
return
...
MPT_FORCEINLINE bool endian_is_little()
noexcept {
return
get_endian()
==
...
MPT_FORCEINLINE bool endian_is_big()
noexcept {
return
get_endian()
==
...
MPT_FORCEINLINE bool endian_is_weird()
noexcept {
return !
endian_is_little() &&
!
...
int count = 0;
for (
int bit = std::numeric_limits<T>::digits - 1;
bit >= 0; --bit) {
if ((x & (1u << bit)) == 0u) {
count++;
} else {
...
return mpt::bit_width(x) == static_cast<typename std::make_unsigned<T>::type>(mpt::popcount(x)) ? mpt::bit_width(x) :
       mpt::bit_width(x) - 1;
}
template<typename T>

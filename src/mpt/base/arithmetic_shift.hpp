...
template<typename T>
constexpr auto rshift_signed_standard(T x, int y)
noexcept ->
decltype(x
>> y) {
...
template<typename T>
constexpr auto lshift_signed_standard(T x, int y)
noexcept ->
decltype(x
<< y) {
static_assert(std::numeric_limits<T>::is_integer);
...
#else
template<typename T>
constexpr auto rshift_signed(T x, int y)
noexcept ->
decltype(x
>> y) {
return
mpt::rshift_signed_standard(x, y
);
}
template<typename T>
constexpr auto lshift_signed(T x, int y)
noexcept ->
decltype(x
<< y) {
return
mpt::lshift_signed_standard(x, y
);
}
#endif
template<typename T>
constexpr auto arithmetic_shift_right(T x, int y)
noexcept ->
decltype(x
>> y) {
return
mpt::rshift_signed(x, y
);
}
template<typename T>
constexpr auto arithmetic_shift_right(T x, int y)
...
<< y) {
return
mpt::lshift_signed(x, y
);
}
template<typename T>
constexpr auto sar(T x, int y)
noexcept ->
decltype(x
>> y) {
return

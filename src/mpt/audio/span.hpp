...
std::size_t m_frames;
public:
constexpr audio_span_planar_strided(SampleType *const *buffers, std::size_t channels, std::size_t frames,
                                    std::ptrdiff_t frame_stride)
noexcept
        : m_buffers(buffers)
...
SampleType *const *data_planar() const
noexcept {
return m_buffers;
}
SampleType *data() const
noexcept {
return nullptr;
}
...
std::size_t size_channels() const
noexcept {
return m_channels;
}
...
std::size_t size_samples() const
noexcept {
return m_channels * m_frames;
}
std::ptrdiff_t frame_stride() const
noexcept {
...
std::size_t m_frames;
public:
constexpr audio_span_planar(SampleType *const *buffers, std::size_t channels, std::size_t frames)
noexcept
        : m_buffers(buffers)
, m_channels(channels)
, m_frames(frames) {
...
std::size_t m_frames;
public:
constexpr audio_span_contiguous(SampleType *buffer, std::size_t channels, std::size_t frames)
noexcept
        : m_buffer(buffer)
, m_channels(channels)
, m_frames(frames) {
...
std::size_t m_frames;
public:
constexpr audio_span_interleaved(SampleType *buffer, std::size_t channels, std::size_t frames)
noexcept
        : m_buffer(buffer)
, m_channels(channels)
, m_frames(frames) {
...
// LRLRLRLRLR
inline constexpr audio_span_frames_are_contiguous_t
audio_span_frames_are_contiguous;
// LLLLLRRRRR
inline constexpr audio_span_channels_are_contiguous_t
audio_span_channels_are_contiguous;
// LLLLL RRRRR
inline constexpr audio_span_channels_are_planar_t
audio_span_channels_are_planar;
// LxxxLxxxLxxxLxxxLxxx xRxxxRxxxRxxxRxxxRxx
inline constexpr audio_span_channels_are_planar_and_strided_t
audio_span_channels_are_planar_and_strided;
template<typename SampleType>
struct audio_span {
public:
using sample_type = SampleType;
...
std::size_t m_frames;
public:
constexpr audio_span(audio_span_interleaved<SampleType> buffer)
noexcept
...
constexpr audio_span(audio_span_contiguous<SampleType> buffer)
noexcept
        : m_frame_stride(1)
, m_channel_stride(buffer
.
size_frames()
)
,
...
constexpr audio_span(audio_span_planar<SampleType> buffer)
noexcept
        : m_frame_stride(1)
, m_channel_stride(
0)
...
constexpr
audio_span(SampleType *const *planes, std::size_t channels, std::size_t frames, audio_span_channels_are_planar_t)
noexcept
        : m_frame_stride(1)
, m_channel_stride(
0)
...
constexpr audio_span(audio_span_planar_strided<SampleType> buffer)
noexcept
...
constexpr audio_span(SampleType *const *planes, std::size_t channels, std::size_t frames, std::ptrdiff_t frame_stride,
                     audio_span_channels_are_planar_and_strided_t)
noexcept
        : m_frame_stride(frame_stride)
, m_channel_stride(
0)
...
SampleType &operator()(std::size_t channel, std::size_t frame) const {
return is_contiguous() ? m_buffer.contiguous[(m_channel_stride * static_cast<std::ptrdiff_t>(channel)) +
                                             (m_frame_stride * static_cast<std::ptrdiff_t>(frame))]
                       : m_buffer.planes[channel][frame * static_cast<std::ptrdiff_t>(m_frame_stride)];
}
bool channels_are_contiguous() const
noexcept {
...
std::size_t m_offset;
public:
audio_span_with_offset(Taudio_span buffer, std::size_t offsetFrames)
noexcept
        : m_buffer(buffer)
...
inline audio_span_with_offset<BufferType> make_audio_span_with_offset(BufferType buf, std::size_t offsetFrames)
noexcept {
assert(offsetFrames
<= buf.
size_frames()
);
return audio_span_with_offset<BufferType> {
buf, offsetFrames
};
}

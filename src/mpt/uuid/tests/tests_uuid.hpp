...
MPT_TEST_EXPECT_EQUAL(mpt::UUID(0x2ed6593au, 0xdfe6, 0x4cf8, 0xb2e575ad7f600c32ull)
.
ToUString(), MPT_USTRING("2ed6593a-dfe6-4cf8-b2e5-75ad7f600c32")
);
#if MPT_OS_WINDOWS
...
#endif // MPT_OS_WINDOWS
MPT_TEST_EXPECT_EQUAL(mpt::UUID::Generate(prng)
.
IsValid(),
true);
MPT_TEST_EXPECT_EQUAL(mpt::UUID::GenerateLocalUseOnly(prng)
.
IsValid(),
true);
MPT_TEST_EXPECT_EQUAL(mpt::UUID::Generate(prng)
!=
mpt::UUID::Generate(prng),
true);
...
std::size_t i = 0;
i < 16; ++i) {
uuiddata[i] = mpt::byte_cast<std::byte>(static_cast
<uint8>(i)
);
}
static_assert(sizeof(mpt::UUID) == 16);
mpt::UUIDbin uuid2;
std::memcpy(&uuid2, uuiddata,
16);
MPT_TEST_EXPECT_EQUAL(mpt::UUID(uuid2)
.
ToUString(), MPT_USTRING("00010203-0405-0607-0809-0a0b0c0d0e0f")
);
constexpr mpt::UUID
uuid3 = "2ed6593a-dfe6-4cf8-b2e5-75ad7f600c32"_uuid;

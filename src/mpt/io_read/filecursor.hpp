...
FileCursor()
        : m_data(DataInitializer()), streamPos(0), m_fileName(nullptr) {
return;
}
...
explicit FileCursor(mpt::span<Tbyte> bytedata, shared_filename_type filename = shared_filename_type{})
        : m_data(DataInitializer(mpt::byte_cast<mpt::const_byte_span>(bytedata))), streamPos(0),
...
explicit FileCursor(value_data_type other, shared_filename_type filename = shared_filename_type{})
        : m_data(std::move(other)), streamPos(0), m_fileName(std::move(filename)) {
return;
}
public:
...
return FileCursor(
        CreateChunkImpl(SharedDataContainer(), position, std::min(length, DataContainer().GetLength() - position)));
}
public:
...
// deprecated because in case of an unseekable std::istream, this triggers caching of the whole file
return mpt::span(mpt::byte_cast<const T *>(DataContainer().GetRawData() + streamPos),
                 DataContainer().GetLength() - streamPos);
}

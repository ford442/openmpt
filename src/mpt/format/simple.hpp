...
static_assert(std::numeric_limits<T>::is_integer);
return mpt::format_simple<Tstring>(x, format_simple_spec().BaseHex().CaseLow().FillNul().Width(width).Group(g).GroupSep(
        s));
}
template<int width, typename T>
static inline Tstring HEX0(unsigned int g, char s, const T &x) {
static_assert(std::numeric_limits<T>::is_integer);
return mpt::format_simple<Tstring>(x, format_simple_spec().BaseHex().CaseUpp().FillNul().Width(width).Group(g).GroupSep(
        s));
}
template<typename T>

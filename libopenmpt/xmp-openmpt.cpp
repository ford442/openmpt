...
self->mod->set_render_param(openmpt::module::RENDER_INTERPOLATIONFILTER_LENGTH,
                            self->settings.interpolationfilterlength);
...
static void WINAPI
openmpt_SetConfig(void *config, DWORD size) {
reset_options();
if(config) {
...
switch ( id ) {
case openmpt_shortcut_tempo_decrease: self->tempo_factor--;
tempo_changed = true;
break;
case openmpt_shortcut_tempo_increase: self->tempo_factor++;
tempo_changed = true;
break;
case openmpt_shortcut_pitch_decrease: self->pitch_factor--;
pitch_changed = true;
break;
case openmpt_shortcut_pitch_increase: self->pitch_factor++;
pitch_changed = true;
break;
...
self->
tempo_factor = std::min(48, std::max(-48, self->tempo_factor));
self->
pitch_factor = std::min(48, std::max(-48, self->pitch_factor));
...
openmpt::ext::interactive *interactive = static_cast<openmpt::ext::interactive *>( self->mod->get_interface(
        openmpt::ext::interactive_id));
interactive->
set_tempo_factor( tempo_factor );
interactive->
set_pitch_factor( pitch_factor );
xmpfin->SetLength( static_cast
<float>( self
->mod->
...
about << SHORT_TITLE << " version " << openmpt::string::get( "library_version" ) << " " << "(built " << openmpt::string::get( "build" ) << ")" <<
std::endl;
about << " Copyright (c) 2013-2021 OpenMPT Project Developers and Contributors (https://lib.openmpt.org/)" <<
std::endl;
about << " OpenMPT version " << openmpt::string::get( "core_version" ) <<
std::endl;
about <<
std::endl;
about << openmpt::string::get( "contact" ) <<
std::endl;
about <<
std::endl;
about << "Show full credits?" <<
std::endl;
if (
MessageBox( win, StringToWINAPI(StringDecode(about.str(), CP_UTF8))
.
c_str(), TEXT(SHORT_TITLE), MB_ICONINFORMATION
| MB_YESNOCANCEL | MB_DEFBUTTON1 ) != IDYES ) {
return;
}
std::ostringstream credits;
credits << openmpt::string::get( "credits" );
credits << "Additional thanks to:" <<
std::endl;
credits <<
std::endl;
credits << "Arseny Kapoulkine for pugixml" <<
std::endl;
credits << "https://pugixml.org/" <<
std::endl;
#if 1
libopenmpt::plugin::gui_show_file_info( win, TEXT(SHORT_TITLE), StringToWINAPI(
        StringReplace(StringDecode(credits.str(), CP_UTF8), L"\n", L"\r\n"))
);
#else
...
win ) {
#if 1
libopenmpt::plugin::gui_edit_settings( &self
->settings, win, TEXT(SHORT_TITLE));
#else
...
xmplay_imemstream(const char *base, size_t size)
        : xmplay_membuf(base, size), std::istream(static_cast<std::streambuf *>(this)) {
return;
}
};
...
if((remaining >= 6) && !MPT_NUMERIC(i[0]) && MPT_NUMERIC(i[1]) && MPT_NUMERIC(i[2]) && MPT_NUMERIC(i[3]) &&
   MPT_NUMERIC(i[4]) && !MPT_NUMERIC(i[5])) {
// Four-digit year
...
append_xmplay_tag(tags, "album",
                  convert_to_native(mod.get_metadata("xmplay-album"))); // todo, libopenmpt does not support that
append_xmplay_tag(tags, "date", convert_to_native(extract_date(mod)));
append_xmplay_tag(tags, "track",
                  convert_to_native(mod.get_metadata("xmplay-tracknumber"))); // todo, libopenmpt does not support that
append_xmplay_tag(tags, "genre",
                  convert_to_native(mod.get_metadata("xmplay-genre"))); // todo, libopenmpt does not support that
...
static BOOL WINAPI
openmpt_CheckFile(const char *filename, XMPFILE file) {
...
return (openmpt::probe_file_header(openmpt::probe_file_header_flags_default2, s) ==
        openmpt::probe_file_header_result_success) ? TRUE : FALSE;
}
break;
...
static DWORD WINAPI
openmpt_GetFileInfo(const char *filename, XMPFILE file, float **length, char **tags) {
static_cast<void>( filename );
try {
...
static DWORD WINAPI
openmpt_Open(const char *filename, XMPFILE file) {
static_cast<void>( filename );
...
static void WINAPI
openmpt_GetInfoText(char *format, char *length) {
if(!self->mod) {
...
static void WINAPI
openmpt_GetGeneralInfo(char *buf) {
if(!self->mod) {
...
str << "Format" << "\t" << sanitize_xmplay_info_string(self->mod->get_metadata("type")) << " ("
    << sanitize_xmplay_info_string(self->mod->get_metadata("type_long")) << ")" << "\r";
if(!self->mod->get_metadata("originaltype").empty()) {
str << "Original Type" << "\t" << sanitize_xmplay_info_string(self->mod->get_metadata("originaltype")) << " ("
    << sanitize_xmplay_info_string(self->mod->get_metadata("originaltype_long")) << ")" << "\r";
}
if(!self->mod->get_metadata("container").empty()) {
str << "Container" << "\t" << sanitize_xmplay_info_string(self->mod->get_metadata("container")) << " ("
    << sanitize_xmplay_info_string(self->mod->get_metadata("container_long")) << ")" << "\r";
}
str
...
for(std::size_t i = 0; i < self->subsong_lengths.size(); ++i) {
str << (i == 0 ? "Subsongs\t" : "\t") << (i + 1) << ". " << seconds_to_string(self->subsong_lengths[i]) << " "
    << names[i] << "\r";
}
}
str
...
static void WINAPI
openmpt_GetMessage(char *buf) {
if(!self->mod) {
clear_xmplay_string(buf);
return;
}
write_xmplay_string(buf, convert_to_native(
        sanitize_xmplay_multiline_string(string_replace(self->mod->get_metadata("message"), "\n", "\r"))));
}
// Seek to a position (in granularity units)
// return the new position in seconds (-1 = failed)
static double WINAPI
openmpt_SetPosition( DWORD
pos ) {
if ( !self->mod ) {
return -1.0;
}
if ( pos == static_cast<DWORD>(static_cast
<LONG>(XMPIN_POS_LOOP)
)) {
...
return -2;
} else if ( pos == static_cast<DWORD>(static_cast
<LONG>(XMPIN_POS_AUTOLOOP)
)) {
...
if (
pos &XMPIN_POS_SUBSONG
) {
self->
single_subsong_mode = (pos & XMPIN_POS_SUBSONG1) != 0;
try {
self->mod->
select_subsong( pos
& 0xffff );
...
xmpfin->SetLength( static_cast
<float> ( self
->subsong_lengths[pos & 0xffff] / interactive->
get_tempo_factor()
), TRUE );
reset_timeinfos( 0 );
return 0.0;
...
static DWORD WINAPI
openmpt_Process(float *dstbuf, DWORD count) {
xmpopenmpt_lock guard;
...
std::size_t frames_chunk = std::min(frames_to_render, static_cast<std::size_t>((self->samplerate + 99) /
                                                                               100 )); // 100 Hz timing info update interval
...
static void
add_names(std::ostream &str, const std::string &title, const std::vector <std::string> &names, int display_offset) {
if(names.size() > 0) {
bool valid = false;
...
str << title << " Names:" << "\r";
for(std::size_t i = 0; i < names.size(); i++) {
str << std::setfill('0') << std::setw(2) << (display_offset + i) << std::setw(0) << "\t" << convert_to_native(names[i])
    << "\r";
}
str << "\r";
}
...
static DWORD WINAPI
openmpt_GetSubSongs(float *length) {
*length = 0.0f;
...
switch (effect_type) {
case openmpt::ext::pattern_vis::effect_unknown:
return col_unknown;
break;
case openmpt::ext::pattern_vis::effect_general:
return col_text;
break;
case openmpt::ext::pattern_vis::effect_global :
return col_global;
break;
case openmpt::ext::pattern_vis::effect_volume :
return col_vol;
break;
case openmpt::ext::pattern_vis::effect_panning:
return col_instr;
break;
case openmpt::ext::pattern_vis::effect_pitch  :
return col_pitch;
break;
default:
return col_unknown;
...
visbitmap = 0;
visfont = 0;
viscolors[col_background].
dw = colors[0];
viscolors[col_unknown].
dw = colors[1];
viscolors[col_text].
dw = colors[2];
viscolors[col_global] =
invert_color( viscolors[col_background] );
const int r = viscolors[col_text].rgba.r, g = viscolors[col_text].rgba.g, b = viscolors[col_text].rgba.b;
viscolors[col_empty].rgba.
r = static_cast<std::uint8_t>((r + viscolors[col_background].rgba.r) / 2 );
viscolors[col_empty].rgba.
g = static_cast<std::uint8_t>((g + viscolors[col_background].rgba.g) / 2 );
viscolors[col_empty].rgba.
b = static_cast<std::uint8_t>((b + viscolors[col_background].rgba.b) / 2 );
viscolors[col_empty].rgba.
a = 0;
...
int pattern_width = ((chars_per_channel * channels + 4) * text_size) +
                    (spaces_per_channel * channels + channels - (num_cols == 1 ? 1 : 2)) * (text_size / 2);
...
if (
GetTextExtentPoint32( dc, TEXT("W"),
1, &text_size ) == FALSE ) {
return
FALSE;
}
...
for (
num_cols = sizeof(pattern_columns) / sizeof(pattern_columns[0]);
num_cols >= 1; num_cols-- ) {
chars_per_channel = 0;
...
i<num_cols;
i++ ) {
chars_per_channel += pattern_columns[i].
num_chars;
}
...
while (
get_pattern_width( chars_per_channel, spaces_per_channel, num_cols, text_size
.cx, channels ) > size.
cx &&chars_per_channel
> 1 ) {
chars_per_channel -= 1;
}
...
right = pattern_width;
rect.
bottom = pattern_height;
FillRect( visDC, &rect, visbrushs[col_background]
);
SetBkColor( visDC, viscolors[col_background]
.dw );
POINT pos;
pos.
y = 0;
...
ColorIndex color;
coldata()
        : is_empty(false), color(col_unknown) {
return;
}
};
...
case 0:
cols[col].
text = self->mod->format_pattern_row_channel_command(pattern, row, channel, openmpt::module::command_note);
break;
case 1:
cols[col].
text = self->mod->format_pattern_row_channel_command(pattern, row, channel, openmpt::module::command_instrument);
break;
case 2:
cols[col].
text = self->mod->format_pattern_row_channel_command(pattern, row, channel, openmpt::module::command_volumeffect)
       + self->mod->format_pattern_row_channel_command(pattern, row, channel, openmpt::module::command_volume);
break;
case 3:
cols[col].
text = self->mod->format_pattern_row_channel_command(pattern, row, channel, openmpt::module::command_effect)
...
== 2 || col == 3 )) {
if ( col == 2 ) {
color = effect_type_to_color_index(
        self->pattern_vis->get_pattern_row_channel_volume_effect_type(pattern, row, channel));
}
if ( col == 3 ) {
...
assure_width( cols[col]
.text, col_width );
SetTextColor( visDC, viscolors[cols[col].color]
.dw );
TextOutA( visDC, pos
.x, pos.y, cols[col].text.
c_str(), cols[col]
.text.
length()
);
...
left = offset_x;
rect.
top = (size.cy - text_size.cy) / 2;
rect.
right = rect.left + pattern_width;
rect.
bottom = rect.top + text_size.cy;
InvertRect( dc, &rect
);
...
const_cast<char *>(xmpin.exts));
}
xmpin.exts = nullptr;
DeleteCriticalSection(&xmpopenmpt_mutex);
}
static XMPIN *XMPIN_GetInterface_cxx(DWORD
face,
InterfaceProc faceproc
) {
if ( face != XMPIN_FACE ) return
nullptr;
...
static constexpr std::pair<DWORD, const char *>
shortcuts[] = {
{
...
#else
XMPIN *WINAPI
XMPIN_GetInterface( DWORD
face , InterfaceProc faceproc ) {
return XMPIN_GetInterface_cxx( face, faceproc ) ;
}

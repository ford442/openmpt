...
static void
UpdateEnvelopes(InstrumentEnvelope &mptEnv, const CModSpecifications &specs, std::bitset <wNumWarnings> &warnings) {
...
if(Reporting::Confirm(
        ...
        "Do you want to continue?", "Warning") != cnfYes)
return false;
BeginWaitCursor();
CriticalSection cs;
...
auto firstPat = std::find_if(m_SndFile.Order().cbegin(), m_SndFile.Order().cend(),
                             [this](PATTERNINDEX pat) { return m_SndFile.Patterns.IsValidPat(pat); });
bool firstPatValid = firstPat != m_SndFile.Order().cend();
bool lossy = false;
if(m_SndFile.m_nDefaultSpeed != 6) {
if(firstPatValid) {
m_SndFile.Patterns[*firstPat].WriteEffect(
        EffectWriter(CMD_SPEED, ModCommand::PARAM(m_SndFile.m_nDefaultSpeed)).RetryNextRow());
}
...
if(firstPatValid) {
m_SndFile.Patterns[*firstPat].WriteEffect(
        EffectWriter(CMD_TEMPO, ModCommand::PARAM(m_SndFile.m_nDefaultTempo.GetInt())).RetryNextRow());
}
m_SndFile.m_nDefaultTempo.Set(125);
lossy = true;
}
if(m_SndFile.m_nDefaultGlobalVolume != MAX_GLOBAL_VOLUME || m_SndFile.m_nSamplePreAmp != 48 ||
...
if(m_SndFile.Order.CanSplitSubsongs() && Reporting::Confirm(
...
if(oldTypeIsMPT && m_SndFile.GetMixLevels() != MixLevels::Compatible &&
   m_SndFile.GetMixLevels() != MixLevels::CompatibleFT2) {

...
CRawSampleDlg(FileReader &file, CWnd *parent = nullptr)
        : CDialog(IDD_LOADRAWSAMPLE, parent), m_file(file) {}
protected:
void DoDataExchange(CDataExchange *pDX) override;
BOOL OnInitDialog() override;
void OnOK() override;
...
public:
CSampleGridDlg(CWnd *parent, SmpLength nSegments, SmpLength nMaxSegments) : CDialog(IDD_SAMPLE_GRID_SIZE, parent) {
...
CSampleXFadeDlg(CWnd *parent, ModSample &sample)
        : CDialog(IDD_SAMPLE_XFADE, parent), m_sample(sample) {}
...
public:
CResamplingDlg(CWnd *parent, uint32 frequency, ResamplingMode srcMode, bool resampleAll) : CDialog(IDD_RESAMPLE,
                                                                                                   parent),
                                                                                           m_srcMode(srcMode),

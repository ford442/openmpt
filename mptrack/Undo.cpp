...
bool CPatternUndo::PrepareUndo(PATTERNINDEX pattern, CHANNELINDEX firstChn, ROWINDEX firstRow, CHANNELINDEX numChns,
                               ROWINDEX numRows, const char *description, bool linkToPrevious, bool storeChannelInfo) {
if(PrepareBuffer(UndoBuffer, pattern, firstChn, firstRow, numChns, numRows, description, linkToPrevious,
                 storeChannelInfo)) {
RedoBuffer.clear();
return true;
}
...
bool CPatternUndo::PrepareBuffer(undobuf_t &buffer, PATTERNINDEX pattern, CHANNELINDEX firstChn, ROWINDEX firstRow,
                                 CHANNELINDEX numChns, ROWINDEX numRows, const char *description, bool linkToPrevious,
                                 bool storeChannelInfo) const {
...
if(storeChannelInfo) {
undo.channelInfo.assign(std::begin(sndFile.ChnSettings) + firstChn,
                        std::begin(sndFile.ChnSettings) + firstChn + numChns);
}
...
PrepareBuffer(toBuf, undo.pattern, undo.firstChannel, undo.firstRow, undo.numChannels, undo.numRows, undo.description,
              linkedFromPrevious, !undo.channelInfo.empty());
...
const bool modifyChannelCount =
        ...
        || (undo.pattern == PATTERNINDEX_INVALID &&
            (undo.firstChannel + undo.channelInfo.size()) > sndFile.GetNumChannels());
...
bool CSampleUndo::PrepareUndo(const SAMPLEINDEX smp, sampleUndoTypes changeType, const char *description,
                              SmpLength changeStart, SmpLength changeEnd) {
...
bool CSampleUndo::PrepareBuffer(undobuf_t &buffer, const SAMPLEINDEX smp, sampleUndoTypes changeType,
                                const char *description, SmpLength changeStart, SmpLength changeEnd) {
...
int8 *
pNewSample = nullptr;    // a new sample is possibly going to be allocated, depending on what's going to be undone.
...
memcpy(pCurrentSample + undo.changeStart * bytesPerSample, pCurrentSample + undo.changeEnd * bytesPerSample,
       (sample.nLength - undo.changeEnd) * bytesPerSample);
// also clean the sample end
memset(pCurrentSample + undo.OldSample.nLength * bytesPerSample, 0,
       (sample.nLength - undo.OldSample.nLength) * bytesPerSample);
} else {
replace = true;
}
break;
...
memcpy(pNewSample + undo.changeEnd * bytesPerSample, pCurrentSample + undo.changeStart * bytesPerSample,
       (undo.OldSample.nLength - undo.changeEnd) * bytesPerSample);
break;
case sundo_replace:
...
capacity -= (buffer[smp - 1][i].changeEnd - buffer[smp - 1][i].changeStart) *
            buffer[smp - 1][i].OldSample.GetBytesPerSample();
...
bool CInstrumentUndo::PrepareBuffer(undobuf_t &buffer, const INSTRUMENTINDEX ins, const char *description,
...
CSoundFile &sndFile = modDoc.GetSoundFile();
if(sndFile.Instruments[ins] == nullptr || !InstrumentBufferExists(fromBuf, ins) || fromBuf[ins - 1].empty())
return false;
...
// newIndex contains one new index for each old index. newIndex[1] represents the first sample.
void
CInstrumentUndo::RearrangeSamples(undobuf_t &buffer, const INSTRUMENTINDEX ins, std::vector <SAMPLEINDEX> &newIndex) {
const CSoundFile &sndFile = modDoc.GetSoundFile();
if(sndFile.Instruments[ins] == nullptr || !InstrumentBufferExists(buffer, ins) || buffer[ins - 1].empty()) return;
for(auto &i : buffer[ins - 1])
if(i.editedEnvelope >= ENV_MAXTYPES) {

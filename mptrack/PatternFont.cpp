...
// Medium Font (Default)
static constexpr
PATTERNFONT gDefaultPatternFont =
        {
                nullptr,
                nullptr,
...
for(int y = 0, x =
        (customFontBitmap.bmiHeader.biHeight - pf.nClrY - pf.nHeight) * bmWidth2 + (pf.nClrX + pf.nWidth - 4) / 2;
    y < pf.nHeight; y++, x += bmWidth2) {
...
CFont gdiFont;
gdiFont.CreateFont(font.size, 0, 0, 0, font.flags[FontSetting::Bold] ? FW_BOLD : FW_NORMAL,
                   font.flags[FontSetting::Italic] ? TRUE : FALSE, FALSE, FALSE, DEFAULT_CHARSET, OUT_RASTER_PRECIS,
                   CLIP_DEFAULT_PRECIS, NONANTIALIASED_QUALITY, FIXED_PITCH | FF_DONTCARE, mpt::ToCString(font.name));
...
uint8 * data = nullptr;
HBITMAP bitmap = ::CreateDIBSection(hDC, (BITMAPINFO * ) & pf.dib->bmiHeader, DIB_RGB_COLORS, (void **) &data, nullptr,
                                    0);
if(!bitmap) {
...
uint8 * data = nullptr;
HBITMAP bitmap = ::CreateDIBSection(hDC, (BITMAPINFO * ) & pf.dibASCII->bmiHeader, DIB_RGB_COLORS, (void **) &data,
                                    nullptr, 0);
if(!bitmap) {
hDC.SelectObject(oldFont);
gdiFont.DeleteObject();

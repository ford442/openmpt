...
InsertItem(TVIF_TEXT | TVIF_IMAGE | TVIF_SELECTEDIMAGE | TVIF_PARAM,
           mpt::ToCString(mpt::Charset::ASCII, szMidiGroupNames[iMidGrp]), IMAGE_FOLDER, IMAGE_FOLDER, 0, 0,
           (MODITEM_HDR_MIDIGROUP << MIDILIB_SHIFT) | iMidGrp, m_hMidiLib, TVI_LAST);
}
}
...
m_tiDLS[iDls] = InsertItem(TVIF_TEXT | TVIF_IMAGE | TVIF_SELECTEDIMAGE | TVIF_PARAM,
                           pDlsBank->GetFileName().GetFullFileName().AsNative().c_str(), IMAGE_FOLDER, IMAGE_FOLDER, 0,
                           0, iDls, TVI_ROOT, hDlsRoot);
// Memorize Banks
...
LPARAM
lParam = DlsItem::EncodeValuePerc((uint8)(iRgn), (uint16)
iIns);
InsertItem(TVIF_TEXT | TVIF_IMAGE | TVIF_SELECTEDIMAGE | TVIF_PARAM,
...
LPARAM
lParam = DlsItem::EncodeValueInstr((pDlsIns->ulInstrument & 0x7F), (uint16)
iIns);
InsertItem(TVIF_TEXT | TVIF_IMAGE | TVIF_SELECTEDIMAGE | TVIF_PARAM,
...
if((IsSampleBrowser() || GetParentRootItem(GetSelectedItem()) == m_hInsLib)
   && GetItemText(GetSampleBrowser()->m_hInsLib) ==
      (m_SongFileName.empty() ? m_InstrLibPath : m_SongFileName).ToCString()) {
...
if(!info.hSong) {
info.hSong = InsertItem(TVIF_TEXT | TVIF_IMAGE | TVIF_SELECTEDIMAGE | TVIF_PARAM, name, IMAGE_FOLDER, IMAGE_FOLDER, 0,
                        0, reinterpret_cast<LPARAM>(&info.modDoc), TVI_ROOT, TVI_FIRST);
...
if(!info.hEffects) {
info.hEffects = InsertItem(_T("Plugins"), IMAGE_FOLDER, IMAGE_FOLDER, info.hSong,
                           info.hInstruments ? info.hInstruments : info.hSamples);
}
...
InsertItem(TVIF_TEXT | TVIF_HANDLE | TVIF_IMAGE | TVIF_SELECTEDIMAGE | TVIF_PARAM, s, nImage, nImage, 0, 0, i,
           info.hEffects, TVI_LAST);
}
hasPlugs = true;
}
}
...
if(info.hOrders &&
   (seqHint.GetType()[HINT_MODTYPE | HINT_MODSEQUENCE | HINT_SEQNAMES] || patternHint.GetType()[HINT_PATNAMES])) {
...
if((info.tiSequences.size() > 1 && sndFile.Order.GetNumSequences() == 1) ||
   (info.tiSequences.size() == 1 && sndFile.Order.GetNumSequences() > 1)) {
...
for(size_t seq = sndFile.Order.GetNumSequences(); seq < info.tiSequences.size(); seq++)
if(info.tiSequences[seq]) {
for(auto &ord : info.tiOrders[seq])
if(ord) {
DeleteItem(ord);
ord = nullptr;
}
DeleteItem(info.tiSequences[seq]);
info.tiSequences[seq] = nullptr;
}
...
for(size_t nOrd = ordLength; nOrd < info.tiOrders[seq].size(); nOrd++)
if(info.tiOrders[seq][nOrd]) {
DeleteItem(info.tiOrders[seq][nOrd]);
info.tiOrders[seq][nOrd] = NULL;
}
...
if(!patName.IsEmpty()) {
wsprintf(s,
         (TrackerSettings::Instance().m_dwPatternSetup & PATTERN_HEXDISPLAY) ? _T("[%02Xh] %u: ") : _T("[%02u] %u: "),
         iOrd, sndFile.Order(seq)[iOrd]);
_tcscat(s, patName.GetString());
} else {
wsprintf(s, (TrackerSettings::Instance().m_dwPatternSetup & PATTERN_HEXDISPLAY) ? _T("[%02Xh] Pattern %u") : _T(
        "[%02u] Pattern %u"),
         iOrd, sndFile.Order(seq)[iOrd]);
}
} else {
...
info.tiOrders[seq][iOrd] = InsertItem(TVIF_HANDLE | TVIF_TEXT | TVIF_IMAGE | TVIF_SELECTEDIMAGE | TVIF_PARAM, s,
                                      IMAGE_PARTITION, IMAGE_PARTITION, 0, 0, param, hAncestorNode, TVI_LAST);
}
}
}
}
...
wsprintf(s, _T("%3d: %s%s%s"), nSmp, sample.uFlags.test_all(SMP_MODIFIED | SMP_KEEPONDISK) ? _T("* ") : _T(""),
         mpt::ToCString(sndFile.GetCharsetInternal(), sndFile.m_szNames[nSmp]).GetString(), status);
} else {
wsprintf(s, _T("%3d: %s"), nSmp, mpt::ToCString(sndFile.GetCharsetInternal(), sndFile.m_szNames[nSmp]).GetString());
}
if(!hChild) {
hChild = InsertItem(TVIF_TEXT | TVIF_IMAGE | TVIF_SELECTEDIMAGE | TVIF_PARAM, s, image, image, 0, 0, nSmp,
                    info.hSamples, TVI_LAST);
} else {
...
wsprintf(s, _T("%3u: %s"), nIns,
         mpt::ToCString(sndFile.GetCharsetInternal(), sndFile.GetInstrumentName(nIns)).GetString());
...
if(!hChild) {
hChild = InsertItem(TVIF_TEXT | TVIF_IMAGE | TVIF_SELECTEDIMAGE | TVIF_PARAM, s, nImage, nImage, 0, 0, nIns,
                    info.hInstruments, TVI_LAST);
} else {
tvi.mask = TVIF_TEXT | TVIF_HANDLE | TVIF_IMAGE;
tvi.hItem = hChild;
tvi.pszText = stmp;
...
return ModItem(MODITEM_DLSBANK_FOLDER, (uint32)
rootItemData);
...
if(Reporting::Confirm(TreeDeletionString(UL_("sequence"), seq + 1, sndFile->Order(seq).GetName()), false, true) ==
   cnfNo)
break;
...
mpt::ustring s = TreeDeletionString(UL_("pattern"), modItemID,
                                    mpt::ToUnicode(sndFile->GetCharsetInternal(), sndFile->Patterns[pat].GetName()));
...
if(!sndFile->GetSample(static_cast<SAMPLEINDEX>(modItemID)).HasSampleData()
   || Reporting::Confirm(TreeDeletionString(UL_("sample"), modItemID, mpt::ToUnicode(sndFile->GetCharsetInternal(),
                                                                                     sndFile->m_szNames[modItemID])),
                         false, true) == cnfYes) {
...
if(sndFile->Instruments[modItemID] == nullptr
   || Reporting::Confirm(TreeDeletionString(UL_("instrument"), modItemID, mpt::ToUnicode(sndFile->GetCharsetInternal(),
                                                                                         sndFile->Instruments[modItemID]->name)),
                         false, true) == cnfYes) {
...
modDoc->UpdateAllViews(nullptr, InstrumentHint(
        modDoc->GetNumInstruments() != oldNumInstrs ? 0 : ins).Info().Envelope().ModType());
}
}
break;
case MODITEM_MIDIINSTRUMENT:
...
FileDialog dlg = OpenFileDialog()
        .EnableAudioPreview()
        .ExtensionFilter(
                "All Instruments and Banks|*.xi;*.pat;*.iti;*.sfz;*.wav;*.w64;*.caf;*.aif;*.aiff;*.sf2;*.sbk;*.dls;*.mss;*.flac;*.opus;*.ogg;*.oga;*.mp1;*.mp2;*.mp3" +
                ToFilterOnlyString(mediaFoundationTypes, true).ToLocale() + "|"
...
_sntprintf(s, std::size(s), _T("%3d: %s"), smp,
           mpt::ToWin(m_SongFile->GetCharsetInternal(), m_SongFile->m_szNames[smp]).c_str());
...
TCHAR sDevice[] = _T("\\\\.\\?:");
sDevice[4] = s[0];
auto hDevice = CreateFile(sDevice, FILE_READ_ATTRIBUTES, FILE_SHARE_READ | FILE_SHARE_WRITE, nullptr, OPEN_EXISTING, 0,
                          nullptr);
...
SHGetFileInfo(s, 0, &fileInfo, sizeof(fileInfo), SHGFI_ICON | SHGFI_SMALLICON |
                                                 SHGFI_USEFILEATTRIBUTES);  // SHGFI_USEFILEATTRIBUTES speeds up retrieval for offline network shares
...
static constexpr auto sampleExts = {"wav", "flac", "ogg", "opus", "mp1", "mp2", "mp3", "smp", "raw", "s3i", "its",
                                    "aif", "aiff", "au", "snd", "svx", "voc", "8sv", "8svx", "16sv", "16svx", "w64",
                                    "caf", "sb0", "sb2", "sbi"};
static constexpr auto allExtsBlacklist = {"txt", "diz", "nfo", "doc", "ini", "pdf", "zip", "rar", "lha", "exe", "dll",
                                          "lnk", "url"};
...
if(auto cr = RegOpenKeyEx(HKEY_CURRENT_USER, _T("Software\\Microsoft\\Windows\\CurrentVersion\\Explorer\\Advanced"), 0,
                          KEY_READ, &hkey); cr == ERROR_SUCCESS) {
DWORD dataType = REG_NONE;
DWORD data = 0, datasize = sizeof(data);
if(RegQueryValueEx(hkey, _T("Hidden"), 0, &dataType, reinterpret_cast<BYTE *>(&data), &datasize) == ERROR_SUCCESS &&
   dataType == REG_DWORD)
showHidden = (data == 1);
RegCloseKey(hkey);
}
...
if(!m_WatchDir.empty()) {
hWatchDir = FindFirstChangeNotification(m_WatchDir.AsNative().c_str(), FALSE,
...
int CALLBACK
CModTree::ModTreeInsLibCompareNamesProc(LPARAM
lParam1,
...
sortOrderR;
return
::CompareString(LOCALE_USER_DEFAULT, that
->m_stringCompareFlags,
that->m_compareStrL.
GetBuffer(),
-1,
that->m_compareStrR.
...
int CALLBACK
CModTree::ModTreeInsLibCompareProc(LPARAM
lParam1,
LPARAM lParam2, LPARAM
)
{
...
int CALLBACK
CModTree::ModTreeDrumCompareProc(LPARAM
lParam1,
LPARAM lParam2, LPARAM
pDLSBank)
{
...
m_InstrLibHighlightPath = mpt::PathString::FromNative(
        prevDir.substr(pos + 1, prevDir.length() - pos - 2));  // Highlight previously accessed directory
...
if((m_itemDrag.type == MODITEM_SEQUENCE || m_itemDrag.type == MODITEM_HDR_ORDERS) && modDoc && sndFile && infoDrag &&
   !sameModDoc) {
...
sndFile->Order().resize(std::min(sndFile->GetModSpecifications().ordersMax, origSeq.GetLength()),
                        sndFile->Order.GetInvalidPatIndex());
for(ORDERINDEX nOrd = 0;
    nOrd < std::min(sndFile->GetModSpecifications().ordersMax, origSeq.GetLengthTailTrimmed()); nOrd++) {
...
sndFile->ReadSampleFromSong(static_cast<SAMPLEINDEX>(modItemDropID), infoDrag->modDoc.GetSoundFile(),
                            static_cast<SAMPLEINDEX>(modItemDragID));
}
...
if(sameItem)
return true;
const INSTRUMENTINDEX from = static_cast<INSTRUMENTINDEX>(modItemDragID - 1), to = static_cast<INSTRUMENTINDEX>(
        modItemDropID - 1);
...
sndFile->ReadInstrumentFromSong(static_cast<INSTRUMENTINDEX>(modItemDropID), infoDrag->modDoc.GetSoundFile(),
                                static_cast<INSTRUMENTINDEX>(modItemDragID));
}
...
AppendMenu(hMenu, MF_STRING | (sndFile->Order.GetNumSequences() < MAX_SEQUENCES ? 0 : MF_GRAYED), ID_MODTREE_INSERT,
           _T("&Insert Sequence"));
AppendMenu(hMenu, MF_STRING | (sndFile->Order.GetNumSequences() < MAX_SEQUENCES ? 0 : MF_GRAYED), ID_MODTREE_DUPLICATE,
           _T("D&uplicate Sequence"));
...
AppendMenu(hMenu, (modDoc->IsSampleMuted((SAMPLEINDEX) modItemID) ? MF_CHECKED : 0) | MF_STRING, ID_MODTREE_MUTE,
           _T("&Mute Sample"));
...
AppendMenu(hMenu, MF_SEPARATOR, NULL, _T(""));
if(menuForThisSample)
AppendMenu(hMenu, MF_STRING | ((sndFile->GetType() == MOD_TYPE_MPT || hasPath) ? 0 : MF_GRAYED), ID_MODTREE_SETPATH,
           _T("Set P&ath"));
if(menuForThisSample)
AppendMenu(hMenu, MF_STRING | ((hasPath && sample.HasSampleData() && sample.uFlags[SMP_MODIFIED]) ? 0 : MF_GRAYED),
           ID_MODTREE_SAVEITEM, _T("&Save"));
...
if(modDoc) {
AppendMenu(hMenu, MF_SEPARATOR, NULL, _T(""));
AppendMenu(hMenu, (modDoc->IsInstrumentMuted((INSTRUMENTINDEX) modItemID) ? MF_CHECKED : 0) | MF_STRING,
           ID_MODTREE_MUTE, _T("&Mute Instrument"));
...
if(modDoc != nullptr) {
AppendMenu(hMenu, (modDoc->GetSoundFile().m_MixPlugins[modItemID].IsBypassed() ? MF_CHECKED : 0) | MF_STRING,
           ID_MODTREE_MUTE, _T("&Bypass"));
}
}
break;
case MODITEM_MIDIINSTRUMENT:
...
AppendMenu(hMenu, TrackerSettings::Instance().showDirsInSampleBrowser ? (MF_STRING | MF_CHECKED) : MF_STRING,
           ID_MODTREE_SHOWDIRS, _T("Show &Directories in Sample Browser"));
AppendMenu(hMenu, (m_showAllFiles) ? (MF_STRING | MF_CHECKED) : MF_STRING, ID_MODTREE_SHOWALLFILES,
           _T("Show &All Files"));
AppendMenu(hMenu, (m_showAllFiles) ? MF_STRING : (MF_STRING | MF_CHECKED), ID_MODTREE_SOUNDFILESONLY,
           _T("Show &Sound Files"));
bSep = TRUE;
}
if((bSep) || (nDefault)) AppendMenu(hMenu, MF_SEPARATOR, NULL, _T(""));
...
const SEQUENCEINDEX newIndex = (modItem.type == MODITEM_HDR_ORDERS) ? sndFile.Order.GetNumSequences()
                                                                    : static_cast<SEQUENCEINDEX>(modItemID + 1);
std::vector <SEQUENCEINDEX> newOrder = GenerateInsertVector<SEQUENCEINDEX>(sndFile.Order.GetNumSequences(), newIndex,
                                                                           static_cast<SEQUENCEINDEX>(insert
...
std::vector <SAMPLEINDEX> newOrder = GenerateInsertVector<SAMPLEINDEX>(sndFile.GetNumSamples(), modItemID,
                                                                       static_cast<SAMPLEINDEX>(insert ? 0 : modItemID),
                                                                       1);
...
std::vector <INSTRUMENTINDEX> newOrder = GenerateInsertVector<INSTRUMENTINDEX>(sndFile.GetNumInstruments(), modItemID,
                                                                               static_cast<INSTRUMENTINDEX>(insert ? 0
                                                                                                                   : modItemID),
                                                                               1);
...
FileDialog dlg = OpenFileDialog()
        .ExtensionFilter(
                "All Samples|*.wav;*.flac|All files(*.*)|*.*||");    // Only show samples that we actually can save as well.
if(path.empty())
...
if(start || stop) {
note = static_cast<ModCommand::NOTE>(wParam - (start ? kcTreeViewStartNotes : kcTreeViewStartNoteStops) + 1 +
...
case MODITEM_SEQUENCE:
if(modItem.val1 < sndFile.Order.GetNumSequences() &&
   sndFile.Order(static_cast<SEQUENCEINDEX>(modItem.val1)).GetName() != itemText) {
...
case MODITEM_PATTERN:
if(modItem.val1 < sndFile.Patterns.GetNumPatterns() && modSpecs.hasPatternNames &&
   mpt::ToUnicode(sndFile.GetCharsetInternal(), sndFile.Patterns[modItem.val1].GetName()) != itemText) {
...
case MODITEM_SAMPLE:
if(modItem.val1 <= sndFile.GetNumSamples() &&
   mpt::ToUnicode(sndFile.GetCharsetInternal(), sndFile.m_szNames[modItem.val1]) != itemText) {
sndFile.m_szNames[modItem.val1] = mpt::truncate(mpt::ToCharset(sndFile.GetCharsetInternal(), itemText),
                                                modSpecs.sampleNameLengthMax);
modDoc->SetModified();
...
case MODITEM_INSTRUMENT:
if(modItem.val1 <= sndFile.GetNumInstruments() && sndFile.Instruments[modItem.val1] != nullptr &&
   mpt::ToUnicode(sndFile.GetCharsetInternal(), sndFile.Instruments[modItem.val1]->name) != itemText) {
sndFile.Instruments[modItem.val1]->name = mpt::truncate(mpt::ToCharset(sndFile.GetCharsetInternal(), itemText),
                                                        modSpecs.instrNameLengthMax);
modDoc->SetModified();

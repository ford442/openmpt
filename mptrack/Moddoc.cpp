...
const TCHAR
FileFilterMOD[]    = _T("ProTracker Modules (*.mod)|*.mod||");
...
switch (modtype) {
case MOD_TYPE_MOD:
return FileFilterMOD;
case MOD_TYPE_XM:
return FileFilterXM;
case MOD_TYPE_S3M:
return FileFilterS3M;
case MOD_TYPE_IT:
return FileFilterIT;
case MOD_TYPE_MPT:
return FileFilterMPT;
default:
return FileFilterNone;
}
}
...
CModDoc::CModDoc()
        : m_notifyType(Notification::Default), m_PatternUndo(*this), m_SampleUndo(*this), m_InstrumentUndo(*this) {
...
SetPathName(filename,
            FALSE);    // Path is not set yet, but loaders processing external samples/instruments (ITP/MPTM) need this for relative paths.
try {
...
return FALSE;
const bool noColors =
        std::find_if(std::begin(m_SndFile.ChnSettings), std::begin(m_SndFile.ChnSettings) + GetNumChannels(),
                     [](const auto &settings) {
...
Reporting::Notification(MPT_UFORMAT(
                                "Warning: this song was last saved with a more recent version of OpenMPT.\r\nSong saved with: v{}. Current version: v{}.\r\n")(
...
switch (m_SndFile.GetType()) {
case MOD_TYPE_MOD:
ok = m_SndFile.SaveMod(f);
break;
case MOD_TYPE_S3M:
ok = m_SndFile.SaveS3M(f);
break;
case MOD_TYPE_XM:
ok = m_SndFile.SaveXM(f);
break;
case MOD_TYPE_IT:
ok = m_SndFile.SaveIT(f, filename);
break;
case MOD_TYPE_MPT:
ok = m_SndFile.SaveIT(f, filename);
break;
...
CMainFrame::GetMainFrame()->UpdateTree(this,
                                       GeneralHint().General()); // Update treeview (e.g. filename might have changed)
return TRUE;
} else {
return FALSE;
}
...
const double hueFactor = rainbow ? (1.5 * mpt::numbers::pi) / std::max(1, numGroups - 1)
                                 : 1000.0;  // Three quarters of the color wheel, red to purple
for(CHANNELINDEX i = 0, group = 0; i < GetNumChannels(); i++) {
if(i > 0 &&
   (m_SndFile.ChnSettings[i].szName.empty() || m_SndFile.ChnSettings[i].szName != m_SndFile.ChnSettings[i - 1].szName))
group++;
...
const auto color = RGB(mpt::saturate_round<uint8>(r * 255), mpt::saturate_round<uint8>(g * 255),
                       mpt::saturate_round<uint8>(b * 255));
...
void CModDoc::AddToLog(LogLevel
level,
const mpt::ustring &text
) const
{
if(m_LogMode == LogModeGather)
{
m_Log.
push_back(LogEntry(level, text)
);
} else
{
if(level<LogDebug)
...
void CModDoc::ProcessMIDI(uint32
midiData,
INSTRUMENTINDEX ins, IMixPlugin
*plugin,
...
if(ih->
HandleMIDIMessage(ctx, midiData
) != kcNull
|| ih->
HandleMIDIMessage(kCtxAllContexts, midiData
) != kcNull)
{
...
NoteOff(note,
false, ins, m_noteChannel[note - NOTE_MIN]);
return;
...
PlayNote(PlayNoteParam(note)
.
Instrument(ins)
.
Volume(vol), &m_noteChannel
);
return;
} else if(plugin != nullptr)
{
...
.m_dwMidiSetup & MIDISETUP_MIDITOPLUG) && CMainFrame::GetMainFrame()->GetModPlaying() == this && plugin != nullptr)
{
plugin->
MidiSend(midiData);
// Sending midi may modify the plug. For now, if MIDI data is not active sensing or aftertouch messages, set modified.
if(midiData !=
MIDIEvents::System(MIDIEvents::sysActiveSense)
&& event !=
MIDIEvents::evPolyAftertouch &&event
!=
MIDIEvents::evChannelAftertouch
        &&event
!=
MIDIEvents::evPitchBend
        &&m_SndFile
.
...
chn.dwFlags.set(CHN_EXTRALOUD,
                !(TrackerSettings::Instance().m_dwPatternSetup & PATTERN_NOEXTRALOUD) && params.m_sample);
...
if(ins != INSTRUMENTINDEX_INVALID && ins <= m_SndFile.GetNumInstruments() &&
   ModCommand::IsNote(ModCommand::NOTE(note))) {
...
void CModDoc::OnFileWaveConvert(ORDERINDEX nMinOrder, ORDERINDEX nMaxOrder,
                                const std::vector<EncoderFactoryBase *> &encFactories) {
ASSERT(!encFactories.empty());
CMainFrame *pMainFrm = CMainFrame::GetMainFrame();
if((!pMainFrm) || (!m_SndFile.GetType()) || encFactories.empty()) return;
CWaveConvert wsdlg(pMainFrm, nMinOrder, nMaxOrder, m_SndFile.Order().GetLengthTailTrimmed() - 1, m_SndFile,
                   encFactories);
{
...
FileDialog dlg = SaveFileDialog()
        ...
        .ExtensionFilter(encFactory->GetTraits().fileDescription + U_(" (*.") + extension.ToUnicode() + U_(")|*.") +
                         extension.ToUnicode() + U_("||"))
...
fileNameAdd += MPT_UFORMAT("-{}_{}")(mpt::ufmt::dec0<3>(i + 1),
                                     mpt::ToUnicode(m_SndFile.GetCharsetInternal(), m_SndFile.ChnSettings[i].szName));
...
if(!m_SndFile.GetSample(static_cast<SAMPLEINDEX>(i + 1)).HasSampleData() ||
   !IsSampleUsed(static_cast<SAMPLEINDEX>(i + 1), false) || instrMuteState[i])
continue;

// Add sample number & name (if available) to path string
if(!m_SndFile.m_szNames[i + 1].empty()) {
fileNameAdd += MPT_UFORMAT("-{}_{}")(mpt::ufmt::dec0<3>(i + 1),
                                     mpt::ToUnicode(m_SndFile.GetCharsetInternal(), m_SndFile.m_szNames[i + 1]));
...
if(m_SndFile.Instruments[i + 1] == nullptr || !IsInstrumentUsed(static_cast<SAMPLEINDEX>(i + 1), false) ||
   instrMuteState[i])
continue;
if(!m_SndFile.Instruments[i + 1]->name.empty()) {
fileNameAdd += MPT_UFORMAT("-{}_{}")(mpt::ufmt::dec0<3>(i + 1), mpt::ToUnicode(m_SndFile.GetCharsetInternal(),
                                                                               m_SndFile.Instruments[i + 1]->name));
caption = MPT_CFORMAT("{}:{}")(i + 1,
                               mpt::ToCString(m_SndFile.GetCharsetInternal(), m_SndFile.Instruments[i + 1]->name));
} else {
fileNameAdd += MPT_UFORMAT("-{}")(mpt::ufmt::dec0<3>(i + 1));
...
if(HANDLE hFile = ::CreateFile(thisName.AsNative().c_str(), GENERIC_WRITE, FILE_SHARE_READ, NULL, OPEN_ALWAYS,
                               FILE_ATTRIBUTE_TEMPORARY, NULL); hFile != INVALID_HANDLE_VALUE) {
::CloseHandle(hFile);
}
}
...
bool cancel = true;
try {
mpt::SafeOutputFile safeFileStream(thisName, std::ios::binary,
                                   mpt::FlushModeFromBool(TrackerSettings::Instance().MiscFlushFileBuffersOnSave));
...
try {
mpt::SafeOutputFile sf(dlg.GetFirstFile(), std::ios::binary,
                       mpt::FlushModeFromBool(TrackerSettings::Instance().MiscFlushFileBuffersOnSave));
mpt::ofstream &f = sf;
f.exceptions(f.exceptions() | std::ios::badbit | std::ios::failbit);
if(!f.good()) {
...
switch (type) {
case MOD_TYPE_XM:
ok = m_SndFile.SaveXM(f, true);
break;
case MOD_TYPE_IT:
ok = m_SndFile.SaveIT(f, filename, true);
break;
default:
MPT_ASSERT_NOTREACHED();
}
...
// Kill editor voices
for(CHANNELINDEX i = m_SndFile.GetNumChannels(); i < MAX_CHANNELS; i++)
if(m_SndFile.m_PlayState.Chn[i].isPreviewNote) {
...
CriticalSection cs;
if((m_SndFile.m_PlayState.m_nCurrentOrder < m_SndFile.Order().GetLength()) &&
   (m_SndFile.Order()[m_SndFile.m_PlayState.m_nCurrentOrder] == nPat)) {
...
s.Format(
        _T("Using alternative tempo interpretation.\n\nAssuming:\n. %.8g ticks per second\n. %u ticks per row\n. %u rows per beat\nthe tempo is approximately: %.8g BPM"),
        m_SndFile.m_PlayState.m_nMusicTempo.ToDouble(), m_SndFile.m_PlayState.m_nMusicSpeed,
        m_SndFile.m_PlayState.m_nCurrentRowsPerBeat, bpm);
break;
...
default:
s.Format(
        _T("Using standard tempo interpretation.\n\nAssuming:\n. A mod tempo (tick duration factor) of %.8g\n. %u ticks per row\n. %u rows per beat\nthe tempo is approximately: %.8g BPM"),
        m_SndFile.m_PlayState.m_nMusicTempo.ToDouble(), m_SndFile.m_PlayState.m_nMusicSpeed,
        m_SndFile.m_PlayState.m_nCurrentRowsPerBeat, bpm);
break;
}
...
OnViewTempoSwingSettings();
break;
case kcShowMacroConfig:
OnSetupZxxMacros();
break;
...
OnFileCompatibilitySave();
break;
case kcEstimateSongLength:
OnEstimateSongLength();
break;
case kcApproxRealBPM:
OnApproximateBPM();
break;
case kcFileSave:
DoSave(GetPathNameMpt());
break;
case kcFileSaveAs:
DoSave(mpt::PathString());
break;
case kcFileSaveCopy:
OnSaveCopy();
break;
case kcFileSaveTemplate:
OnSaveTemplateModule();
break;
case kcFileClose:
SafeFileClose();
break;
...
SetLoopSong(!TrackerSettings::Instance().gbLoopSong);
break;
...
m_SndFile.m_PlayState.m_nMusicTempo = std::min(modSpecs.GetTempoMax(),
                                               tempo + TEMPO(wParam == kcTempoIncrease ? 1.0 : 0.1));
break;
...
m_SndFile.m_PlayState.m_nMusicTempo = std::max(modSpecs.GetTempoMin(),
                                               tempo - TEMPO(wParam == kcTempoDecrease ? 1.0 : 0.1));
break;
...
CString message;
message.Format(_T("Parameter %i beyond controllable range. Use Parameter Control Events to automate this parameter."),
               static_cast<int>(paramToUse));
...
CHANNELINDEX newChannels = Clamp(dlg.m_nChannels, m_SndFile.GetModSpecifications().channelsMin,
                                 m_SndFile.GetModSpecifications().channelsMax);
...
double t = m_SndFile.GetPlaybackTimeAt(nOrd, nRow, true, setSamplePos && (TrackerSettings::Instance().m_dwPatternSetup &
                                                                          PATTERN_SYNCSAMPLEPOS) != 0);
if(t < 0) {
...
if(m_SndFile.Patterns.IsValidPat(pat)) {
GetPatternUndo().PrepareUndo(pat, 0, 0, GetNumChannels(), m_SndFile.Patterns[pat].GetNumRows(), description, linkUndo,
                             storeChannelInfo && pat == lastPat);
linkUndo = true;
}
}
}
...
return _T("Center");
CString s;
s.Format(_T("%i%% %s"), (std::abs(static_cast<int>(value) - valueAtCenter) * 100) / valueAtCenter,
         value < valueAtCenter ? _T("Left") : _T("Right"));
return s;
}
...
const mpt::PathString pathName = theApp.IsPortableMode() ? GetPathNameMpt().AbsolutePathToRelative(
        theApp.GetInstallPath()) : GetPathNameMpt();
if(pathName.empty()) {
return;
}
...
for(PLUGINDEX i = 0; i < MAX_MIXPLUGINS; i++) {
if(m_SndFile.m_MixPlugins[i].IsValidPlugin() && m_SndFile.m_MixPlugins[i].editorX != int32_min && cxScreen &&
   cyScreen) {

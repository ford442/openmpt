...
CMIDIMappingDialog::CMIDIMappingDialog(CWnd *pParent, CSoundFile &rSndfile)
        : CDialog(IDD_MIDIPARAMCONTROL, pParent), m_sndFile(rSndfile), m_rMIDIMapper(m_sndFile.GetMIDIMapper()) {
...
IDC_LIST1, &CMIDIMappingDialog::OnSelectionChanged)
...
static constexpr CListCtrlEx::Header
headers[] =
        {
...
if(m.GetController() <= MIDIEvents::MIDICC_end)
s += mpt::ToCString(mpt::Charset::UTF8, MIDIEvents::MidiCCNames[m.GetController()]);
break;
case MIDIEvents::evPolyAftertouch:
s = _T("Polyphonic Aftertouch");
break;
case MIDIEvents::evChannelAftertouch:
s = _T("Channel Aftertouch");
break;
default:
s.Format(_T("0x%02X"), m.GetEvent());
break;
}

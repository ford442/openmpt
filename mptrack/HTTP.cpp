...
uri.
scheme = str.substr(0, scheme_delim_pos);
...
uri.
username = userinfo.substr(0, username_delim_pos);
if(username_delim_pos != mpt::ustring::npos)
{
uri.
password = userinfo.substr(username_delim_pos + 1);
}
}
...
if(beg_bracket_pos !=
mpt::ustring::npos &&end_bracket_pos
!= mpt::ustring::npos)
{
if(port_delim_pos !=
mpt::ustring::npos &&port_delim_pos
> end_bracket_pos)
{
uri.
host = authority.substr(0, port_delim_pos);
uri.
port = authority.substr(port_delim_pos + 1);
} else
{
uri.
host = authority;
...
uri.
path = str.substr(0, path_delim_pos);
...
{
uri.
query = str.substr(1, query_delim_pos - 1);
uri.
fragment = str.substr(query_delim_pos + 1);
} else
{
uri.
fragment = str.substr(query_delim_pos + 1);
...
if(HttpQueryInfo(NativeHandle(request), HTTP_QUERY_STATUS_CODE | HTTP_QUERY_FLAG_NUMBER, &statusCode, &length, NULL) ==
   FALSE) {
throw HTTP::LastErrorException();
}
...
if(HttpQueryInfo(NativeHandle(request), HTTP_QUERY_CONTENT_LENGTH | HTTP_QUERY_FLAG_NUMBER, &contentLength, &length,
                 NULL) == FALSE) {

...
noexcept
        : b(0), g(
0), r(0), a(0) {}
constexpr Pixel(uint8 r, uint8 g, uint8 b, uint8 a)
noexcept
        : b(b), g(g), r(r), a(a) {}
constexpr Pixel(COLORREF color)
noexcept
        : b(GetBValue(color)), g(GetGValue(color)), r(GetRValue(color)), a(
0) {}
};
private:
uint32 width;
...
constexpr uint32
Width() const
noexcept { return width; }
constexpr uint32
Height() const
noexcept { return height; }
MPT_FORCEINLINE Pixel
&
operator()(uint32 x, uint32 y)
noexcept { return pixels[y * width + x]; }
MPT_FORCEINLINE const Pixel
&
operator()(uint32 x, uint32 y) const
noexcept { return pixels[y * width + x]; }
...
std::unique_ptr <Gdiplus::Image>
ResizeImage(Gdiplus::Image &src, double scaling, int spriteWidth = 0, int spriteHeight = 0);
std::unique_ptr <Gdiplus::Bitmap>
ResizeImage(Gdiplus::Bitmap &src, double scaling, int spriteWidth = 0, int spriteHeight = 0);
using Pixel = Gdiplus::ARGB;
template<typename TBitmapData>
inline Pixel *GetScanline(const TBitmapData &bitmapData, std::size_t y)
noexcept {
if(bitmapData.Stride >= 0) {
return reinterpret_cast<Pixel*>(
mpt::void_cast<void *>(mpt::void_cast<std::byte *>(bitmapData.Scan0)
+
y *bitmapData
.Stride));
}
...
return reinterpret_cast<Pixel*>(
mpt::void_cast<void *>(mpt::void_cast<std::byte *>(bitmapData.Scan0)
+ (bitmapData.Height - 1 - y) * (-bitmapData.Stride)));
}
}
constexpr Pixel
AsPixel(uint8
...
<uint8>(p
>> ALPHA_SHIFT); }
constexpr RawGDIDIB::Pixel
ToRawGDIDIB(Pixel
p) noexcept
{
return
RawGDIDIB::Pixel(GDIP::R(p), GDIP::G(p), GDIP::B(p), GDIP::A(p)
);
}
} // namespace GDIP
...
std::unique_ptr <RawGDIDIB>
LoadPixelImage(mpt::const_byte_span file, double scaling = 1.0, int spriteWidth = 0, int spriteHeight = 0);
std::unique_ptr <RawGDIDIB>
LoadPixelImage(FileReader file, double scaling = 1.0, int spriteWidth = 0, int spriteHeight = 0);

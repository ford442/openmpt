...
void SaveTags();
public:
CWaveConvert(CWnd *parent, ORDERINDEX minOrder, ORDERINDEX maxOrder, ORDERINDEX numOrders, CSoundFile &sndFile,
...
bool m_bGivePlugsIdleTime;
public:
CDoWaveConvert(CSoundFile &sndFile, mpt::ofstream &f, const CString &caption, const CWaveConvertSettings &settings,
               CWnd *parent = NULL)
        : CProgressDialog(parent), m_Settings(settings), m_SndFile(sndFile), fileStream(f), caption(caption),
          m_dwSongLimit(0) {}
void Run() override;
};

...
0, 0xFFFF, &CModControlDlg::OnToolTipText)
ON_NOTIFY_EX_RANGE(TTN_NEEDTEXTA,
0, 0xFFFF, &CModControlDlg::OnToolTipText)
//}}AFX_MSG_MAP
END_MESSAGE_MAP()
CModControlDlg::CModControlDlg(CModControlView &parent, CModDoc &document) : m_modDoc(document),
                                                                             m_sndFile(document.GetSoundFile()),
                                                                             m_parent(parent) {
...
IDC_TABCTRL1,    &CModControlView::OnTabSelchange)
...
DeferWindowPos(hdwp, m_TabCtrl.m_hWnd, NULL, rcClient.left, rcClient.top, rcClient.Width(), rcClient.Height(),
               SWP_NOZORDER);
...
void CModControlView::OnUpdate(CView * , LPARAM
lHint,
CObject *pHint
)
{
...
if(m_hWndMDI) ::PostMessage(m_hWndMDI, WM_MOD_CHANGEVIEWCLASS, (WPARAM)
lParam, (LPARAM)
pDlg);
return TRUE;
}
...
if((m_nActiveDlg >= 0) && (m_nActiveDlg < MAX_PAGES)) {
CModControlDlg *pActiveDlg = m_Pages[m_nActiveDlg];
if(pActiveDlg) return (LRESULT)
pActiveDlg->GetToolTipText(static_cast<UINT>(uId), (LPTSTR) pszText);
}
return 0;
}
...
#endif
IMPLEMENT_SERIAL(CModScrollView, CScrollView,
0)
...
void CModScrollView::OnUpdate(CView * pView, LPARAM
lHint,
CObject *pHint
)
{
...
const int imgSize = Util::ScalePixels(16, m_hWnd), btnSizeX = Util::ScalePixels(26,
                                                                                m_hWnd), btnSizeY = Util::ScalePixels(
        24, m_hWnd);

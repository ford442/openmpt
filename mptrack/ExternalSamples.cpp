...
ON_NOTIFY(NM_DBLCLK,
IDC_LIST1,    &MissingExternalSamplesDlg::OnSetPath)
...
MissingExternalSamplesDlg::MissingExternalSamplesDlg(CModDoc &modDoc, CWnd *parent)
        : ResizableDialog(IDD_MISSINGSAMPLES, parent), m_modDoc(modDoc), m_sndFile(modDoc.GetSoundFile()) {
}
...
BrowseForFolder dlg(TrackerSettings::Instance().PathSamples.GetWorkingDir(),
                    _T("Select a folder to search for missing samples..."));
if(dlg.Show()) {
...
IDC_LIST1, &ModifiedExternalSamplesDlg::OnSelectionChanged)
//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
ModifiedExternalSamplesDlg::ModifiedExternalSamplesDlg(CModDoc &modDoc, CWnd *parent)
        : ResizableDialog(IDD_MODIFIEDSAMPLES, parent), m_modDoc(modDoc), m_sndFile(modDoc.GetSoundFile()) {
}

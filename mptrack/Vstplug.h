...
static DWORD SETryOrError(bool maskCrashes, Tfn fn);
private:
template<typename Tfn>
DWORD SETryOrError(Tfn fn);
public:
CVstPlugin(bool maskCrashes, HMODULE hLibrary, VSTPluginLib &factory, SNDMIXPLUGIN &mixPlugin, Vst::AEffect &effect,
           CSoundFile &sndFile);
~CVstPlugin();
static Vst::AEffect *
LoadPlugin(bool maskCrashes, VSTPluginLib &plugin, HMODULE &library, bool forceBridge, bool forceLegacy);
protected:
void Initialize();
public:
...
return GetParamPropertyString(param, Vst::effGetParamDisplay);
};
static intptr_t
DispatchSEH(bool maskCrashes, Vst::AEffect *effect, Vst::VstOpcodeToPlugin opCode, int32 index, intptr_t value,
...
MasterCallBack(Vst::AEffect *effect, Vst::VstOpcodeToHost opcode, int32 index, intptr_t value, void *ptr, float opt);
protected:

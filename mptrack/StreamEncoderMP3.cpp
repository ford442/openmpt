...
ReplayGain()
        : Tag(TagSkip), TrackPeak(0.0f), TrackPeakValid(false), TrackGaindB(0.0f), TrackGaindBValid(false) {
return;
}
};
class ID3V2Tagger {
...
ID3V2Tagger::ID3V2Tagger(const Encoder::StreamSettings &settings_)
        : settings(settings_), totalID3v2Size(0) {
return;
}
...
uint32 ID3V2Tagger::intToSynchsafe(uint32
in)
{
...
return mpt::saturate_cast<uint32>(
        sizeof(ID3v2Frame) + 1 + std::strlen("REPLAYGAIN_TRACK_GAIN") + 1 + std::strlen("-123.45 dB") +
        1); // should be enough
}
uint32 ID3V2Tagger::GetMaxReplayGainTxxxTrackPeakFrameSize() {
return mpt::saturate_cast<uint32>(
        sizeof(ID3v2Frame) + 1 + std::strlen("REPLAYGAIN_TRACK_PEAK") + 1 + std::strlen("2147483648.123456") +
        1); // unrealistic worst case
}
...
MP3Encoder::ConstructStreamEncoder(std::ostream &file, const Encoder::Settings &settings, const FileTags &tags) const {

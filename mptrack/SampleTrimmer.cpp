...
CRenderProgressDlg(CWnd *parent, CSoundFile &sndFile)
        : CProgressDialog(parent), m_SndFile(sndFile) {
...
SetRange(0, mpt::saturate_round<uint32>(std::accumulate(subSongs.begin(), subSongs.end(), 0.0,
                                                        [](double acc, const GetLengthType &glt) {
                                                        return acc + glt.duration;
...
SetText(MPT_CFORMAT("Analyzing... {}:{}:{}")(timeSec / 3600, mpt::cfmt::dec0<2>((timeSec / 60) % 60),
                                             mpt::cfmt::dec0<2>(timeSec % 60)));
...
mpt::ustring s = MPT_UFORMAT("{} sample{} can be trimmed, saving {} bytes.")(numTrimmed,
                                                                             (numTrimmed == 1) ? U_("") : U_("s"),
                                                                             mpt::ufmt::dec(3, ',', numBytes));
if(dlg.m_abort) {
...
ModSample &sample = m_SndFile.GetSample(smp);
if(dlg.m_SamplePlayLengths[smp] != 0 && sample.nLength > dlg.m_SamplePlayLengths[smp]) {
GetSampleUndo().PrepareUndo(smp, sundo_delete, "Automatic Sample Trimming", dlg.m_SamplePlayLengths[smp],
                            sample.nLength);

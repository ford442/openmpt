...
CFindReplaceTab(UINT nIDD, bool isReplaceTab, CSoundFile &sf, FindReplace &settings, const ModCommand &initialValues)
        : CPropertyPage(nIDD), m_sndFile(sf), m_settings(settings), m_effectInfo(sf), m_initialValues(initialValues),
...
void CheckOnChange(int nIDButton) {
CheckDlgButton(nIDButton, BST_CHECKED);
CheckReplace(nIDButton);
};

...
static thread_local ExceptionHandler::Context
*
g_Context = nullptr;
...
ExceptionHandler::Context *ExceptionHandler::SetContext(Context * newContext)
noexcept
{
...
CrashOutputDirectory()
        : valid(true) {
const mpt::PathString timestampDir = mpt::PathString::FromCString(
...
DebugReporter::DebugReporter(DumpMode mode, _EXCEPTION_POINTERS *pExceptionInfo)
        : crashCount(g_CrashCount().fetch_add(1) + 1), taintCountDriver(g_TaintCountDriver().load()),
          taintCountPlugin(g_TaintCountPlugin().load()), stateFrozen(FreezeState(mode)), mode(mode),
          writtenMiniDump(false), writtenTraceLog(false), rescuedFiles(0) {
...
return WriteMemoryDump(pExceptionInfo, (crashDirectory.path + P_("crash.dmp")).AsNative().c_str(),
...
errorMessage += MPT_UFORMAT(
        "{} modified files have been rescued, but it cannot be guaranteed that they are still intact.")(rescuedFiles);
}
}
errorMessage += UL_("\n\n");
errorMessage += MPT_UFORMAT("OpenMPT {} {} ({} ({}))")
        (Build::GetVersionStringExtended(), mpt::OS::Windows::Name(mpt::OS::Windows::GetProcessArchitecture()),
         SourceInfo::Current().GetUrlWithRevision(), SourceInfo::Current().GetStateString()
        );
errorMessage += UL_("\n\n");
...
f
        << mpt::ToCharset(mpt::Charset::UTF8,
                          it.first.FormatAsString() + U_(" = ") + it.second.GetRefValue().FormatValueAsString())
        << std::endl;
}
}
}
...
CopyFile
        (theApp.GetConfigFileName().AsNative().c_str(), crashStoredSettingsFilename.AsNative().c_str(), FALSE
        );
...
Reporting::Error(errorMessage, (mode == DumpModeWarning) ? "OpenMPT Warning" : "OpenMPT Crash",
...
mpt::log::Trace::Seal();
if(mode == DumpModeCrash || mode == DumpModeWarning) {
if(CMainFrame::GetMainFrame() && CMainFrame::GetMainFrame()->gpSoundDevice &&
   CMainFrame::GetMainFrame()->gpSoundDevice->DebugIsFragileDevice()) {
...
errorMessage = MPT_UFORMAT("Unhandled MFC exception occurred while processming window message '{}': {}.")
        (mpt::ufmt::dec(pMsg ? pMsg->message : 0), mpt::ToUnicode(CString(tmp))
        );
} else {
errorMessage = MPT_UFORMAT("Unhandled MFC exception occurred while processming window message '{}': {}.")
        (mpt::ufmt::dec(pMsg ? pMsg->message : 0), mpt::ToUnicode(CString(tmp))
...
errorMessage += MPT_UFORMAT("Unhandled exception 0x{} at address 0x{} occurred.")
        (mpt::ufmt::HEX0<8>(pExceptionInfo->ExceptionRecord->ExceptionCode), mpt::ufmt::hex0<mpt::pointer_size * 2>(
                reinterpret_cast<std::uintptr_t>(pExceptionInfo->ExceptionRecord->ExceptionAddress))
        );
}
report.ReportError(errorMessage);
...
if(msg) {
errorMessage = MPT_UFORMAT(
        "Internal state inconsistency detected at {}({}). This is just a warning that could potentially lead to a crash later on: {} [{}].")
        (mpt::ToUnicode(mpt::Charset::ASCII, loc.file_name() ? loc.file_name() : ""), loc.line(),

...
MidiInOut(factory, sndFile, mixStruct
);
} catch(RtMidiError &)
{
return
...
MidiInOut::MidiInOut(VSTPluginLib &factory, CSoundFile &sndFile, SNDMIXPLUGIN *mixStruct)
        : IMidiPlugin(factory, sndFile, mixStruct), m_inputDevice(m_midiIn), m_outputDevice(m_midiOut)
#ifdef MODPLUG_TRACKER
...
static void FindPort(MidiDevice::ID & id, unsigned int
numPorts,
const std::string &name,
const std::string &friendlyName, MidiDevice
&midiDevice,
bool isInput
)
{
...
== kInputParameter), newDevice);
}
float MidiInOut::GetParameter(PlugParamIndex
index)
{
const MidiDevice &device = (index == kInputParameter) ? m_inputDevice : m_outputDevice;
return
DeviceIDToParameter(device
.index);
}
#ifdef MODPLUG_TRACKER
...
bool MidiInOut::MidiSend(uint32
midiCode)
{
if(!m_midiOut.
isPortOpen()
...
mpt::lock_guard<mpt::mutex> lock(m_mutex);
m_outQueue.
push_back(Message(GetOutputTimestamp(), &midiCode, 3)
);
return true;
}
...
void MidiInOut::OpenDevice(MidiDevice::ID
newDevice,
bool asInputDevice
)
{
...
device.
name = device.GetPortName(newDevice);
//if(m_isResumed)
{
mpt::lock_guard<mpt::mutex> lock(m_mutex);
try
...
// Get a device name
std::string MidiDevice::GetPortName(MidiDevice::ID
port)
{
return stream.
getPortName(port);

...
m_MIDIByte1 = static_cast<uint8>(controller);
}
...
size_t SetDirective(const size_t i, const CMIDIMappingDirective &d) {
m_Directives[i] = d;
Sort();
return std::find(m_Directives.begin(), m_Directives.end(), d) - m_Directives.begin();
}
// Return the index after sorting for the added element
size_t AddDirective(const CMIDIMappingDirective &d) {
m_Directives.push_back(d);
Sort();
return std::find(m_Directives.begin(), m_Directives.end(), d) - m_Directives.begin();
}
...
bool AreOrderEqual(const size_t a, const size_t b) const {
return !(m_Directives[a] < m_Directives[b] || m_Directives[b] < m_Directives[a]);
}
private:

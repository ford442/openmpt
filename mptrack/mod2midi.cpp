...
MidiTrack(VSTPluginLib &factory, CSoundFile &sndFile, SNDMIXPLUGIN *mixStruct, MidiTrack *tempoTrack,
          const mpt::ustring &name, const ModInstrument *oldInstr, bool overlappingInstruments)
        : IMidiPlugin(factory, sndFile, mixStruct), m_oldInstr(oldInstr), m_sndFile(sndFile), m_tempoTrack(tempoTrack),
          m_sampleRate(sndFile.GetSampleRate()), m_overlappingInstruments(overlappingInstruments) {
...
const uint8 msg[] = {ch, 0x64, 0x00, 0x00, ch, 0x65, 0x00, 0x00, ch, 0x06,
                     static_cast<uint8>(std::abs(m_instr.midiPWD))};
WriteTicks();
...
uint8 msg[6] = {0xFF, 0x51, 0x03, static_cast<uint8>(mspq >> 16), static_cast<uint8>(mspq >> 8),
                static_cast<uint8>(mspq)};
...
uint8 msg[9] = {0xF0, 0x07, 0x7F, 0x7F, 0x04, 0x01, static_cast<uint8>(midiVol & 0x7F),
                static_cast<uint8>((midiVol >> 7) & 0x7F), 0xF7};
...
if(note == NOTE_NOTECUT &&
   (m_oldInstr == nullptr || !(m_oldInstr->nMixPlug != 0 && m_oldInstr->HasValidMIDIChannel()))) {
...
Conversion(CSoundFile &sndFile, const InstrMap &instrMap, mpt::ofstream &file, bool overlappingInstruments)
        : m_oldInstruments(sndFile.GetNumInstruments()), m_plugFactory(nullptr, true, {}, {}, {}), m_sndFile(sndFile),
          m_file(file), m_wasInstrumentMode(sndFile.GetNumInstruments() > 0) {
...
MidiTrack &tempoTrack = *(new MidiTrack(m_plugFactory, m_sndFile, &tempoTrackPlugin, nullptr,
                                        mpt::ToUnicode(m_sndFile.GetCharsetInternal(), m_sndFile.m_songName), nullptr,
...
if(!m_sndFile.GetpModDoc()->IsInstrumentUsed(i) || (m_wasInstrumentMode && m_oldInstruments[i - 1] == nullptr) ||
   nextPlug >= MAX_MIXPLUGINS)
continue;
...
MidiTrack &midiInstr = *(new MidiTrack(m_plugFactory, m_sndFile, &mixPlugin, &tempoTrack,
                                       m_wasInstrumentMode ? mpt::ToUnicode(m_sndFile.GetCharsetInternal(),
                                                                            oldInstr->name) : mpt::ToUnicode(
                                               m_sndFile.GetCharsetInternal(), m_sndFile.GetSampleName(i)), oldInstr,
                                       overlappingInstruments));
...
CModToMidi::CModToMidi(CSoundFile &sndFile, CWnd *pWndParent)
        : CDialog(IDD_MOD2MIDI, pWndParent), m_sndFile(sndFile),
...
auto choice = Reporting::Confirm(
        _T("No instruments have been selected for export. Would you still like to export the file?"), true, true);
if(choice == cnfYes)
CDialog::OnOK();
...
auto conv = std::make_unique<MidiExport::Conversion>(m_sndFile, m_instrMap, m_file,
                                                     CModToMidi::s_overlappingInstruments);
...
SetText(fmt(curTime / 60u, mpt::tfmt::dec0<2>(curTime % 60u), timeRemaining / 60u,
            mpt::tfmt::dec0<2>(timeRemaining % 60u)).c_str());

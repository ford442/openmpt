...
const RawGDIDIB::Pixel *in1 = m_bitmapSrc->Pixels().data(), *in2 =
        m_bitmapSrc->Pixels().data() + (m_bitmapSrc->Height() - 1) * m_bitmapSrc->Width();
RawGDIDIB::Pixel * out1 = m_bitmapTarget->Pixels().data(), *out2 = m_bitmapTarget->Pixels().data() +
                                                                   (m_bitmapSrc->Height() - 1) * m_bitmapSrc->Width();
...
RawGDIDIB::Pixel * limitMin = m_bitmapSrc->Pixels().data(), *limitMax = m_bitmapSrc->Pixels().data() +
                                                                        m_bitmapSrc->Pixels().size() - 1;
...
mpt::ustring name = mpt::ToUnicode(mpt::Charset::ASCII,
                                   (info.name.substr(0, 9) == "Component") ? info.name.substr(9) : info.name);
...
text += MPT_UFORMAT("{}: {}")
        (name, info.state == ComponentStateAvailable ? U_("ok") :
...
text += MPT_UFORMAT("Windows version: {}\n")
        (
                ...
                mpt::OS::Windows::Version::Current().IsAtLeast(mpt::OS::Windows::Version::WinVista) ? U_(
                        "Windows Vista") :
...
IDC_TABABOUT, &CAboutDlg::OnTabChange)
END_MESSAGE_MAP()

...
public:
CFindRangeDlg(CWnd *parent, int minVal, int minDefault, int maxVal, int maxDefault, DisplayMode displayMode) : CDialog(
        IDD_FIND_RANGE, parent), m_minVal(minVal), m_minDefault(minDefault), m_maxVal(maxVal), m_maxDefault(maxDefault),
                                                                                                               m_displayMode(
                                                                                                                       displayMode) {}
...
sel = 2;
break;
case FindReplace::ReplaceOctaveDown:
sel = 3;
break;
case FindReplace::ReplaceOctaveUp  :
sel = 4;
break;
default:
sel = 5;
...
if(searchNote == m_cbnNote.GetItemData(i)) {
sel = i;
break;
}
}
...
if(fxndx == m_cbnVolCmd.GetItemData(i)) {
...
fxndx = m_effectInfo.GetIndexFromEffect(m_isReplaceTab ? m_settings.replaceCommand : m_settings.findCommand,
                                        static_cast<ModCommand::PARAM>(m_isReplaceTab ? m_settings.replaceParam
                                                                                      : m_settings.findParamMin));
for(UINT i = 0; i <= count; i++)
if(fxndx == m_cbnCommand.GetItemData(i)) {
...
if(m_isReplaceTab) {
m_cbnInstr.SetItemData(m_cbnInstr.AddString(isPCEvent ? _T("Plugin -1") : _T("Instrument -1")),
                       kReplaceInstrumentMinusOne);
m_cbnInstr.SetItemData(m_cbnInstr.AddString(isPCEvent ? _T("Plugin +1") : _T("Instrument +1")),
                       kReplaceInstrumentPlusOne);
...
switch (item) {
case kReplaceNoteMinusOne:
m_settings.replaceNote = -1;
break;
case kReplaceNotePlusOne:
m_settings.replaceNote = 1;
break;
case kReplaceNoteMinusOctave:
m_settings.replaceNote = FindReplace::ReplaceOctaveDown;
break;
case kReplaceNotePlusOctave:
m_settings.replaceNote = FindReplace::ReplaceOctaveUp;
break;
...
switch (item) {
case kReplaceInstrumentMinusOne:
m_settings.replaceInstr = -1;
break;
case kReplaceInstrumentPlusOne:
m_settings.replaceInstr = 1;
break;
case kReplaceRelative: {
...
if(item == kFindRange) {
CFindRangeDlg dlg(this, 1, m_settings.findInstrMin, MAX_INSTRUMENTS - 1, m_settings.findInstrMax,
                  CFindRangeDlg::kDecimal);
if(dlg.DoModal() == IDOK) {
...
void
CFindReplaceTab::RelativeOrMultiplyPrompt(CComboBox &comboBox, FindReplace::ReplaceMode &action, int &value, int range,
                                          bool isHex) {
...
if(item == kFindRange) {
CFindRangeDlg dlg(this, 0, m_settings.findParamMin & ~mask, m_cbnParam.GetCount() - 2, m_settings.findParamMax & ~mask,
                  CFindRangeDlg::kHex);
if(dlg.DoModal() == IDOK) {
...
if(item == kFindRange) {
CFindRangeDlg dlg(this, 0, m_settings.findParamMin, ModCommand::maxColumnValue, m_settings.findParamMax,
                  CFindRangeDlg::kDecimal);
if(dlg.DoModal() == IDOK) {

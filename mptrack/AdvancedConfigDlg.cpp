...
ON_NOTIFY(NM_DBLCLK,
IDC_LIST1,    &COptionsAdvanced::OnOptionDblClick)
#ifndef MPT_MFC_FULL
ON_NOTIFY(NM_CUSTOMDRAW,
IDC_LIST1, &COptionsAdvanced::OnCustomDrawList)
#endif
...
if(m_listGrouped) {
static constexpr ListCtrl::Header
headers[] =
        {
...
COLORREF modColor = RGB(GetRValue(defColor) * 0.9 + GetRValue(txtColor) * 0.1,
                        GetGValue(defColor) * 0.9 + GetGValue(txtColor) * 0.1,
                        GetBValue(defColor) * 0.9 + GetBValue(txtColor) * 0.1);
...
CInputDlg inputDlg(this, _T("Enter new value for ") + mpt::ToCString(path.FormatAsString()),
                   mpt::ToCString(val.FormatValueAsString()));

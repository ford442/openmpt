...
if(m_hWndView)
::SendMessage(m_hWndView, WM_COMMAND, ID_EDIT_MIXPASTE_ITSTYLE, 0);
}
...
if(m_hWndView)
::SendMessage(m_hWndView, WM_COMMAND, ID_EDIT_PUSHFORWARDPASTE, 0);
}
...
afx_msg LRESULT
OnUnlockControls(WPARAM, LPARAM
) {
if(m_nLockCount > 0) m_nLockCount--;
return 0;
}
afx_msg BOOL
OnToolTipText(UINT, NMHDR
* pNMHDR,
LRESULT *pResult
);
afx_msg LRESULT
OnDPIChanged(WPARAM = 0, LPARAM = 0
);
//}}AFX_MSG
DECLARE_MESSAGE_MAP()
};
...
afx_msg void OnSwitchToView() { if(m_hWndView) ::PostMessage(m_hWndView, WM_MOD_VIEWMSG, VIEWMSG_SETFOCUS, 0); }
afx_msg LRESULT
OnActivateModView(WPARAM, LPARAM
);
afx_msg LRESULT
OnModCtrlMsg(WPARAM
wParam,
...
afx_msg LRESULT
OnGetToolTipText(WPARAM, LPARAM
);
//}}AFX_MSG
DECLARE_MESSAGE_MAP()
};
...
return static_cast<CModControlView *>(CWnd::FromHandle(m_hWndCtrl))->GetCurrentControlDlg();
}
protected:
//{{AFX_MSG(CModScrollView)
afx_msg void OnDestroy();
afx_msg LRESULT
OnReceiveModViewMsg(WPARAM
wParam,
LPARAM lParam
);
afx_msg BOOL
...
afx_msg LRESULT
OnDragonDropping(WPARAM
bDoDrop,
LPARAM lParam
) {
return OnDragonDrop((BOOL)
bDoDrop, (const DRAGONDROP *) lParam);
}
afx_msg LRESULT
...
void SetScrollSizes(int nMapMode, SIZE sizeTotal, const SIZE &sizePage = CScrollView::sizeDefault,

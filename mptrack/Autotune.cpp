...
static SmpLength NoteToShift(uint32
sampleFreq,
int note,
...
return
std::max(mpt::saturate_round<SmpLength>((double) sampleFreq / fundamentalFrequency), SmpLength(1)
);
}
...
static inline AutotuneHistogram operator+(AutotuneHistogram a, AutotuneHistogram b)
noexcept {
...
static inline AutotuneHistogram &operator+=(AutotuneHistogram &a, AutotuneHistogramEntry b)
noexcept
{
a.histogram[b.index] += b.
sum;
return
a;
}
struct AutotuneHistogramReduce {
inline AutotuneHistogram operator()(AutotuneHistogram a, AutotuneHistogram b)
noexcept
{
return a + b;
}
inline AutotuneHistogram operator()(AutotuneHistogramEntry a, AutotuneHistogramEntry b)
noexcept
{
AutotuneHistogram result;
result += a;
...
inline AutotuneHistogram operator()(AutotuneHistogramEntry a, AutotuneHistogram b)
noexcept
{
b += a;
return b;
}
inline AutotuneHistogram operator()(AutotuneHistogram a, AutotuneHistogramEntry b)
noexcept
{
a += b;
return a;
...
#endif
        std::transform_reduce(std::execution::par_unseq, std::begin(notes), std::end(notes), AutotuneHistogram{},
                              AutotuneHistogramReduce{}, [ctx](int note) { return CalculateNoteHistogram(note, ctx); });
...
int minimumBin = static_cast<int>(
        std::min_element(std::begin(interpolated.histogram), std::end(interpolated.histogram)) -
        std::begin(interpolated.histogram));
...
minimumBin += targetNote * BINS_PER_NOTE;
const double newFundamentalFreq = NoteToFrequency(
        static_cast<double>(69 - targetNote) + static_cast<double>(minimumBin) / BINS_PER_NOTE, pitchReference);
if(const auto newFreq = mpt::saturate_round<uint32>(sampleFreq * pitchReference / newFundamentalFreq); newFreq !=
                                                                                                       sampleFreq)

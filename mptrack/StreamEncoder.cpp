...
WriteInterleavedImpl(std::ostream &f, uint16 channels, Encoder::Format format, std::size_t frameCount,
                     const Tsample *interleaved) {
MPT_ASSERT(endian == format.endian);
MPT_ASSERT(format.GetSampleFormat() == SampleFormatTraits<Tsample>::sampleFormat());
bool success = true;
std::size_t written = 0;
MPT_MAYBE_CONSTANT_IF(endian == mpt::get_endian() && format.encoding != Encoder::Format::Encoding::Alaw &&
                      format.encoding != Encoder::Format::Encoding::ulaw)
{
if(!mpt::IO::WriteRaw(f, reinterpret_cast<const std::byte *>(interleaved),
                      frameCount * channels * format.GetSampleFormat().GetSampleSize())) {
success = false;
}
...
mpt::IO::WriteRaw(bf, reinterpret_cast<const std::byte *>(frameData.data()),
                  channels * format.GetSampleFormat().GetSampleSize());
...
WriteInterleavedLE(std::ostream &f, uint16 channels, Encoder::Format format, std::size_t frameCount,
...
std::pair<bool, std::size_t>
WriteInterleavedLE(std::ostream &f, uint16 channels, Encoder::Format format, std::size_t frameCount,
                   const int32 *interleaved) {
MPT_ASSERT(format.endian == mpt::endian::little);
return WriteInterleavedImpl<mpt::endian::little>(f, channels, format, frameCount, interleaved);
}
std::pair<bool, std::size_t>
WriteInterleavedLE(std::ostream &f, uint16 channels, Encoder::Format format, std::size_t frameCount,
                   const int24 *interleaved) {
MPT_ASSERT(format.endian == mpt::endian::little);
return WriteInterleavedImpl<mpt::endian::little>(f, channels, format, frameCount, interleaved);
}
std::pair<bool, std::size_t>
WriteInterleavedLE(std::ostream &f, uint16 channels, Encoder::Format format, std::size_t frameCount,
                   const int16 *interleaved) {
MPT_ASSERT(format.endian == mpt::endian::little);
return WriteInterleavedImpl<mpt::endian::little>(f, channels, format, frameCount, interleaved);
}
std::pair<bool, std::size_t>
WriteInterleavedLE(std::ostream &f, uint16 channels, Encoder::Format format, std::size_t frameCount,
                   const int8 *interleaved) {
MPT_ASSERT(format.endian == mpt::endian::little);
return WriteInterleavedImpl<mpt::endian::little>(f, channels, format, frameCount, interleaved);
}
std::pair<bool, std::size_t>
...
std::pair<bool, std::size_t>
WriteInterleavedBE(std::ostream &f, uint16 channels, Encoder::Format format, std::size_t frameCount,
                   const double *interleaved) {

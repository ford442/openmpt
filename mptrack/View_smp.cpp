...
static constexpr SmpLength
MIN_TRIM_LENGTH = 4;
static constexpr UINT
cLeftBarButtons[SMP_LEFTBAR_BUTTONS] =
{
ID_SAMPLE_ZOOMUP,
ID_SAMPLE_ZOOMDOWN,
ID_SEPARATOR,
...
IMPLEMENT_SERIAL(CViewSample, CModScrollView,
0)
...
ON_MESSAGE(WM_MOD_KEYCOMMAND, &CViewSample::OnCustomKeyMsg
) //rewbs.customKeys
//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
CViewSample::CViewSample()
        : m_lastDrawPoint(-1, -1), m_timelineHeight(TIMELINE_HEIGHT) {
...
m_oplEditor->SetWindowPos(nullptr, -m_nScrollPosX, -m_nScrollPosY, std::max(size.cx, m_rcClient.right),
                          std::max(size.cy, m_rcClient.bottom), SWP_NOZORDER | SWP_NOACTIVATE);
...
return m_nSample >= 1 && m_nSample <= GetDocument()->GetNumSamples() &&
       GetDocument()->GetSoundFile().GetSample(m_nSample).uFlags[CHN_ADLIB];
}
...
m_oplEditor->SetWindowPos(nullptr, -m_nScrollPosX, -m_nScrollPosY, std::max(size.cx, m_rcClient.right),
                          std::max(size.cy, m_rcClient.bottom), SWP_NOZORDER | SWP_NOACTIVATE | SWP_SHOWWINDOW);
}
}
}
...
mpt::ustring(*fmt)(unsigned int, char,
const SmpLength &) = &mpt::ufmt::dec<SmpLength>;
if(TrackerSettings::Instance().cursorPositionInHex)
fmt = &mpt::ufmt::HEX<SmpLength>;
s = MPT_UFORMAT("[{}-{}] ({} sample{}, ")(fmt(3, ',', m_dwBeginSel), fmt(3, ',', m_dwEndSel), fmt(3, ',', selLength),
                                          (selLength == 1) ? U_("") : U_("s"));
...
beats *= sndFile.GetSampleRate() / static_cast<double>(
        Util::mul32to64_unsigned(sndFile.m_PlayState.m_nCurrentRowsPerBeat, sndFile.m_PlayState.m_nMusicSpeed) *
        Util::mul32to64_unsigned(sndFile.m_PlayState.m_nSamplesPerTick, sampleRate));
}
...
size_t cue = std::size(sample.cues) - 1 -
             i;  // If two cues overlap visually, the cue with the higher ID is drawn on top, so pick it first
if(sample.cues[cue] < sample.nLength &&
   HitTest(point.x, SampleToScreen(sample.cues[cue]), m_timelineHeight / 2, m_timelineHeight / 2, 0, m_timelineHeight,
           rect))
...
if(m_nSample <= sndFile.GetNumSamples() &&
   m_cachedSampleRate != sndFile.GetSample(m_nSample).GetSampleRate(sndFile.GetType())) {
UpdateScrollSize();
InvalidateTimeline();
}
...
void CViewSample::DrawSampleData1(HDC hdc, int ymed, int cx, int cy, SmpLength len, SampleFlags uFlags,
...
void CViewSample::DrawSampleData2(HDC hdc, int ymed, int cx, int cy, SmpLength len, SampleFlags uFlags,
                                  const void *pSampleData) {
int oldsmin, oldsmax;
int yrange = cy / 2;
const int8 *psample = static_cast<const int8 *>(pSampleData);
...
mpt::tstring text;
for(int x = -(SampleToScreen(ScrollPosToSamplePos(), true) % m_timelineInterval);
    x < m_rcClient.right + m_timelineInterval; x += m_timelineInterval) {
text.clear();
if(format == TimelineFormat::Seconds && sampleRate) {
const int64 time = mpt::saturate_round<int64>(
        std::round(ScreenToSeconds(x, true) * 1000.0 / m_timelineUnit) * m_timelineUnit);
...
const SmpLength smp = mpt::saturate_round<SmpLength>(
        std::round(ScreenToSample(x, true) / static_cast<double>(m_timelineUnit)) * m_timelineUnit);
...
m_offScreenDC.TransparentBlt(rect.left, rect.top, rect.Width(), rect.Height(), &m_waveformDC, rect.left, rect.top,
                             rect.Width(), rect.Height(), backgroundCol);
}
}
...
const auto rightSegment = std::min(m_nGridSegments,
                                   mpt::saturate_round<uint32>(ScreenToSample(rect.right) * segmentsByLength));
...
DrawPositionMarks();
BitBlt(pDC->m_hDC, m_rcClient.left, m_rcClient.top, m_rcClient.Width(), m_rcClient.Height(), m_offScreenDC, 0, 0,
       SRCCOPY);
if(oldFont)
...
bool CViewSample::GetNcButtonRect(UINT
button,
CRect &rect
) const
{
...
top = 3;
rect.
bottom = rect.top + SMP_LEFTBAR_CYBTN;
...
{
rect.
right = rect.left + SMP_LEFTBAR_CXBTN;
}
return true;
}
...
nImage = SIMAGE_ZOOMUP;
break;
case ID_SAMPLE_ZOOMDOWN:
nImage = SIMAGE_ZOOMDOWN;
break;
case ID_SAMPLE_DRAW:
nImage = SIMAGE_DRAW;
break;
case ID_SAMPLE_ADDSILENCE:
nImage = SIMAGE_RESIZE;
break;
case ID_SAMPLE_GRID:
nImage = SIMAGE_GRID;
break;
...
*data = static_cast<T>(static_cast<double>(oldvalue) +
                       (value - oldvalue) * (static_cast<double>(i - old) / static_cast<double>(m_dwEndDrag - old)));
}
*data = static_cast<T>(value);
}
void CViewSample::OnMouseMove(UINT
flags,
CPoint point
)
{
...
{
m_dwStatus.
reset(SMPSTATUS_NCLBTNDOWN);
m_nBtnMouseOver = 0xFFFF;
UpdateNcButtonState();
CMainFrame *pMainFrm = CMainFrame::GetMainFrame();
if (pMainFrm) pMainFrm->
SetHelpText(_T("")
);
}
if(!pModDoc)
...
if(
TrackerSettings::Instance()
.cursorPositionInHex)
fmt = &mpt::cfmt::HEX<SmpLength>;
UpdateIndicator(MPT_CFORMAT("Cursor: {}")
(fmt(3, ',', x)));
CMainFrame *pMainFrm = CMainFrame::GetMainFrame();
if(
pMainFrm &&m_dwEndSel
...
if(m_nSample > 0 && m_nSample <= sndFile.
GetNumSamples() &&
x<sample.nLength)
{
...
const char highOffsetChar = sndFile.GetModSpecifications().GetEffectLetter(
        static_cast<ModCommand::COMMAND>(sndFile.GetModSpecifications().HasCommand(CMD_S3MCMDEX) ? CMD_S3MCMDEX
                                                                                                 : CMD_XFINEPORTAUPDOWN));
CString s;
if(xHigh == 0)
s.
Format(_T("Offset: %c%02X"), offsetChar, xLow
);
else if(
hasHighOffset &&xHigh<
0x10)
s.
Format(_T("Offset: %c%02X, %cA%X"), offsetChar, xLow, highOffsetChar, xHigh
);
else
...
const SmpLength x = fineDrag ? m_startDragValue + (point.x - m_startDragPoint.x) / Util::ScalePixels(2, m_hWnd)
                             : SnapToGrid(ScreenToSample(point.x));
if((
flags &MK_SHIFT
) && !fineDrag)
{
...
if(
updateLoopPoint &&updateLoopDesc
&& *updateLoopPoint != x)
{
...
GetSampleUndo()
.
PrepareUndo(m_nSample, sundo_none, updateLoopDesc
);
...
void CViewSample::OnLButtonDown(UINT
flags,
CPoint point
)
{
...
GetSampleUndo()
.
PrepareUndo(m_nSample, sundo_replace,
"Draw Sample");
if(sample.
GetElementarySampleSize()
== 2)
...
void CViewSample::OnRButtonDown(UINT, CPoint
pt)
{
CModDoc *pModDoc = GetDocument();
if(pModDoc)
...
wsprintf(s, _T("&Delete Cue Point %d"),
1 + static_cast
<int>(m_dwMenuParam)
);
::AppendMenu(hMenu, MF_STRING,
ID_SAMPLE_DELETE_CUEPOINT, s);
::AppendMenu(hMenu, MF_SEPARATOR,
0, _T(""));
}
auto fmt = TrackerSettings::Instance().sampleEditorTimelineFormat.Get();
::AppendMenu(hMenu, MF_STRING
| (fmt == TimelineFormat::Seconds ? MF_CHECKED : 0), ID_SAMPLE_TIMELINE_SECONDS, _T("&Seconds"));
::AppendMenu(hMenu, MF_STRING
| (fmt == TimelineFormat::Samples ? MF_CHECKED : 0), ID_SAMPLE_TIMELINE_SAMPLES, _T("S&amples"));
::AppendMenu(hMenu, MF_STRING
| (fmt == TimelineFormat::SamplesPow2 ? MF_CHECKED : 0), ID_SAMPLE_TIMELINE_SAMPLES_POW2, _T("Samples (&Power of 2)"));
ClientToScreen(&pt);
...
| (
CanZoomSelection()
? 0 : MF_GRAYED), ID_SAMPLE_ZOOMONSEL, ih->
GetKeyTextFromCommand(kcSampleZoomSelection, _T("Zoom")
));
::AppendMenu(hMenu, MF_STRING,
ID_SAMPLE_SETLOOP, _T("Set As Loop"));
if (sndFile.
GetType() &
(MOD_TYPE_IT|MOD_TYPE_MPT))
::AppendMenu(hMenu, MF_STRING,
ID_SAMPLE_SETSUSTAINLOOP, _T("Set As Sustain Loop"));
::AppendMenu(hMenu, MF_SEPARATOR,
0, _T(""));
} else
{
...
::AppendMenu(hMenu, MF_STRING
| (dwPos >= sample.nLoopStart + 4 ? 0 : MF_GRAYED),
ID_SAMPLE_SETLOOPEND, s);
if(sample.
HasPingPongLoop()
)
::AppendMenu(hMenu, MF_STRING,
ID_CONVERT_PINGPONG_LOOP, ih->
GetKeyTextFromCommand(kcSampleConvertPingPongLoop, _T("Convert to Unidirectional Loop")
));
...
::AppendMenu(hMenu, MF_STRING
| (dwPos + 4 <= sustainEnd ? 0 : MF_GRAYED),
...
::AppendMenu(hMenu, MF_STRING
| (dwPos >= sample.nSustainStart + 4 ? 0 : MF_GRAYED),
ID_SAMPLE_SETSUSTAINEND, s);
if(sample.
HasPingPongSustainLoop()
)
::AppendMenu(hMenu, MF_STRING,
ID_CONVERT_PINGPONG_SUSTAIN, ih->
GetKeyTextFromCommand(kcSampleConvertPingPongSustain, _T("Convert to Unidirectional Sustain Loop")
));
}
...
for(
std::size_t i = 0;
i< std::size(sample.cues);
i++)
{
const SmpLength cue = sample.cues[i];
wsprintf(s, _T("Cue &%d: %s"),
1 + static_cast
<int>(i),
        cue<sample
.nLength ? mpt::cfmt::dec(3, ',', cue).
GetString() : _T("unused")
);
::AppendMenu(hCueMenu, MF_STRING,
ID_SAMPLE_CUE_1 + i, s);
if(cue > 0 && cue<sample.nLength)
hasValidCues = true;
}
...
::AppendMenu(hMenu, MF_STRING
| (hasValidCues ? 0 : MF_GRAYED), ID_SAMPLE_SLICE, ih->
GetKeyTextFromCommand(kcSampleSlice, _T("Slice at cue points")
));
}
::AppendMenu(hMenu, MF_SEPARATOR,
0, _T(""));
...
> 1)
::AppendMenu(hMenu, MF_STRING,
ID_SAMPLE_8BITCONVERT, ih->
GetKeyTextFromCommand(kcSample8Bit, _T("Convert to &8-bit")
));
else
::AppendMenu(hMenu, MF_STRING,
ID_SAMPLE_16BITCONVERT, ih->
GetKeyTextFromCommand(kcSample8Bit, _T("Convert to &16-bit")
));
if(sample.
GetNumChannels()
> 1)
...
::AppendMenu(hMonoMenu, MF_STRING,
ID_SAMPLE_MONOCONVERT, ih->
GetKeyTextFromCommand(kcSampleMonoMix, _T("&Mix Channels")
));
::AppendMenu(hMonoMenu, MF_STRING,
ID_SAMPLE_MONOCONVERT_LEFT, ih->
GetKeyTextFromCommand(kcSampleMonoLeft, _T("&Left Channel")
));
::AppendMenu(hMonoMenu, MF_STRING,
ID_SAMPLE_MONOCONVERT_RIGHT, ih->
GetKeyTextFromCommand(kcSampleMonoRight, _T("&Right Channel")
));
::AppendMenu(hMonoMenu, MF_STRING,
ID_SAMPLE_MONOCONVERT_SPLIT, ih->
GetKeyTextFromCommand(kcSampleMonoSplit, _T("&Split Sample")
));
::AppendMenu(hMenu, MF_POPUP,
reinterpret_cast
<UINT_PTR>(hMonoMenu), _T("Convert to &Mono")
);
}
...
::AppendMenu(hMenu, MF_STRING
| (isGrayed ? MF_GRAYED : 0), ID_SAMPLE_TRIM, ih->
GetKeyTextFromCommand(kcSampleTrim, trimMenuText
));
if((m_dwBeginSel == 0 && m_dwEndSel != 0) || (m_dwBeginSel<sample.
nLength &&m_dwEndSel
== sample.nLength))
{
::AppendMenu(hMenu, MF_STRING,
ID_SAMPLE_QUICKFADE, ih->
GetKeyTextFromCommand(kcSampleQuickFade, _T("Quick &Fade")
));
}
::AppendMenu(hMenu, MF_STRING, ID_EDIT_CUT, ih
->
GetKeyTextFromCommand(kcEditCut, _T("Cu&t")
));
::AppendMenu(hMenu, MF_STRING, ID_EDIT_COPY, ih
->
GetKeyTextFromCommand(kcEditCopy, _T("&Copy")
));
}
const UINT clipboardFlag = (IsClipboardFormatAvailable(CF_WAVE) ? 0 : MF_GRAYED);
::AppendMenu(hMenu, MF_STRING
| clipboardFlag, ID_EDIT_PASTE, ih->
GetKeyTextFromCommand(kcEditPaste, _T("&Paste (Replace)")
));
::AppendMenu(hMenu, MF_STRING
| clipboardFlag, ID_EDIT_PUSHFORWARDPASTE, ih->
GetKeyTextFromCommand(kcEditPushForwardPaste, _T("Paste (&Insert)")
));
::AppendMenu(hMenu, MF_STRING
| clipboardFlag, ID_EDIT_MIXPASTE, ih->
GetKeyTextFromCommand(kcEditMixPaste, _T("Mi&x Paste")
));
::AppendMenu(hMenu, MF_STRING
| (pModDoc->
GetSampleUndo()
.
CanUndo(m_nSample)
? 0 : MF_GRAYED), ID_EDIT_UNDO, ih->
GetKeyTextFromCommand(kcEditUndo, _T("&Undo ")
+
mpt::ToCString(pModDoc
->
GetSoundFile()
.
GetCharsetInternal(), pModDoc
->
GetSampleUndo()
.
GetUndoName(m_nSample)
)));
::AppendMenu(hMenu, MF_STRING
| (pModDoc->
GetSampleUndo()
.
CanRedo(m_nSample)
? 0 : MF_GRAYED), ID_EDIT_REDO, ih->
GetKeyTextFromCommand(kcEditRedo, _T("&Redo ")
+
mpt::ToCString(pModDoc
->
GetSoundFile()
...
void CViewSample::OnNcMouseMove(UINT
nHitTest,
CPoint point
)
{
...
void CViewSample::OnNcLButtonDown(UINT
uFlags,
CPoint point
)
{
...
void CViewSample::OnNcLButtonUp(UINT
uFlags,
CPoint point
)
{
...
void CViewSample::OnNcLButtonDblClk(UINT
uFlags,
CPoint point
)
{
...
mpt::String::WriteBuf(mpt::String::maybeNullTerminated, sampleHeader.reserved2) = mpt::ToCharset(mpt::Charset::UTF8,
...
mpt::IO::OFile<std::pair < mpt::byte_span, mpt::IO::Offset>>
ff(mf);
WAVWriter file(ff);

// Write sample format
file.WriteFormat(sample.GetSampleRate(sndFile.GetType()), sample.GetElementarySampleSize() * 8, sample.GetNumChannels(),
                 WAVFormatChunk::fmtPCM);
...
static void MixSampleOnto(const ModSample &sample, SmpLength offset, int amplify, uint8 chn, uint8 newNumChannels,
                          int16 *pNewSample) {
uint8 numChannels = sample.GetNumChannels();
switch (sample.GetElementarySampleSize()) {
case 1:
MixSampleLoop(sample.nLength, sample.sample8() + (chn % numChannels), numChannels, amplify,
              pNewSample + offset * newNumChannels + chn, newNumChannels);
break;
case 2:
MixSampleLoop(sample.nLength, sample.sample16() + (chn % numChannels), numChannels, amplify,
              pNewSample + offset * newNumChannels + chn, newNumChannels);
break;
default:
...
CopySample(oldSample.sample8() + offset + chn, sample.nLength, numChannels, sample.sample8() + newChn,
           sample.GetSampleSizeInBytes(), sample.GetNumChannels(), SC::ConversionChain < SC::Convert < int8, int8 > ,
           SC::DecodeIdentity < int8 > > ());
} else if(oldSample.GetElementarySampleSize() == 2 && sample.GetElementarySampleSize() == 1) {
CopySample(oldSample.sample16() + offset + chn, sample.nLength, numChannels, sample.sample8() + newChn,
           sample.GetSampleSizeInBytes(), sample.GetNumChannels(), SC::ConversionChain < SC::Convert < int16, int8 > ,
           SC::DecodeIdentity < int8 > > ());
} else if(oldSample.GetElementarySampleSize() == 2 && sample.GetElementarySampleSize() == 2) {
CopySample(oldSample.sample16() + offset + chn, sample.nLength, numChannels, sample.sample16() + newChn,
           sample.GetSampleSizeInBytes(), sample.GetNumChannels(), SC::ConversionChain < SC::Convert < int16, int16 > ,
           SC::DecodeIdentity < int16 > > ());
} else {
MPT_ASSERT_NOTREACHED();
}
}
...
pModDoc->PlayNote(
        PlayNoteParam(note).Sample(m_nSample).Volume(volume).LoopStart(loopstart).LoopEnd(loopend).Offset(nStartPos),
        &m_noteChannel);
m_dwStatus.set(SMPSTATUS_KEYDOWN);
uint32 freq = sndFile.GetFreqFromPeriod(
        sndFile.GetPeriodFromNote(note + (sndFile.GetType() == MOD_TYPE_XM ? sample.RelativeTone : 0), sample.nFineTune,
                                  sample.nC5Speed), sample.nC5Speed, 0);
...
SendCtrlMessage(CTRLMSG_SMP_SONGDROP, (LPARAM)
dropInfo);
break;
case DRAGONDROP_MIDIINSTR:
...
void CViewSample::OnSetCuePoint(UINT
nID)
{
nID -= ID_SAMPLE_CUE_1;
CModDoc *pModDoc = GetDocument();
...
GetSampleUndo()
.
PrepareUndo(m_nSample, sundo_none,
"Set Cue Point");
sample.cues[nID] =
m_dwMenuParam;
SetModified (SampleHint()
...
CString str;
str.Format(_T("Cannot add silence because the new sample length would exceed maximum sample length %u."),
           MAX_SAMPLE_LENGTH);
...
TrimSample(false);
return wParam;
case kcSampleTrimToLoopEnd:
TrimSample(true);
return wParam;
case kcSampleZoomUp:
OnZoomUp();
return wParam;
case kcSampleZoomDown:
OnZoomDown();
return wParam;
case kcSampleZoomSelection:
OnZoomOnSel();
return wParam;
...
break;
case kcSampleCenterLoopStart:
point = sample.nLoopStart;
break;
case kcSampleCenterLoopEnd:
point = sample.nLoopEnd;
break;
case kcSampleCenterSustainStart:
point = sample.nSustainStart;
break;
case kcSampleCenterSustainEnd:
point = sample.nSustainEnd;
break;
}
if(!m_nZoom) {
...
OnEditRedo();
return wParam;
case kcSampleConvertPingPongLoop:
OnConvertPingPongLoop();
return wParam;
case kcSampleConvertPingPongSustain:
OnConvertPingPongSustain();
return wParam;
case kcSample8Bit:
if(sndFile.GetSample(m_nSample).uFlags[CHN_16BIT])
On8BitConvert();
else
On16BitConvert();
return wParam;
case kcSampleMonoMix:
OnMonoConvertMix();
return wParam;
case kcSampleMonoLeft:
OnMonoConvertLeft();
return wParam;
case kcSampleMonoRight:
OnMonoConvertRight();
return wParam;
case kcSampleMonoSplit:
OnMonoConvertSplit();
return wParam;
case kcSampleReverse:
PostCtrlMessage(IDC_SAMPLE_REVERSE);
return wParam;
case kcSampleSilence:
PostCtrlMessage(IDC_SAMPLE_SILENCE);
return wParam;
case kcSampleNormalize:
PostCtrlMessage(IDC_SAMPLE_NORMALIZE);
return wParam;
case kcSampleAmplify:
PostCtrlMessage(IDC_SAMPLE_AMPLIFY);
return wParam;
case kcSampleInvert:
PostCtrlMessage(IDC_SAMPLE_INVERT);
return wParam;
case kcSampleSignUnsign:
PostCtrlMessage(IDC_SAMPLE_SIGN_UNSIGN);
return wParam;
case kcSampleRemoveDCOffset:
PostCtrlMessage(IDC_SAMPLE_DCOFFSET);
return wParam;
case kcSampleXFade:
PostCtrlMessage(IDC_SAMPLE_XFADE);
return wParam;
case kcSampleAutotune:
PostCtrlMessage(IDC_SAMPLE_AUTOTUNE);
return wParam;
case kcSampleQuickFade:
PostCtrlMessage(IDC_SAMPLE_QUICKFADE);
return wParam;
case kcSampleStereoSep:
PostCtrlMessage(IDC_SAMPLE_STEREOSEPARATION);
return wParam;
case kcSampleSlice:
OnSampleSlice();
return wParam;
case kcSampleToggleDrawing:
OnDrawingToggle();
return wParam;
case kcSampleResize:
OnAddSilence();
...
case kcNoteOff:
PlayNote(NOTE_KEYOFF);
return wParam;
case kcNoteCut:
PlayNote(NOTE_NOTECUT);
return wParam;
}
CMainFrame *pMainFrm = CMainFrame::GetMainFrame();
if(wParam >= kcSampStartNotes && wParam <= kcSampEndNotes) {
const ModCommand::NOTE note = static_cast<ModCommand::NOTE>(wParam - kcSampStartNotes + NOTE_MIN +
                                                            pMainFrm->GetBaseOctave() * 12);
...
const ModCommand::NOTE note = static_cast<ModCommand::NOTE>(wParam - kcSampStartNoteStops + NOTE_MIN +
                                                            pMainFrm->GetBaseOctave() * 12);
if(ModCommand::IsNote(note)) {
switch (TrackerSettings::Instance().sampleEditorKeyBehaviour) {
...
constexpr
size_t NUM_CUES = mpt::array_size<decltype(sample.cues)>::size;
...
memcpy(newSample.sampleb(), sample.sampleb() + cues[i] * sample.GetBytesPerSample(),
...
BOOL CViewSample::OnMouseWheel(UINT
nFlags,
short zDelta, CPoint
pt)
{
...
return
CModScrollView::OnMouseWheel(nFlags, zDelta, pt
);
}
void CViewSample::OnXButtonUp(UINT
nFlags,
UINT nButton, CPoint
point)
{
...
pCmdUI->SetText(CMainFrame::GetInputHandler()->GetKeyTextFromCommand(kcEditUndo, _T("Undo ") + mpt::ToCString(
        pModDoc->GetSoundFile().GetCharsetInternal(), pModDoc->GetSampleUndo().GetUndoName(m_nSample))));
}
}
...
pCmdUI->SetText(CMainFrame::GetInputHandler()->GetKeyTextFromCommand(kcEditRedo, _T("Redo ") + mpt::ToCString(
        pModDoc->GetSoundFile().GetCharsetInternal(), pModDoc->GetSampleUndo().GetRedoName(m_nSample))));
}
}
INT_PTR CViewSample::OnToolHitTest(CPoint
point,
TOOLINFO *pTI
) const
{
...
break;
case ID_SAMPLE_ZOOMDOWN:
cmd = kcSampleZoomDown;
break;
case ID_SAMPLE_DRAW:
cmd = kcSampleToggleDrawing;
break;
case ID_SAMPLE_ADDSILENCE:
cmd = kcSampleResize;
break;
...
{
const ModSample &sample = GetDocument()->GetSoundFile().GetSample(
        m_nSample <= GetDocument()->GetNumSamples() ? m_nSample : 0);
...
return
CModScrollView::OnToolHitTest(point, pTI
);

...
CFileDialogEx(bool bOpenFileDialog,
              ...
              BOOL bVistaStyle,
              bool preview)
        : CFileDialog(bOpenFileDialog ? TRUE : FALSE, lpszDefExt, lpszFileName, dwFlags, lpszFilter, pParentWnd, dwSize,
                      bVistaStyle), m_fileNameBuf(65536), doPreview(preview), played(false) {
...
HRESULT hr = SHCreateItemFromParsingName(path.ToWide().c_str(), nullptr, IID_IShellItem,
...
CFileDialogEx dlg(m_load,
                  ...
                  OFN_EXPLORER | OFN_NOCHANGEDIR | OFN_HIDEREADONLY | OFN_ENABLESIZING | OFN_PATHMUSTEXIST |
                  OFN_FILEMUSTEXIST | (m_multiSelect ? OFN_ALLOWMULTISELECT : 0) |
                  (m_load ? 0 : (OFN_OVERWRITEPROMPT | OFN_NOREADONLYRETURN)),
                  m_extFilter.c_str(),
                  parent != nullptr ? parent : CMainFrame::GetMainFrame(),
                  0,
                  (mpt::OS::Windows::IsWine() ||
                   mpt::OS::Windows::Version::Current().IsBefore(mpt::OS::Windows::Version::WinVista)) ? FALSE : TRUE,
...
int CALLBACK
BrowseForFolder::BrowseCallbackProc(HWND
hwnd,
UINT uMsg, LPARAM /*lParam*/, LPARAM
lpData)
{
if(uMsg ==
BFFM_INITIALIZED &&lpData
!= NULL)
{

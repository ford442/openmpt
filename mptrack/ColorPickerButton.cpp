...
SetWindowText(MPT_TFORMAT("Colour: {}% red, {}% green, {}% blue")
                      (Util::muldivr(GetRValue(color), 100, 255), Util::muldivr(GetGValue(color), 100, 255),
                       Util::muldivr(GetBValue(color), 100, 255)).c_str());
Invalidate(FALSE);
}
...
std::stable_sort(colors.begin(), colors.end(),
                 [&usedColors](COLORREF l, COLORREF r) { return usedColors.count(l) > usedColors.count(r); });
}

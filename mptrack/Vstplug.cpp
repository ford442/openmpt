...
static VstTimeInfo
g_timeInfoFallback = {0};
#ifdef MPT_ALL_LOGGING
#define VST_LOG
#endif
using VstCrash = Windows::SEH::Code;
bool CVstPlugin::MaskCrashes()
noexcept
{
return
m_maskCrashes;
...
AEffect *
CVstPlugin::LoadPlugin(bool maskCrashes, VSTPluginLib &plugin, HMODULE &library, bool forceBridge, bool forceLegacy) {
...
// Try normal loading
if(!isNative) {
Reporting::Error("Could not locate the plugin bridge executable, which is required for running non-native plugins.",
                 "OpenMPT Plugin Bridge");
return nullptr;
}
...
const CString msg =
        MPT_CFORMAT(
                "The following error occurred while trying to load\n{}\n\n{}\n\nDo you want to try to load the plugin natively?")
...
CVstPluginManager::ReportPlugException(
        U_("Plugin initialization failed. This may be caused by loading too many plugins.\nTry activating the Plugin Bridge for this plugin."));
}
#ifdef _DEBUG
...
CVstPlugin::MasterCallBack(AEffect *effect, VstOpcodeToHost opcode, int32 index, intptr_t value, void *ptr, float opt) {
#ifdef VST_LOG
...
pVstPlugin->AutomateParameter((PlugParamIndex)
index);
}
return 0;
...
BrowseForFolder dlg(mpt::PathString::FromLocale(fileSel.initialPath != nullptr ? fileSel.initialPath : ""),
                    mpt::ToCString(mpt::Charset::Locale, fileSel.title != nullptr ? fileSel.title : ""));
if(!dlg.Show(GetEditor()))
return 0;
...
CVstPlugin::CVstPlugin(bool maskCrashes, HMODULE hLibrary, VSTPluginLib &factory, SNDMIXPLUGIN &mixStruct,
                       AEffect &effect, CSoundFile &sndFile)
        : IMidiPlugin(factory, sndFile, &mixStruct), m_maskCrashes(maskCrashes), m_Effect(effect), timeInfo{},
          isBridged(!memcmp(&effect.reservedForHost2, "OMPT", 4)), m_hLibrary(hLibrary),
          m_nSampleRate(sndFile.GetSampleRate()), m_isInitialized(false), m_needIdle(false) {
...
bool CVstPlugin::CanAutomateParameter(PlugParamIndex
index)
{
return (
Dispatch(effCanBeAutomated, index,
0, nullptr, 0.0f) != 0);
}
...
intptr_t
CVstPlugin::DispatchSEH(bool maskCrashes, AEffect *effect, VstOpcodeToPlugin opCode, int32 index, intptr_t value,
                        void *ptr, float opt, unsigned long &exception) {
...
Dispatch(effSetProgramName, 0, 0,
         const_cast<char *>(mpt::ToCharset(mpt::Charset::Locale, name.Left(kVstMaxProgNameLen)).c_str()), 0.0f);
}
CString CVstPlugin::GetProgramName(int32
program)
{
// kVstMaxProgNameLen is 24... too short for some plugins, so use at least 256 bytes.
...
{
if(
Dispatch(effGetProgramNameIndexed, program,
-1 /*category*/, rawname.
data(),
0) != 1)
{
...
return
mpt::ToCString(mpt::Charset::Locale, rawname
.
data()
);
}
void CVstPlugin::SetCurrentProgram(int32
nIndex)
{
if(m_Effect.numPrograms > 0)
{
...
void CVstPlugin::BeginSetProgram(int32
program)
{
...
void CVstPlugin::BeginGetProgram(int32
program)
{
if(program != -1)
Dispatch(effSetProgram,
0, program, nullptr, 0);
if(isBridged)
Dispatch(effVendorSpecific, kVendorOpenMPT, kBeginGetProgram, nullptr,
0);
}
...
PlugParamValue CVstPlugin::GetParameter(PlugParamIndex
nIndex)
{
float fResult = 0;
if(nIndex<m_Effect.
...
void CVstPlugin::SetParameter(PlugParamIndex
nIndex,
PlugParamValue fValue
)
{
DWORD exception = 0;
if(nIndex<m_Effect.
...
CString CVstPlugin::GetParamPropertyString(int32
param,
Vst::VstOpcodeToPlugin opcode
)
{
...
std::vector<char> s(256, 0);
Dispatch(opcode, param,
0, s.
data(),
0);
return
mpt::ToCString(mpt::Charset::Locale, s
.
data()
);
}
...
CString CVstPlugin::GetParamName(PlugParamIndex
param)
{
VstParameterProperties properties{};
if(param<m_Effect.
numParams &&Dispatch(effGetParameterProperties, param, 0, &properties, 0.0f)
== 1)
{
mpt::String::SetNullTerminator(properties
.label);
return
mpt::ToCString(mpt::Charset::Locale, properties
.label);
} else
{
return
GetParamPropertyString(param, effGetParamName
);
}
}
...
if(m_isResumed) {
Dispatch(effStopProcess, 0, 0, nullptr, 0.0f);
Dispatch(effMainsChanged, 0, 0, nullptr,
         0.0f); // calls plugin's suspend (theoretically, plugins should clean their buffers here, but oh well, the number of plugins which don't do this is surprisingly high.)
m_isResumed = false;
}
}
...
ReportPlugException(
        MPT_UFORMAT("The plugin threw an exception ({}) in {}. It has automatically been set to \"Bypass\".")(
                mpt::ufmt::HEX0<8>(exception), processMethod));
}
}
...
bool CVstPlugin::MidiSend(uint32
dwMidiCode)
{
...
void CVstPlugin::RestoreAllParameters(int32
program)
{
if(m_pMixStruct !=
nullptr &&m_pMixStruct
->pluginData.
size()
>= 4)
{
...
if (!
memcmp(data,
"fEvN", 4))    // 'NvEf', return value of deprecated effIdentify call
{
if ((program>=0) && (program<m_Effect.numPrograms))
{
// Bank
Dispatch(effSetChunk,
0, m_pMixStruct->pluginData.
size()
- 4, data + 4, 0);
SetCurrentProgram(program);
} else
...
void CVstPlugin::CacheProgramNames(int32
firstProg,
int32 lastProg
)
{
...
Dispatch(effVendorSpecific, kVendorOpenMPT, kCacheProgramNames, offsets,
0.0f);
}
}
// Cache parameter names for plugin bridge
void CVstPlugin::CacheParameterNames(int32
firstParam,
int32 lastParam
)
{
...
Dispatch(effVendorSpecific, kVendorOpenMPT, kCacheParameterInfo, offsets,
0.0f);
}
}

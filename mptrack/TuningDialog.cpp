...
const mpt::Charset
TuningCharsetFallback = mpt::Charset::Locale;
...
const HTREEITEM temp = m_TreeCtrlTuning.InsertItem(
        (IsDeletable(pTC) ? CString(_T("loaded: ")) : CString()) + m_TuningCollectionsNames[pTC], parent, insertAfter);
...
m_CombobTuningType.SetItemData(m_CombobTuningType.AddString(_T("GroupGeometric")),
                               static_cast<uint16>(Tuning::Type::GROUPGEOMETRIC));
m_CombobTuningType.SetItemData(m_CombobTuningType.AddString(_T("Geometric")),
                               static_cast<uint16>(Tuning::Type::GEOMETRIC));
...
if(m_pActiveTuning) m_RatioMapWnd.m_nNote = m_RatioMapWnd.m_nNoteCentre + m_pActiveTuning->GetNoteRange().first +
                                            (m_pActiveTuning->GetNoteRange().last -
                                             m_pActiveTuning->GetNoteRange().first) / 2 + 1;
UpdateView();
return TRUE;
}
...
m_TreeCtrlTuning.SetItemText(treeitem, (IsDeletable(m_pActiveTuningCollection) ? CString(_T("loaded: ")) : CString()) +
                                       m_TuningCollectionsNames[m_pActiveTuningCollection]);
}
}
//<--Updating treeview
...
m_ButtonRemove.EnableWindow(((m_pActiveTuning && (m_pActiveTuningCollection == m_TuningCollections[0])) ||
                             (!m_pActiveTuning && m_pActiveTuningCollection &&
                              m_pActiveTuningCollection != m_TuningCollections[0])) ? TRUE : FALSE);
...
if(m_pActiveTuning->GetType() == Tuning::Type::GROUPGEOMETRIC ||
   m_pActiveTuning->GetType() == Tuning::Type::GEOMETRIC) {
...
ON_NOTIFY(TVN_SELCHANGED,
IDC_TREE_TUNING, &CTuningDialog::OnTvnSelchangedTreeTuning)
ON_NOTIFY(TVN_DELETEITEM,
IDC_TREE_TUNING, &CTuningDialog::OnTvnDeleteitemTreeTuning)
ON_NOTIFY(NM_RCLICK,
IDC_TREE_TUNING, &CTuningDialog::OnNMRclickTreeTuning)
ON_NOTIFY(TVN_BEGINDRAG,
IDC_TREE_TUNING, &CTuningDialog::OnTvnBegindragTreeTuning)
...
else if(m_pActiveTuning->GetType() == Tuning::Type::GROUPGEOMETRIC)
m_CombobTuningType.SetCurSel(1);
else
...
ratios.push_back(std::pow(static_cast<Tuning::RATIOTYPE>(2.0),
                          static_cast<Tuning::RATIOTYPE>(n) / static_cast<Tuning::RATIOTYPE>(12)));
}
...
{
filters++;
filter += std::string("Tuning files (*") + CTuning::s_FileExtension + std::string(")|*") + CTuning::s_FileExtension +
          std::string("|");
...
BeginWaitCursor();
try {
mpt::SafeOutputFile sfout(dlg.GetFirstFile(), std::ios::binary,
...
Reporting::Message(LogWarning,
                   _T("The Scala SCL file format does not contain enough information to represent General Tunings without data loss.\n\nOpenMPT exported as much information as possible, but other software as well as OpenMPT itself will not be able to re-import the just exported Scala SCL in a way that resembles the original data completely.\n\nPlease consider additionally exporting the Tuning as an OpenMPT .tun file."),
                   _T("Tuning - Incompatible export"), this);
}
}
}
...
std::string filter = std::string("Multiple Tuning files (") + CTuning::s_FileExtension + std::string(")|*") +
                     CTuning::s_FileExtension + std::string("|");
...
try {
mpt::SafeOutputFile sfout(fileName, std::ios::binary,
                          mpt::FlushModeFromBool(TrackerSettings::Instance().MiscFlushFileBuffersOnSave));
mpt::ofstream &fout = sfout;
fout.exceptions(fout.exceptions() | std::ios::badbit | std::ios::failbit);
...
template<typename Tfile, std::size_t N>
static bool CheckMagic(Tfile &f, mpt::IO::Offset offset, const uint8(&magic)[N]) {
...
const bool bIsTc = (
        mpt::PathString::CompareNoCase(fileExt, mpt::PathString::FromUTF8(CTuningCollection::s_FileExtension)) == 0);
...
mpt::ustring name;
if(pTC->Deserialize(fin, name, TuningCharsetFallback) == Tuning::SerializationResult::Success) { // success
if(pTC->GetNumTunings() == 1) {
Reporting::Message(LogInformation,
                   U_("- Tuning Collection with a Tuning file extension (.tun) detected. It only contains a single Tuning, importing the file as a Tuning.\n"),
                   this);
...
Reporting::Message(LogNotification,
                   U_("- Tuning Collection with a Tuning file extension (.tun) detected. It only contains multiple Tunings, importing the file as a Tuning Collection.\n"),
                   this);
// ok
}
} else {
...
pTC = new CTuningCollection();
mpt::ustring name;
if(pTC->Deserialize(fin, name, TuningCharsetFallback) != Tuning::SerializationResult::Success) { // failure
delete pTC;
pTC = nullptr;
// fail
} else {
...
Reporting::Message(LogNotification, _T("For the time being Drag and Drop is only supported for tuning instances."),
                   this);
return;
}
...
SetCursor((hItem == NULL || m_rParentDialog.CanDrop(hItem) == nullptr) ? CMainFrame::curNoDrop2
                                                                       : CMainFrame::curDragging);
}
...
return MPT_CFORMAT("OpenMPT supports importing scl-files with at most {} notes")(
        mpt::cfmt::val(s_nSclImportMaxNoteCount));
...
CTuningDialog::EnSclImport
CTuningDialog::ImportScl(const mpt::PathString &filename, const mpt::ustring &name, std::unique_ptr <CTuning> &result) {
...
CTuningDialog::EnSclImport
CTuningDialog::ImportScl(std::istream &iStrm, const mpt::ustring &name, std::unique_ptr <CTuning> &result) {
MPT_ASSERT(result == nullptr);
result = nullptr;
std::string str;
std::string filename;

...
OPLInstrDlg::OPLInstrDlg(CWnd &parent, const CSoundFile &sndFile)
        : m_parent(parent), m_sndFile(sndFile) {
...
static uint8 KeyScaleLevel(uint8
kslVolume)
{
...
patch[2 + op] = static_cast<uint8>((63 - m_volume[op].GetPos()) |
                                   KeyScaleLevel(static_cast<uint8>(m_levelScaling[op].GetPos() << 6)));
...
static constexpr const char *feedback[] = {"disabled", "\xCF\x80/16", "\xCF\x80/8", "\xCF\x80/4", "\xCF\x80/2",
                                           "\xCF\x80", "2\xCF\x80", "4\xCF\x80"};
static constexpr const TCHAR
*ksl[] = {_T("disabled"), _T("1.5 dB / octave"), _T("3 dB / octave"), _T("6 dB / octave")};
mpt::tstring text;

...
return (octaveLink && (octaveModifier != 0)) || (splitInstrument != 0) || (splitVolume != 0);
}
...
PlayNoteParam &LoopStart(SmpLength loopStart) {
m_loopStart = loopStart;
return *this;
}
PlayNoteParam &LoopEnd(SmpLength loopEnd) {
m_loopEnd = loopEnd;
return *this;
}
PlayNoteParam &Offset(SmpLength sampleOffset) {
m_sampleOffset = sampleOffset;
return *this;
}
PlayNoteParam &Volume(int32 volume) {
m_volume = volume;
return *this;
}
PlayNoteParam &Sample(SAMPLEINDEX sample) {
m_sample = sample;
return *this;
}
PlayNoteParam &Instrument(INSTRUMENTINDEX instr) {
m_instr = instr;
return *this;
}
PlayNoteParam &Channel(CHANNELINDEX channel) {
...
/*[[deprecated]]*/ void AddToLog(const std::string &text) const {
AddToLog(LogInformation, mpt::ToUnicode(mpt::Charset::Locale, text));
}
/*[[deprecated]]*/ void AddToLog(const char *text) const {
AddToLog(LogInformation, mpt::ToUnicode(mpt::Charset::Locale, text ? text : ""));
}
...
void SetNotifications(FlagSet <Notification::Type> type, Notification::Item item = 0) {
m_notifyType = type;
m_notifyItem = item;
}
...
INSTRUMENTINDEX ReArrangeInstruments(const std::vector <INSTRUMENTINDEX> &newOrder,
...
INSTRUMENTINDEX
InsertInstrument(SAMPLEINDEX sample = SAMPLEINDEX_INVALID, INSTRUMENTINDEX duplicateSource = INSTRUMENTINDEX_INVALID,
                 bool silent = false);
...
bool NoteOff(UINT note, bool fade = false, INSTRUMENTINDEX ins = INSTRUMENTINDEX_INVALID,
             CHANNELINDEX currentChn = CHANNELINDEX_INVALID);
...
bool MoveOrder(ORDERINDEX nSourceNdx, ORDERINDEX nDestNdx, bool bUpdate = true, bool bCopy = false,
               SEQUENCEINDEX nSourceSeq = SEQUENCEINDEX_INVALID, SEQUENCEINDEX nDestSeq = SEQUENCEINDEX_INVALID);
...
void
OnFileWaveConvert(ORDERINDEX nMinOrder, ORDERINDEX nMaxOrder, const std::vector<EncoderFactoryBase *> &encFactories);
...
CString GetPatternViewInstrumentName(INSTRUMENTINDEX nInstr, bool bEmptyInsteadOfNoName = false,

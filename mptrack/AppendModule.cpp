...
std::vector <INSTRUMENTINDEX> instrMapping(
        (source.GetNumInstruments() ? source.GetNumInstruments() : source.GetNumSamples()) + 1,
...
m_SndFile.m_MixPlugins[pluginMapping[i + 1] - 1].SetOutputPlugin(
        pluginMapping[source.m_MixPlugins[i].GetOutputPlugin() + 1] - 1);
}
}
}
#endif // NO_PLUGINS
...
for(const auto &pat : source.Patterns)
if(pat.IsValid()) {
...
if(source.Instruments[i] != nullptr && !instrMapping[i]) {
...
patternMapping[srcPat] = InsertPattern(
        Clamp(source.Patterns[srcPat].GetNumRows(), specs.patternRowsMin, specs.patternRowsMax));
...
const bool timeSigDiffers = source.m_nDefaultRowsPerBeat != m_SndFile.m_nDefaultRowsPerBeat ||
                            source.m_nDefaultRowsPerMeasure != m_SndFile.m_nDefaultRowsPerMeasure;

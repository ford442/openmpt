...
static_assert((ORDERINDEX_INVALID
& SEQU_MASK) == ORDERINDEX_INVALID, "ORDERINDEX doesn't fit in GetItemData() parameter");
static_assert((ORDERINDEX_MAX
& SEQU_MASK) == ORDERINDEX_MAX, "ORDERINDEX doesn't fit in GetItemData() parameter");
static_assert((((SEQUENCEINDEX_INVALID
<< SEQU_SHIFT) &
~SEQU_INDICATOR
) >> SEQU_SHIFT) == SEQUENCEINDEX_INVALID, "SEQUENCEINDEX doesn't fit in GetItemData() parameter");
struct ModItem {
uint32 val1;
uint16 val2;
ModItemType type;
...
DlsItem(uint32 bank, uint32 itemData) : ModItem(MODITEM_DLSBANK_INSTRUMENT,
                                                itemData | ((bank << (DLS_HIBANKSHIFT - 16)) & DLS_HIBANKMASK),
                                                (uint16) bank) {}
...
static uint32 EncodeValueInstr(uint8 region, uint16 instr) {
return DLS_TYPEINST | (region << DLS_REGIONSHIFT) | instr;
}
};
...
static int CALLBACK
ModTreeInsLibCompareNamesProc(LPARAM
lParam1,
LPARAM lParam2, LPARAM
lParamSort);
static int CALLBACK
ModTreeInsLibCompareProc(LPARAM
lParam1,
LPARAM lParam2, LPARAM
lParamSort);
static int CALLBACK
ModTreeDrumCompareProc(LPARAM
lParam1,
LPARAM lParam2, LPARAM
lParamSort);
...
return info ? &info->modDoc : nullptr;
}

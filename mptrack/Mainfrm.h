...
static HMENU
CreateFileMenu(const size_t maxCount, std::vector <mpt::PathString> &paths, const mpt::PathString &folderName,
               const uint16 idRangeBegin);
...
BOOL ResetSoundCard();
BOOL SetupSoundCard(SoundDevice::Settings deviceSettings, SoundDevice::Identifier deviceIdentifier,
                    SoundDeviceStopMode stoppedMode, bool forceReset = false);
...
nID);
afx_msg LRESULT
OnUpdatePosition(WPARAM, LPARAM
lParam);
afx_msg LRESULT
OnUpdateViews(WPARAM
modDoc,
LPARAM hint
);
afx_msg LRESULT
OnSetModified(WPARAM
modDoc, LPARAM);
...
afx_msg LRESULT
OnInvalidatePatterns(WPARAM, LPARAM
);
afx_msg LRESULT
...
nMaxItemsInExampleModulesMenu = 50;
static constexpr size_t
nMaxItemsInTemplateModulesMenu = 50;

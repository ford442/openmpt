...
IDC_TAB1,    &CChannelManagerDlg::OnTabSelchange)
ON_WM_LBUTTONDBLCLK()
ON_WM_RBUTTONDBLCLK()
END_MESSAGE_MAP()
...
if(hint.ToType<GeneralHint>().GetType()[HINT_MODCHANNELS] && m_quickChannelProperties.m_hWnd &&
   pHint != &m_quickChannelProperties)
...
SetWindowPos(nullptr, 0, 0, window.Width(), window.Height() + (m_buttonHeight - chnSizeY) * lines,
...
button->SetWindowPos(nullptr, btn.left, client.Height() - btn.Height() - MulDiv(3, dpiY, 96), 0, 0,
...
::BitBlt(pDC.hdc, rcPaint.left, rcPaint.top, rcPaint.Width(), rcPaint.Height(), bdc, rcPaint.left, rcPaint.top,
         SRCCOPY);
BLENDFUNCTION ftn;
...
client.SetRect(client.left + MulDiv(2, dpiX, 96), client.top + MulDiv(32, dpiY, 96), client.right - MulDiv(2, dpiX, 96),
               client.bottom - MulDiv(24, dpiY, 96));
// Draw background
{
const auto bgIntersected = client &
                           pDC.rcPaint;  // In case of partial redraws, FillRect may still draw into areas that are not part of the redraw area and thus make some buttons disappear
...
const COLORREF highlight = GetSysColor(COLOR_HIGHLIGHT), red = RGB(192, 96, 96), green = RGB(96, 192,
                                                                                             96), redBright = RGB(218,
                                                                                                                  163,
                                                                                                                  163), greenBright = RGB(
        163, 218, 163);
...
s = MPT_CFORMAT("{}: {}")(sourceChn + 1,
                          mpt::ToCString(sndFile.GetCharsetInternal(), sndFile.ChnSettings[sourceChn].szName));
else
...
// Rearrange channels
const auto IsSelected = std::bind(&decltype(select)
::test, &select, std::placeholders::_1);
...
const CHANNELINDEX selectedBeforeDropChn = static_cast<CHANNELINDEX>(std::count_if(pattern.begin(),
                                                                                   pattern.begin() + dropChn,
                                                                                   IsSelected));
...
CHANNELINDEX chn;
CRect rect;
if(m_ModDoc != nullptr && (m_ModDoc->GetModType() & (MOD_TYPE_XM | MOD_TYPE_IT | MOD_TYPE_MPT)) &&
...
if(m_buttonAction == kUndetermined)
m_buttonAction = (rec == RecordGroup::NoGroup ||
                  rec != (button == CM_BT_LEFT ? RecordGroup::Group1 : RecordGroup::Group2)) ? kAction1 : kAction2;

...
ON_MESSAGE(WM_MOD_KEYCOMMAND, &CAbstractVstEditor::OnCustomKeyMsg
) //rewbs.customKeys
...
CInputDlg dlg(this, _T("Input parameter randomization amount (0 = no change, 100 = completely random)"), 0.0, 100.0,
              randomFactor);
if(dlg.DoModal() == IDOK) {
...
CInputDlg dlg(this, _T("New name for this plugin instance:"), mpt::ToCString(plugin.GetName()),
              static_cast<int32>(std::size(plugin.Info.szName.buf)));
if(dlg.DoModal() == IDOK) {
...
bool hasCustomName = (m_VstPlugin.m_pMixStruct->GetName() != U_("")) &&
                     (m_VstPlugin.m_pMixStruct->GetName() != m_VstPlugin.m_pMixStruct->GetLibraryName());
if(hasCustomName)
...
switch (wParam) {
case kcVSTGUIPrevPreset:
OnSetPreviousVSTPreset();
return wParam;
case kcVSTGUIPrevPresetJump:
OnVSTPresetBackwardJump();
return wParam;
case kcVSTGUINextPreset:
OnSetNextVSTPreset();
return wParam;
case kcVSTGUINextPresetJump:
OnVSTPresetForwardJump();
return wParam;
case kcVSTGUIRandParams:
OnRandomizePreset();
return wParam;
case kcVSTGUIToggleRecordParams:
OnRecordAutomation();
return wParam;
case kcVSTGUIToggleSendKeysToPlug:
OnPassKeypressesToPlug();
return wParam;
case kcVSTGUIBypassPlug:
OnBypassPlug();
return wParam;
}
...
const ModCommand::NOTE note = static_cast<ModCommand::NOTE>(wParam - kcVSTGUIStartNotes + NOTE_MIN +
...
CModDoc *pModDoc = m_VstPlugin.GetModDoc();
CMainFrame *pMainFrm = CMainFrame::GetMainFrame();
const ModCommand::NOTE note = static_cast<ModCommand::NOTE>(wParam - kcVSTGUIStartNoteStops + NOTE_MIN +
                                                            pMainFrm->GetBaseOctave() * 12);
if(ModCommand::IsNote(note)) {
...
if(!m_VstPlugin.IsInstrument() || m_VstPlugin.GetSoundFile().GetModSpecifications().instrumentsMax == 0 ||
   Reporting::Confirm(
           _T("You need to assign an instrument to this plugin before you can play notes from here.\nCreate a new instrument and assign this plugin to the instrument?"),
           false, false, this) == cnfNo) {
return false;
} else {
OnCreateInstrument();
...
parent.AppendMenu(MF_STRING | (p == curProg ? MF_CHECKED : MF_UNCHECKED) | splitMenuFlag, ID_PRESET_SET + id,
                  programName);
}
}
...
CString name = MPT_CFORMAT("FX{}: {}")(mpt::cfmt::dec0<2>(plug->m_nSlot + 1),
                                       mpt::ToCString(plug->m_pMixStruct->GetName()));
...
CString name = MPT_CFORMAT("Chn{}: {}")(mpt::cfmt::dec0<2>(chn + 1),
                                        mpt::ToCString(sndFile.GetCharsetInternal(), sndFile.ChnSettings[chn].szName));
...
CString name = MPT_CFORMAT("Ins{}: {}")(mpt::cfmt::dec0<2>(ins),
                                        mpt::ToCString(sndFile.GetCharsetInternal(), sndFile.GetInstrumentName(ins)));
...
m_OptionsMenu.AppendMenu(MF_STRING | (m_VstPlugin.m_recordAutomation ? MF_CHECKED : 0),
                         ID_PLUG_RECORDAUTOMATION,
                         ih->GetKeyTextFromCommand(kcVSTGUIToggleRecordParams, _T("Record &Parameter Changes")));
//Record MIDI Out
m_OptionsMenu.AppendMenu(MF_STRING | (m_VstPlugin.m_recordMIDIOut ? MF_CHECKED : 0),
                         ID_PLUG_RECORD_MIDIOUT, ih->GetKeyTextFromCommand(kcVSTGUIToggleRecordMIDIOut,
                                                                           _T("Record &MIDI Out to Pattern Editor")));
//Pass on keypresses
m_OptionsMenu.AppendMenu(MF_STRING | (m_VstPlugin.m_passKeypressesToPlug ? MF_CHECKED : 0),
                         ID_PLUG_PASSKEYS,
                         ih->GetKeyTextFromCommand(kcVSTGUIToggleSendKeysToPlug, _T("Pass &Keys to Plugin")));

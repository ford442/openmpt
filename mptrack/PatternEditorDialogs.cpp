...
static constexpr
EffectCommand ExtendedCommands[] = {CMD_OFFSET, CMD_PATTERNBREAK, CMD_POSITIONJUMP, CMD_TEMPO, CMD_FINETUNE,
                                    CMD_FINETUNE_SMOOTH};
...
void getXParam(ModCommand::COMMAND command, PATTERNINDEX nPat, ROWINDEX nRow, CHANNELINDEX nChannel,
               const CSoundFile &sndFile, UINT &xparam, UINT &multiplier) {
UINT xp = 0, mult = 1;
...
if(m.vol == 0 && smp != 0) {
xp = Util::muldivr_unsigned(sndFile.GetSample(smp).nLength, pattern.GetpModCommand(nRow, nChannel)->param * mult + xp,
                            256u << (8u * (std::max(uint32(2), n) - 1u)));
mult = 0;
...
GetDlgItem(IDC_BUTTON1)->EnableWindow(
        IsDlgButtonChecked(IDC_CHECK1) && modDoc.GetSoundFile().m_nTempoMode == TempoMode::Modern);
}
...
// Enable signature
ROWINDEX nNewBeat = (ROWINDEX) GetDlgItemInt(IDC_ROWSPERBEAT, NULL, FALSE), nNewMeasure = (ROWINDEX) GetDlgItemInt(
        IDC_ROWSPERMEASURE, NULL, FALSE);
if(nNewBeat != pattern.GetRowsPerBeat() || nNewMeasure != pattern.GetRowsPerMeasure() ||
   m_tempoSwing != pattern.GetTempoSwing()) {
...
ROWINDEX lastRow = resizeAtEnd ? sndFile.Patterns[m_nPattern].GetNumRows() : sndFile.Patterns[m_nPattern].GetNumRows() -
                                                                             newSize;
...
resize = (Reporting::Confirm(MPT_AFORMAT("Data at the {} of the pattern will be lost.\nDo you want to continue?")(
        resizeAtEnd ? "end" : "start"), "Shrink Pattern") == cnfYes);
break;
}
}
}
...
modDoc.GetPatternUndo().PrepareUndo(m_nPattern, 0, 0, sndFile.Patterns[m_nPattern].GetNumChannels(),
                                    sndFile.Patterns[m_nPattern].GetNumRows(), "Resize");
...
AppendNotesToControl(m_CbnShortcut, NOTE_MIN,
                     NOTE_MIN + static_cast<int>(kcVPEndChords) - static_cast<int>(kcVPStartChords));
...
CheckDlgButton(IDC_PATTERN_OCTAVELINK,
               (m_Settings.octaveLink && m_Settings.octaveModifier != 0) ? BST_CHECKED : BST_UNCHECKED);
...
m_CbnSplitInstrument.InitStorage(
        1 + (sndFile.GetNumInstruments() ? sndFile.GetNumInstruments() : sndFile.GetNumSamples()), 16);
...
m_Settings.splitInstrument = static_cast<ModCommand::INSTR>(m_CbnSplitInstrument.GetItemData(
        m_CbnSplitInstrument.GetCurSel()));
}
...
0, 0xFFFF, &QuickChannelProperties::OnToolTipText)
END_MESSAGE_MAP()
...
rect.MoveToXY(
        ...
        Clamp(static_cast<int>(position.y) - rect.Height() / 2, 0,
              static_cast<int>(screenRect.bottom) - rect.Height()));
MoveWindow(rect);

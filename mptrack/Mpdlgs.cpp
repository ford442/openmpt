...
COptionsSoundcard::COptionsSoundcard(SoundDevice::Identifier deviceIdentifier)
        : CPropertyPage(IDD_OPTIONS_SOUNDCARD), m_InitialDeviceIdentifier(deviceIdentifier) {
return;
}
...
newDynamicCaps = theApp.GetSoundDevicesManager()->GetDeviceDynamicCaps(dev,
                                                                       TrackerSettings::Instance().GetSampleRates(),
                                                                       CMainFrame::GetMainFrame(),
                                                                       CMainFrame::GetMainFrame()->gpSoundDevice, true);
...
TrackerSettings::Instance().m_SoundShowDeprecatedDevices = (IsDlgButtonChecked(IDC_CHECK_SOUNDCARD_SHOWALL) ==
                                                            BST_CHECKED);
...
if(theApp.GetSoundDevicesManager()->IsDeviceUnavailable(m_CurrentDeviceInfo.GetIdentifier()) ||
   !m_CurrentDeviceCaps.CanUpdateInterval) {
...
CheckDlgButton(IDC_CHECK7,
               TrackerSettings::Instance().m_SoundSettingsOpenDeviceAtStartup ? BST_CHECKED : BST_UNCHECKED);
}
...
CheckDlgButton(IDC_CHECK_SOUNDCARD_SHOWALL,
               TrackerSettings::Instance().m_SoundShowDeprecatedDevices ? BST_CHECKED : BST_UNCHECKED);
...
if(static_cast<SoundDevice::Manager::GlobalID>(cbi.lParam) ==
   theApp.GetSoundDevicesManager()->GetGlobalID(m_CurrentDeviceInfo.GetIdentifier())) {
...
GetDlgItem(IDC_STATIC_RECORDING)->ShowWindow(
        TrackerSettings::Instance().m_SoundShowRecordingSettings ? SW_SHOW : SW_HIDE);
...
if(m_CurrentDeviceCaps.CanInput &&
   ((m_CurrentDeviceCaps.HasNamedInputSources && m_CurrentDeviceDynamicCaps.inputSourceNames.size() > 0) ||
    !m_CurrentDeviceCaps.HasNamedInputSources)) {
...
const int pos = (int) ::SendMessageW(m_CbnRecordingSource.m_hWnd, CB_ADDSTRING, 0,
                                     (LPARAM) m_CurrentDeviceDynamicCaps.inputSourceNames[ch].second.c_str());
...
SampleFormat sampleFormat = SampleFormat::FromInt(
        static_cast<int>(m_CbnSampleFormat.GetItemData(m_CbnSampleFormat.GetCurSel())));
...
{
const int pos = (int) ::SendMessageW(combo->m_hWnd, CB_ADDSTRING, 0,
                                     (LPARAM) m_CurrentDeviceDynamicCaps.channelNames[dch].c_str());
...
if(n >= 0) {
SetDevice(theApp.GetSoundDevicesManager()->FindDeviceInfo(
        static_cast<SoundDevice::Manager::GlobalID>(m_CbnDevice.GetItemData(n))).GetIdentifier());
UpdateDevice();
OnSettingsChanged();
}
}
...
theApp.GetSoundDevicesManager()->OpenDriverSettings(
        theApp.GetSoundDevicesManager()->FindDeviceInfo(
                static_cast<SoundDevice::Manager::GlobalID>(m_CbnDevice.GetItemData(
                        m_CbnDevice.GetCurSel()))).GetIdentifier(),
        CMainFrame::GetMainFrame(),
...
if(static_cast<int>(m_CbnChannelMapping[hostChannel].GetItemData(m_CbnChannelMapping[hostChannel].GetCurSel())) ==
   deviceChannel) {
used = true;
break;
}
...
CheckDlgButton(IDC_CHECK4,
               m_CurrentDeviceCaps.CanExclusiveMode && m_Settings.ExclusiveMode ? BST_CHECKED : BST_UNCHECKED);
CheckDlgButton(IDC_CHECK5, m_CurrentDeviceCaps.CanBoostThreadPriority && m_Settings.BoostThreadPriority ? BST_CHECKED
                                                                                                        : BST_UNCHECKED);
CheckDlgButton(IDC_CHECK9,
               m_CurrentDeviceCaps.CanUseHardwareTiming && m_Settings.UseHardwareTiming ? BST_CHECKED : BST_UNCHECKED);
if(theApp.GetSoundDevicesManager()->IsDeviceUnavailable(m_CurrentDeviceInfo.GetIdentifier())) {
...
m_Settings.UpdateInterval = Clamp(m_Settings.UpdateInterval, m_CurrentDeviceCaps.UpdateIntervalMin,
                                  m_CurrentDeviceCaps.UpdateIntervalMax);
m_CbnUpdateIntervalMS.SetWindowText(PrintTime(m_Settings.UpdateInterval));
}
// Channel Mapping
{
...
// Recording
{
if(TrackerSettings::Instance().m_SoundShowRecordingSettings && m_CurrentDeviceCaps.CanInput &&
   ((m_CurrentDeviceCaps.HasNamedInputSources && m_CurrentDeviceDynamicCaps.inputSourceNames.size() > 0) ||
    !m_CurrentDeviceCaps.HasNamedInputSources)) {
...
CMainFrame::GetMainFrame()->SetupSoundCard(m_Settings, m_CurrentDeviceInfo.GetIdentifier(),
                                           (SoundDeviceStopMode) m_CbnStoppedMode.GetCurSel());
...
s += MPT_UFORMAT("Buffer: {}% ({}/{})\r\n")((bufferAttributes.Latency > 0.0) ? mpt::saturate_round<int64>(
        stats.InstantaneousLatency / bufferAttributes.Latency * 100.0) : 0,
                                            (stats.LastUpdateInterval > 0.0) ? mpt::saturate_round<int64>(
                                                    bufferAttributes.Latency / stats.LastUpdateInterval) : 0,
                                            bufferAttributes.NumBuffers);
} else {
s += MPT_UFORMAT("Buffer: {}%\r\n")((bufferAttributes.Latency > 0.0) ? mpt::saturate_round<int64>(
        stats.InstantaneousLatency / bufferAttributes.Latency * 100.0) : 0);
}
s += MPT_UFORMAT("Latency: {} ms (current: {} ms, {} frames)\r\n")(mpt::ufmt::fix(bufferAttributes.Latency * 1000.0, 1),
                                                                   mpt::ufmt::fix(stats.InstantaneousLatency * 1000.0,
                                                                                  1), mpt::saturate_round<int64>(
                stats.InstantaneousLatency * samplerate));
s += MPT_UFORMAT("Period: {} ms (current: {} ms, {} frames)\r\n")(
        mpt::ufmt::fix(bufferAttributes.UpdateInterval * 1000.0, 1),
        mpt::ufmt::fix(stats.LastUpdateInterval * 1000.0, 1),
        mpt::saturate_round<int64>(stats.LastUpdateInterval * samplerate));
#if 0
...
static constexpr std::pair<const TCHAR *, Resampling::AmigaFilter>
Filters[] =
        {
...
// volume ramping
{
m_CEditRampUp.SetWindowText(
        mpt::ToCString(mpt::ufmt::val(TrackerSettings::Instance().GetMixerSettings().GetVolumeRampUpMicroseconds())));
m_CEditRampDown.SetWindowText(
        mpt::ToCString(mpt::ufmt::val(TrackerSettings::Instance().GetMixerSettings().GetVolumeRampDownMicroseconds())));
...
// soft pan
{
CheckDlgButton(IDC_CHECK_SOFTPAN,
               (TrackerSettings::Instance().MixerFlags & SNDMIX_SOFTPANNING) ? BST_CHECKED : BST_UNCHECKED);
}
...
// resampler mode
{
TrackerSettings::Instance().ResamplerMode = static_cast<ResamplingMode>(m_CbnResampling.GetItemData(
        m_CbnResampling.GetCurSel()));
}
...
TrackerSettings::Instance().ResamplerEmulateAmiga = static_cast<Resampling::AmigaFilter>(m_CbnAmigaType.GetItemData(
        m_CbnAmigaType.GetCurSel()));
...
SetDlgItemText(IDC_BUTTON1 + i,
               mpt::ToCString(mpt::Charset::Locale, TrackerSettings::Instance().m_EqUserPresets[i].szName));
}
}
...
static constexpr std::pair<const TCHAR *, RecordAftertouchOptions>
aftertouchOptions[] =
        {
...
static constexpr std::pair<const TCHAR *, uint32>
quantizeOptions[] =
        {
                {_T("1/4th Notes"),  4},
                {_T("1/6th Notes"),  6},
...
int item = m_InputDevice.AddString(
        theApp.GetFriendlyMIDIPortName(mpt::ToCString(mpt::String::ReadWinBuf(mic.szPname)), true));
...
TrackerSettings::Instance().aftertouchBehaviour = static_cast<RecordAftertouchOptions>(m_ATBehaviour.GetItemData(
        m_ATBehaviour.GetCurSel()));
...
int index;
index = m_CbnPulseAudio.AddString(_T("Auto"));
m_CbnPulseAudio.SetItemData(index, 1);
index = m_CbnPulseAudio.AddString(_T("Enabled"));
m_CbnPulseAudio.SetItemData(index, 2);
index = m_CbnPulseAudio.AddString(_T("Disabled"));
m_CbnPulseAudio.SetItemData(index, 0);
...
index = m_CbnPortAudio.AddString(_T("Auto"));
m_CbnPortAudio.SetItemData(index, 1);
index = m_CbnPortAudio.AddString(_T("Enabled"));
m_CbnPortAudio.SetItemData(index, 2);
index = m_CbnPortAudio.AddString(_T("Disabled"));
m_CbnPortAudio.SetItemData(index, 0);
for(index = 0; index < 3; ++index) {
...
index = m_CbnRtAudio.AddString(_T("Auto"));
m_CbnRtAudio.SetItemData(index, 1);
index = m_CbnRtAudio.AddString(_T("Enabled"));
m_CbnRtAudio.SetItemData(index, 2);
index = m_CbnRtAudio.AddString(_T("Disabled"));
m_CbnRtAudio.SetItemData(index, 0);
for(index = 0; index < 3; ++index) {
...
TrackerSettings::Instance().WineSupportEnablePulseAudio = static_cast<int32>(m_CbnPulseAudio.GetItemData(
        m_CbnPulseAudio.GetCurSel()));
TrackerSettings::Instance().WineSupportEnablePortAudio = static_cast<int32>(m_CbnPortAudio.GetItemData(
        m_CbnPortAudio.GetCurSel()));
TrackerSettings::Instance().WineSupportEnableRtAudio = static_cast<int32>(m_CbnRtAudio.GetItemData(
        m_CbnRtAudio.GetCurSel()));
CPropertyPage::OnOK();
}

...
public:
KeyCombination(InputTargetContext context = kCtxAllContexts, FlagSet <Modifiers> modifier = ModNone, UINT key = 0,
               FlagSet <KeyEventType> type = kKeyEventNone)
        : ctx(context), mod(modifier), code(static_cast<uint8>(key)), event(type) {
}
...
constexpr InputTargetContext
Context() const { return ctx; }
void Modifier(FlagSet <Modifiers> modifier) { mod = modifier; }
constexpr FlagSet<Modifiers>
Modifier() const { return mod; }
...
constexpr UINT
KeyCode() const { return code; }
void EventType(FlagSet <KeyEventType> type) { event = type; }
constexpr FlagSet<KeyEventType>
EventType() const { return event; }
...
IsConflicting(KeyCombination kc, CommandID cmd, bool checkEventConflict = true) const;
...
return (cmd != kcNull) ? static_cast<int>(m_commands[cmd].kcList.size()) : 0;
}

...
CString str;
str.Format(_T("Can't apply change: Number of channels should be between %u and %u."), specs.channelsMin,
           specs.channelsMax);
...
std::vector <ModChannel> chns(std::begin(m_SndFile.m_PlayState.Chn),
                              std::begin(m_SndFile.m_PlayState.Chn) + oldNumChannels);
std::vector <ModChannelSettings> settings(std::begin(m_SndFile.ChnSettings),
                                          std::begin(m_SndFile.ChnSettings) + oldNumChannels);
...
INSTRUMENTINDEX
CModDoc::ReArrangeInstruments(const std::vector <INSTRUMENTINDEX> &newOrder, deleteInstrumentSamples removeSamples) {
...
ins.name = mpt::ToCharset(m_SndFile.GetCharsetInternal(),
                          MPT_UFORMAT("{}: {}")(plug + 1, m_SndFile.m_MixPlugins[plug].GetName()));
...
result = Reporting::Confirm("Remove samples associated with an instrument if they are unused?", "Removing instrument",
                            true);
if(result == cnfCancel) {
return false;
}
...
bool instrumentsLeft = std::find_if(std::begin(m_SndFile.Instruments), std::end(m_SndFile.Instruments),
                                    [](ModInstrument *ins) { return ins != nullptr; }) !=
                       std::end(m_SndFile.Instruments);
if(!instrumentsLeft)
...
bool CModDoc::MoveOrder(ORDERINDEX sourceOrd, ORDERINDEX destOrd, bool update, bool copy, SEQUENCEINDEX sourceSeq,
                        SEQUENCEINDEX destSeq) {
...
const PATTERNINDEX sourcePat =
        sourceOrd < sourceSequence.size() ? sourceSequence[sourceOrd] : sourceSequence.GetInvalidPatIndex();
...
*
pszEnvHdr = "ModPlug Tracker Envelope\r\n";
static constexpr const CHAR
*
pszEnvFmt = "%d,%d,%d,%d,%d,%d,%d,%d\r\n";
...
if(m_SndFile.Patterns[pat].WriteEffect(EffectWriter(CMD_GLOBALVOLUME, mpt::saturate_cast<ModCommand::PARAM>(
        m_SndFile.m_nDefaultGlobalVolume * 64 / MAX_GLOBAL_VOLUME)).RetryNextRow())) {
result = true;
break;
}
}

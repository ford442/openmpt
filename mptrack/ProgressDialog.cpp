...
void CProgressDialog::SetRange(uint64
min,
uint64 max
)
{
...
::SendMessage(::GetDlgItem(m_hWnd, IDC_PROGRESS1), PBM_SETRANGE32,
static_cast
<uint32>(m_min
>> m_shift), static_cast
<uint32>(m_max
...
void CProgressDialog::SetProgress(uint64
progress)
{
::SendMessage(::GetDlgItem(m_hWnd, IDC_PROGRESS1), PBM_SETPOS,
static_cast
<uint32>(progress
>> m_shift), 0);
...
{
m_taskBarList->
SetProgressValue(*theApp
.m_pMainWnd, progress - m_min, m_max - m_min);
}
}

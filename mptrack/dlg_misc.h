...
CLegacyPlaybackSettingsDlg(CWnd *parent, PlayBehaviourSet &playBehaviour, MODTYPE modType)
        : ResizableDialog{IDD_LEGACY_PLAYBACK, parent}, m_playBehaviour{playBehaviour}, m_modType{modType} {
}
...
CRemoveChannelsDlg(CSoundFile &sf, CHANNELINDEX toRemove, bool showCancel = true, CWnd *parent = nullptr)
        : CDialog{IDD_REMOVECHANNELS, parent}, sndFile{sf}, m_bKeepMask(sf.GetNumChannels(), true), m_nRemove{toRemove},
...
public:
CSampleMapDlg(CSoundFile &sf, INSTRUMENTINDEX nInstr, CWnd *parent = NULL) : CDialog(IDD_EDITSAMPLEMAP, parent),
                                                                             sndFile(sf), mouseAction(
                mouseUnknown) { m_nInstrument = nInstr; }
protected:
void DoDataExchange(CDataExchange *pDX) override;
BOOL OnInitDialog() override;
void OnOK() override;
...
afx_msg LRESULT
OnKeyboardNotify(WPARAM, LPARAM
);
DECLARE_MESSAGE_MAP()
};
...
CInputDlg(CWnd *parent, const TCHAR *desc, const TCHAR *defaultString, int32 maxLength = -1) : CDialog(IDD_INPUT,
                                                                                                       parent),
                                                                                               m_description(desc),
...
CInputDlg(CWnd *parent, const TCHAR *desc, double minVal, double maxVal, double defaultNumber) : CDialog(IDD_INPUT,
                                                                                                         parent),
                                                                                                 m_description(desc),
                                                                                                 m_minValueDbl(minVal),
                                                                                                 m_maxValueDbl(maxVal),
                                                                                                 resultAsDouble(
                                                                                                         defaultNumber) {}
CInputDlg(CWnd *parent, const TCHAR *desc, float minVal, float maxVal, float defaultNumber) : CDialog(IDD_INPUT,
                                                                                                      parent),
                                                                                              m_description(desc),
                                                                                              m_minValueDbl(minVal),
                                                                                              m_maxValueDbl(maxVal),
...
CInputDlg(CWnd *parent, const TCHAR *desc, int32 minVal, int32 maxVal, int32 defaultNumber) : CDialog(IDD_INPUT,
                                                                                                      parent),
                                                                                              m_description(desc),
                                                                                              m_minValueInt(minVal),
                                                                                              m_maxValueInt(maxVal),
                                                                                              resultAsInt(
                                                                                                      defaultNumber) {}
protected:
void DoDataExchange(CDataExchange *pDX) override;

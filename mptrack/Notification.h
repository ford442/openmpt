...
PosInvalid = SmpLength(-1);    // pos[i] is not valid (if it contains sample or envelope position)
static constexpr uint32
ClipVU = 0x80000000;            // Master VU clip indicator bit (sound output has previously clipped)
...
Notification(FlagSet <Notification::Type> t = Default, Item i = 0, int64 s = 0, ROWINDEX r = 0, uint32 ti = 0,
             uint32 tir = 0, ORDERINDEX o = 0, PATTERNINDEX p = 0, uint32 x = 0, uint8 outChannels = 0,
             uint8 inChannels = 0) : timestampSamples(s), type(t), item(i), row(r), tick(ti), ticksOnRow(tir), order(o),
                                     pattern(p), mixedChannels(x), masterVUinChannels(inChannels),
                                     masterVUoutChannels(outChannels) {
masterVUin.fill(0);
masterVUout.fill(0);
pos.fill(0);
}

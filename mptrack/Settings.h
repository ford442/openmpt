...
template<>
inline SettingValue ToSettingValue(const std::string &val) {
return SettingValue(mpt::ToUnicode(mpt::Charset::Locale, val));
}
template<>
inline std::string FromSettingValue(const SettingValue &val) {
return mpt::ToCharset(mpt::Charset::Locale, val.as<mpt::ustring>());
}
template<>
inline SettingValue ToSettingValue(const mpt::lstring &val) { return SettingValue(mpt::ToUnicode(val)); }
template<>
inline mpt::lstring FromSettingValue(const SettingValue &val) { return mpt::ToLocale(val.as<mpt::ustring>()); }
#if !MPT_USTRING_MODE_WIDE
template<>
inline SettingValue ToSettingValue(const std::wstring &val) { return SettingValue(mpt::ToUnicode(val)); }
template<>
inline std::wstring FromSettingValue(const SettingValue &val) { return mpt::ToWide(val.as<mpt::ustring>()); }
#endif
template<>
inline SettingValue ToSettingValue(const CString &val) { return SettingValue(mpt::ToUnicode(val)); }
template<>
inline CString FromSettingValue(const SettingValue &val) { return mpt::ToCString(val.as<mpt::ustring>()); }
template<>
inline SettingValue ToSettingValue(const mpt::PathString &val) { return SettingValue(val.ToUnicode()); }
template<>
inline mpt::PathString FromSettingValue(const SettingValue &val) { return mpt::PathString::FromUnicode(val); }
template<>
inline SettingValue ToSettingValue(const float &val) { return SettingValue(double(val)); }
template<>
inline float FromSettingValue(const SettingValue &val) { return float(val.as<double>()); }
template<>
inline SettingValue ToSettingValue(const int64 &val) { return SettingValue(mpt::ufmt::dec(val), "int64"); }
template<>
inline int64 FromSettingValue(const SettingValue &val) { return ConvertStrTo<int64>(val.as<mpt::ustring>()); }
template<>
inline SettingValue ToSettingValue(const uint64 &val) { return SettingValue(mpt::ufmt::dec(val), "uint64"); }
template<>
inline uint64 FromSettingValue(const SettingValue &val) { return ConvertStrTo<uint64>(val.as<mpt::ustring>()); }
template<>
inline SettingValue ToSettingValue(const uint32 &val) { return SettingValue(int32(val)); }
template<>
inline uint32 FromSettingValue(const SettingValue &val) { return uint32(val.as<int32>()); }
template<>
inline SettingValue ToSettingValue(const uint16 &val) { return SettingValue(int32(val)); }
template<>
inline uint16 FromSettingValue(const SettingValue &val) { return uint16(val.as<int32>()); }
template<>
inline SettingValue ToSettingValue(const uint8 &val) { return SettingValue(int32(val)); }
template<>
inline uint8 FromSettingValue(const SettingValue &val) { return uint8(val.as<int32>()); }
template<>
inline SettingValue ToSettingValue(const LONG &val) { return SettingValue(int32(val)); }
template<>
inline LONG FromSettingValue(const SettingValue &val) { return LONG(val.as<int32>()); }
...
SettingState(const SettingValue &def)
        : value(def), defaultValue(def), dirty(false) {
return;
}
SettingState &assign(const SettingValue &other, bool setDirty = true) {
MPT_ASSERT(defaultValue.GetType() == SettingTypeNone ||
           (defaultValue.GetType() == other.GetType() && defaultValue.GetTypeTag() == other.GetTypeTag()));
if(setDirty) {
if(value != other) {
value = other;
dirty = true;
...
Setting &operator+=(const Trhs &rhs) {
T tmp = *this;
tmp += rhs;
*this = tmp;
return *this;
...
CachedSetting(SettingsContainer &conf_, mpt::ustring section, mpt::ustring key, const T &def)
        : value(def), conf(conf_), path(std::move(section), std::move(key)) {
{
...
CachedSetting(SettingsContainer &conf_, const SettingPath &path_, const T &def)
        : value(def), conf(conf_), path(path_) {
{
mpt::lock_guard<mpt::mutex> l(valueMutex);
value = conf.Read(path, def);
}
...
template<typename Trhs>
CachedSetting &operator+=(const Trhs &rhs) {
T tmp = *this;
tmp += rhs;
*this = tmp;
return *this;
...
SettingChangedNotifyGuard(SettingsContainer &conf, const SettingPath &path)
        : conf(conf), m_Path(path), m_Registered(false), m_Handler(nullptr) {
return;
}

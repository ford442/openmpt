...
static constexpr FlagSet<MODTYPE>
MOD_TYPE_MODXM = MOD_TYPE_MOD | MOD_TYPE_XM;
static constexpr FlagSet<MODTYPE>
MOD_TYPE_S3MIT = MOD_TYPE_S3M | MOD_TYPE_IT;
static constexpr FlagSet<MODTYPE>
MOD_TYPE_S3MITMPT = MOD_TYPE_S3M | MOD_TYPE_IT | MOD_TYPE_MPT;
static constexpr FlagSet<MODTYPE>
MOD_TYPE_NOMOD = MOD_TYPE_S3M | MOD_TYPE_XM | MOD_TYPE_IT | MOD_TYPE_MPT;
static constexpr FlagSet<MODTYPE>
MOD_TYPE_XMIT = MOD_TYPE_XM | MOD_TYPE_IT;
static constexpr FlagSet<MODTYPE>
MOD_TYPE_XMITMPT = MOD_TYPE_XM | MOD_TYPE_IT | MOD_TYPE_MPT;
static constexpr FlagSet<MODTYPE>
MOD_TYPE_ITMPT = MOD_TYPE_IT | MOD_TYPE_MPT;
static constexpr FlagSet<MODTYPE>
MOD_TYPE_ALL = MODTYPE(~0);
static constexpr MPTEffectInfo
gFXInfo[] =
{
{
...
if((bXX) && (bSupported)) {
pszDescription.Format(_T("%c%c%c: "), sndFile.GetModSpecifications().GetEffectLetter(command),
...
bool EffectInfo::GetEffectInfo(UINT ndx, CString *s, bool bXX, ModCommand::PARAM *prangeMin,
                               ModCommand::PARAM *prangeMax) const {
if(s) s->Empty();
...
if(gFXInfo[ndx].paramValue >= 0x30 && gFXInfo[ndx].paramValue <= 0x50)
nmax = gFXInfo[ndx].paramValue | ((sndFile.m_playBehaviour[kITVibratoTremoloPanbrello] ||
                                   sndFile.GetType() == MOD_TYPE_S3M) ? 0x03 : 0x07);
break;
...
pszName.Format(_T("SFx MIDI Macro z=%d (SF%X: %s)"), param, macroIndex,
               sndFile.m_MidiCfg.GetParameteredMacroName(macroIndex, pPlugin).GetString());
} else {
...
break;
case 0x0A:
s = _T("9A: Center surround");
break;
case 0x0B:
s = _T("9B: Quad surround");
break;
case 0x0C:
s = _T("9C: Global filters");
break;
case 0x0D:
s = _T("9D: Local filters");
break;
case 0x0E:
s = _T("9E: Play Forward");
break;
case 0x0F:
s = _T("9F: Play Backward");
break;
default:
s.Format(_T("%02X: undefined"), param);
}
} else if(((gFXInfo[ndx].effect == CMD_XFINEPORTAUPDOWN) || (gFXInfo[ndx].effect == CMD_S3MCMDEX))
          && ((gFXInfo[ndx].paramValue & 0xF0) == 0x70) && ((param & 0xF0) == 0x70)) {
switch (param & 0x0F) {
case 0x00:
s = _T("70: Past note cut");
break;
case 0x01:
s = _T("71: Past note off");
break;
case 0x02:
s = _T("72: Past note fade");
break;
case 0x03:
s = _T("73: NNA note cut");
break;
case 0x04:
s = _T("74: NNA continue");
break;
case 0x05:
s = _T("75: NNA note off");
break;
case 0x06:
s = _T("76: NNA note fade");
break;
case 0x07:
s = _T("77: Volume Env Off");
break;
case 0x08:
s = _T("78: Volume Env On");
break;
case 0x09:
s = _T("79: Pan Env Off");
break;
...
break;
case 0x0B:
s = _T("7B: Pitch Env Off");
break;
case 0x0C:
s = _T("7C: Pitch Env On");
break;
case 0x0D:
if(sndFile.GetType() == MOD_TYPE_MPT) {
s = _T("7D: Force Pitch Env");
break;
}
[[fallthrough]];
case 0x0E:
if(sndFile.GetType() == MOD_TYPE_MPT) {
s = _T("7E: Force Filter Env");
break;
}
[[fallthrough]];
default:
...
static constexpr MPTVolCmdInfo
gVolCmdInfo[] =
{
{
...
static_assert(mpt::array_size<decltype(gVolCmdInfo)>::size
== (MAX_VOLCMDS - 1));
...
if(s) {
s->Format(_T("%c: %s"), sndFile.GetModSpecifications().GetVolEffectLetter(GetVolCmdFromIndex(ndx)),
          gVolCmdInfo[ndx].name);
}
if((prangeMin) && (prangeMax)) {

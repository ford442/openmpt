...
std::vector <std::reference_wrapper<SmpLength>> loopPoints = {smp.nLoopStart, smp.nLoopEnd, smp.nSustainStart,
                                                              smp.nSustainEnd};
...
if(silenceLength == 0 || silenceLength > MAX_SAMPLE_LENGTH || smp.nLength > MAX_SAMPLE_LENGTH - silenceLength ||
   startFrom > smp.nLength)
return smp.nLength;
...
memcpy(pNewSmp + silenceOffset + silenceBytes, smp.sampleb() + silenceOffset,
       smp.GetSampleSizeInBytes() - silenceOffset);
}
...
void
AdjustLoopPoints(SmpLength selStart, SmpLength selEnd, SmpLength &loopStart, SmpLength &loopEnd, SmpLength length) {
...
template<typename T>
static void
ApplyAmplifyImpl(T *MPT_RESTRICT pSample, const SmpLength length, const double amplifyStart, const double amplifyEnd,
                 const bool isFadeIn, const Fade::Law fadeLaw) {
...
bool
AmplifySample(ModSample &smp, SmpLength start, SmpLength end, double amplifyStart, double amplifyEnd, bool isFadeIn,
              Fade::Law fadeLaw, CSoundFile &sndFile) {
...
bool XFadeSample(ModSample &smp, SmpLength fadeLength, int fadeLaw, bool afterloopFade, bool useSustainLoop,
                 CSoundFile &sndFile) {
...
return false;
CopySample<SC::ConversionChain < SC::Convert < int8, int16>, SC::DecodeIdentity <
                                                             int16>>>(static_cast<int8 *>(smp.samplev()), smp.nLength *
                                                                                                          smp.GetNumChannels(), 1, smp.sample16(), smp.GetSampleSizeInBytes(), 1);
...
return false;
CopySample<SC::ConversionChain < SC::Convert < int16, int8>, SC::DecodeIdentity < int8>>>(newSample, smp.nLength *
                                                                                                     smp.GetNumChannels(), 1, smp.sample8(), smp.GetSampleSizeInBytes(), 1);
